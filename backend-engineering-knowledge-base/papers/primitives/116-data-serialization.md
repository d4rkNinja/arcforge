...
**JSON**, **Protobuf**, **Binary formats**, **Null semantics**, **Date representation**, **Unknown fields**, **Payload size**.
...
**Enum evolution**, **Backward compatibility**.

### Boundary of the paper

This paper treats **Data Serialization** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — JSON:** Define the exact semantics of **JSON** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — MessagePack:** Define the exact semantics of **MessagePack** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Null semantics:** Define the exact semantics of **Null semantics** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Date representation:** Define the exact semantics of **Date representation** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Unknown fields:** Specify presence, null, empty, whitespace-only, default, and unknown-field semantics separately for create, replace, and partial update.
- **SHOULD — Payload size:** Define the exact semantics of **Payload size** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|---|
| JSON vs Protobuf/binary serialization | JSON is inspectable and flexible but ambiguous around numbers/nulls; Protobuf is compact and typed but requires strict field evolution. | Choose by ecosystem and performance; specify semantic compatibility separately. |
...
- **SHOULD — engineering rule:** Define the exact semantics of **JSON** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for json is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for json, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Protobuf

- **SHOULD — engineering rule:** Define the exact semantics of **Protobuf** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for protobuf is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for protobuf, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. MessagePack

- **SHOULD — engineering rule:** Define the exact semantics of **MessagePack** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for messagepack is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for messagepack, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Binary formats

- **SHOULD — engineering rule:** Define the exact semantics of **Binary formats** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for binary formats is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for binary formats, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Null semantics

- **SHOULD — engineering rule:** Define the exact semantics of **Null semantics** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for null semantics is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for null semantics, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Numeric precision

- **SHOULD — engineering rule:** Define the exact semantics of **Numeric precision** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for numeric precision is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for numeric precision, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Date representation

- **SHOULD — engineering rule:** Define the exact semantics of **Date representation** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for date representation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for date representation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Enum evolution

- **SHOULD — engineering rule:** Define the exact semantics of **Enum evolution** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for enum evolution is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for enum evolution, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Payload size** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for payload size is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for payload size, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Data Serialization path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `JSON`, `MessagePack`, `Numeric precision`. |
...
| Privacy / retention | Classify data produced or touched by Data Serialization; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Data Serialization** before choosing framework or provider mechanisms.
...
- **MUST** — For **JSON**: Define the exact semantics of **JSON** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **MessagePack**: Define the exact semantics of **MessagePack** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Null semantics**: Define the exact semantics of **Null semantics** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Date representation**: Define the exact semantics of **Date representation** within Data Serialization: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **JSON vs Protobuf/binary serialization** according to the stated trade-off: Choose by ecosystem and performance; specify semantic compatibility separately.
...
- [ ] **JSON:** Locate every implementation path for json, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **MessagePack:** Locate every implementation path for messagepack, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Null semantics:** Locate every implementation path for null semantics, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Date representation:** Locate every implementation path for date representation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Payload size:** Locate every implementation path for payload size, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **JSON:** A framework or provider default for json is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **MessagePack:** A framework or provider default for messagepack is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Binary formats:** A framework or provider default for binary formats is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Numeric precision:** A framework or provider default for numeric precision is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Enum evolution:** A framework or provider default for enum evolution is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Payload size:** A framework or provider default for payload size is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Data Serialization** correct, and which component is authoritative for it?
...
- For **JSON**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for json is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **MessagePack**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for messagepack is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Null semantics**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for null semantics is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Date representation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for date representation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Unknown fields**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Clients cannot clear a field, defaults overwrite explicit values, or silent unknown fields hide typos and compatibility bugs.
...
- [ ] Map every entry point for **Data Serialization**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **JSON:** Locate every implementation path for json, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **MessagePack:** Locate every implementation path for messagepack, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Binary formats:** Locate every implementation path for binary formats, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Numeric precision:** Locate every implementation path for numeric precision, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Enum evolution:** Locate every implementation path for enum evolution, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Unknown fields:** Build a contract matrix for omitted/null/empty/zero/false/unknown values and verify serialization round trips.
- [ ] **Payload size:** Locate every implementation path for payload size, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [117. Compression](117-compression.md) — layer: `primitives`; profile: `performance`.
...
**Paper metadata:** canonical subtopics: 11; layer: `primitives`; domain profile: `api`; verified through: `2026-08-17`.
