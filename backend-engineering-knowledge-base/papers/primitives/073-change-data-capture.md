...
**Transaction logs**, **Database change events**.

### Concurrency and distributed behavior

**CDC streams**, **Ordering**, **Duplicate events**, **Search synchronization**.

### Testing and evolution

**Consumers**, **Backfills**, **Replay**, **Offset management**.

### Boundary of the paper

This paper treats **Change Data Capture** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Change Data Capture path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Transaction logs`, `Database change events`, `Schema changes`. |
...
| Privacy / retention | Classify data produced or touched by Change Data Capture; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Change Data Capture** before choosing framework or provider mechanisms.
...
- **Transaction logs:** A framework or provider default for transaction logs is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Database change events:** A framework or provider default for database change events is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Schema changes:** A framework or provider default for schema changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Replay:** A framework or provider default for replay is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Offset management:** A framework or provider default for offset management is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Search synchronization:** A framework or provider default for search synchronization is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Change Data Capture** correct, and which component is authoritative for it?
...
- [ ] Map every entry point for **Change Data Capture**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Schema changes:** Locate every implementation path for schema changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Replay:** Locate every implementation path for replay, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Offset management:** Locate every implementation path for offset management, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Search synchronization:** Locate every implementation path for search synchronization, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
