...
**States**, **Allowed transitions**, **Forbidden transitions**, **Transition guards**, **Terminal states**, **Transition history**, **Concurrent transitions**, **Idempotent transitions**, **Recovery**, **Invalid-state repair**.
...
This paper treats **State Machines** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — States:** Define the exact semantics of **States** within State Machines: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Allowed transitions:** Model transitions as commands with expected current state/version, guard evaluation, atomic state/history update, and idempotent side-effect triggering.
- **SHOULD — Transition guards:** Model transitions as commands with expected current state/version, guard evaluation, atomic state/history update, and idempotent side-effect triggering.
- **SHOULD — Transition history:** Define the exact semantics of **Transition history** within State Machines: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Idempotent transitions:** Define the exact semantics of **Idempotent transitions** within State Machines: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Invalid-state repair:** Define the exact semantics of **Invalid-state repair** within State Machines: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|---|
| State column vs explicit transition ledger | A single state is efficient; a transition ledger preserves causality and audit but needs consistency with current state. | Keep authoritative current state and append transition history in one transaction where history matters. |
...
- **SHOULD — engineering rule:** Define the exact semantics of **States** within State Machines: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for states is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for states, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Events

- **SHOULD — engineering rule:** Define the exact semantics of **Events** within State Machines: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for events is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for events, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Allowed transitions

- **SHOULD — engineering rule:** Model transitions as commands with expected current state/version, guard evaluation, atomic state/history update, and idempotent side-effect triggering.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Forbidden transitions** within State Machines: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for forbidden transitions is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for forbidden transitions, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Transition guards
...
- **SHOULD — engineering rule:** Define the exact semantics of **Side effects** within State Machines: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for side effects is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for side effects, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.8. Transition history

- **SHOULD — engineering rule:** Define the exact semantics of **Transition history** within State Machines: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for transition history is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for transition history, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Concurrent transitions
...
### 7.10. Idempotent transitions

- **MUST — engineering rule:** Define the exact semantics of **Idempotent transitions** within State Machines: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for idempotent transitions is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for idempotent transitions, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Recovery

- **MUST — engineering rule:** Define the exact semantics of **Recovery** within State Machines: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.12. Invalid-state repair

- **SHOULD — engineering rule:** Define the exact semantics of **Invalid-state repair** within State Machines: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for invalid-state repair is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for invalid-state repair, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every State Machines path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `States`, `Forbidden transitions`, `Terminal states`. |
...
| Privacy / retention | Classify data produced or touched by State Machines; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **State Machines** before choosing framework or provider mechanisms.
...
- **MUST** — For **States**: Define the exact semantics of **States** within State Machines: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Allowed transitions**: Model transitions as commands with expected current state/version, guard evaluation, atomic state/history update, and idempotent side-effect triggering.
- **MUST** — For **Transition guards**: Model transitions as commands with expected current state/version, guard evaluation, atomic state/history update, and idempotent side-effect triggering.
- **MUST** — For **Transition history**: Define the exact semantics of **Transition history** within State Machines: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **State column vs explicit transition ledger** according to the stated trade-off: Keep authoritative current state and append transition history in one transaction where history matters.
...
- [ ] **States:** Locate every implementation path for states, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Allowed transitions:** Generate the state-transition matrix and test every allowed/forbidden pair plus simultaneous conflicting commands.
- [ ] **Transition guards:** Generate the state-transition matrix and test every allowed/forbidden pair plus simultaneous conflicting commands.
- [ ] **Transition history:** Locate every implementation path for transition history, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Idempotent transitions:** Locate every implementation path for idempotent transitions, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Invalid-state repair:** Locate every implementation path for invalid-state repair, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **States:** A framework or provider default for states is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Allowed transitions:** Generic updates skip guards, concurrent transitions both succeed, or terminal records are silently reopened.
- **Transition guards:** Generic updates skip guards, concurrent transitions both succeed, or terminal records are silently reopened.
- **Terminal states:** Generic updates skip guards, concurrent transitions both succeed, or terminal records are silently reopened.
- **Transition history:** A framework or provider default for transition history is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Idempotent transitions:** A framework or provider default for idempotent transitions is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Invalid-state repair:** A framework or provider default for invalid-state repair is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **State Machines** correct, and which component is authoritative for it?
...
- For **States**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for states is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Allowed transitions**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Generic updates skip guards, concurrent transitions both succeed, or terminal records are silently reopened.
- For **Transition guards**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Generic updates skip guards, concurrent transitions both succeed, or terminal records are silently reopened.
- For **Transition history**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for transition history is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Idempotent transitions**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for idempotent transitions is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **State Machines**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Allowed transitions:** Generate the state-transition matrix and test every allowed/forbidden pair plus simultaneous conflicting commands.
- [ ] **Transition guards:** Generate the state-transition matrix and test every allowed/forbidden pair plus simultaneous conflicting commands.
- [ ] **Terminal states:** Generate the state-transition matrix and test every allowed/forbidden pair plus simultaneous conflicting commands.
- [ ] **Transition history:** Locate every implementation path for transition history, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Idempotent transitions:** Locate every implementation path for idempotent transitions, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Invalid-state repair:** Locate every implementation path for invalid-state repair, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [046. Event Systems](../systems/046-event-systems.md) — layer: `systems`; profile: `async`.
...
**Paper metadata:** canonical subtopics: 12; layer: `primitives`; domain profile: `transactions`; verified through: `2026-08-17`.
