...
**Domain errors**, **Validation errors**, **Not-found errors**, **Conflict errors**, **Infrastructure errors**, **External-provider errors**, **Retryable errors**, **Non-retryable errors**, **Error codes**, **HTTP status mapping**, **Public vs internal error details**, **Error wrapping**, **Error propagation**, **Localization**, **Structured errors**, **Error correlation**.
...
This paper treats **Error Architecture** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Domain errors:** Define the exact semantics of **Domain errors** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Not-found errors:** Define the exact semantics of **Not-found errors** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — External-provider errors:** Classify provider failures, set deadlines and bulkheads, respect rate limits, preserve request identity for safe retry/reconciliation, and define whether fallback changes semantics, data residency, or user consent.
- **SHOULD — Error codes:** Define stable machine codes independent of localized text; map domain outcomes consistently to protocol status; expose recovery-relevant detail while retaining internal cause and correlation privately.
- **SHOULD — Error propagation:** Define the exact semantics of **Error propagation** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Error correlation:** Define the exact semantics of **Error correlation** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Domain errors** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for domain errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for domain errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Validation errors

- **MUST — engineering rule:** Define the exact semantics of **Validation errors** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for validation errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for validation errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Authentication errors

- **MUST — engineering rule:** Define the exact semantics of **Authentication errors** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for authentication errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for authentication errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Authorization errors

- **MUST — engineering rule:** Define the exact semantics of **Authorization errors** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for authorization errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for authorization errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Not-found errors

- **SHOULD — engineering rule:** Define the exact semantics of **Not-found errors** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for not-found errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for not-found errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Conflict errors

- **SHOULD — engineering rule:** Define the exact semantics of **Conflict errors** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for conflict errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for conflict errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Infrastructure errors

- **SHOULD — engineering rule:** Define the exact semantics of **Infrastructure errors** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for infrastructure errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for infrastructure errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Database errors

- **SHOULD — engineering rule:** Define the exact semantics of **Database errors** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for database errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for database errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. External-provider errors
...
- **SHOULD — engineering rule:** Define stable machine codes independent of localized text; map domain outcomes consistently to protocol status; expose recovery-relevant detail while retaining internal cause and correlation privately.
- **Production failure mode:** Clients parse human text, sensitive internals leak, or the same failure is represented differently across endpoints.
- **Existing-codebase evidence:** Snapshot the public error schema, verify redaction, and test wrapping of database/provider errors without losing classification.

### 7.13. HTTP status mapping

- **SHOULD — engineering rule:** Define stable machine codes independent of localized text; map domain outcomes consistently to protocol status; expose recovery-relevant detail while retaining internal cause and correlation privately.
- **Production failure mode:** Clients parse human text, sensitive internals leak, or the same failure is represented differently across endpoints.
- **Existing-codebase evidence:** Snapshot the public error schema, verify redaction, and test wrapping of database/provider errors without losing classification.

### 7.14. Public vs internal error details
...
- **SHOULD — engineering rule:** Define the exact semantics of **Error wrapping** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for error wrapping is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for error wrapping, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.16. Error propagation

- **SHOULD — engineering rule:** Define the exact semantics of **Error propagation** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for error propagation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for error propagation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.17. Sensitive-data masking
...
- **SHOULD — engineering rule:** Define the exact semantics of **Localization** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for localization is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for localization, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.19. Structured errors

- **SHOULD — engineering rule:** Define stable machine codes independent of localized text; map domain outcomes consistently to protocol status; expose recovery-relevant detail while retaining internal cause and correlation privately.
- **Production failure mode:** Clients parse human text, sensitive internals leak, or the same failure is represented differently across endpoints.
- **Existing-codebase evidence:** Snapshot the public error schema, verify redaction, and test wrapping of database/provider errors without losing classification.

### 7.20. Error correlation

- **SHOULD — engineering rule:** Define the exact semantics of **Error correlation** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for error correlation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for error correlation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Error Architecture path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Domain errors`, `Conflict errors`, `Non-retryable errors`. |
...
| Privacy / retention | Classify data produced or touched by Error Architecture; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Error Architecture** before choosing framework or provider mechanisms.
...
- **MUST** — For **Domain errors**: Define the exact semantics of **Domain errors** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Not-found errors**: Define the exact semantics of **Not-found errors** within Error Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **External-provider errors**: Classify provider failures, set deadlines and bulkheads, respect rate limits, preserve request identity for safe retry/reconciliation, and define whether fallback changes semantics, data residency, or user consent.
- **MUST** — For **Error codes**: Define stable machine codes independent of localized text; map domain outcomes consistently to protocol status; expose recovery-relevant detail while retaining internal cause and correlation privately.
...
- [ ] **Domain errors:** Locate every implementation path for domain errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Not-found errors:** Locate every implementation path for not-found errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **External-provider errors:** Inject DNS/TLS/timeout/429/5xx/partial-success cases and reconcile provider state after ambiguous outcomes.
- [ ] **Error codes:** Snapshot the public error schema, verify redaction, and test wrapping of database/provider errors without losing classification.
- [ ] **Error propagation:** Locate every implementation path for error propagation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Error correlation:** Locate every implementation path for error correlation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Domain errors:** A framework or provider default for domain errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Authorization errors:** A framework or provider default for authorization errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Infrastructure errors:** A framework or provider default for infrastructure errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Non-retryable errors:** Permanent failures loop forever, retries synchronize into a storm, or a timed-out successful operation is duplicated.
- **Public vs internal error details:** Clients parse human text, sensitive internals leak, or the same failure is represented differently across endpoints.
- **Sensitive-data masking:** Sensitive data spreads into logs, caches, search, analytics, and backups where deletion and access controls are weaker.
- **Error correlation:** A framework or provider default for error correlation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Error Architecture** correct, and which component is authoritative for it?
...
- For **Domain errors**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for domain errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Not-found errors**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for not-found errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **External-provider errors**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: The system retries unsafe requests, sends duplicates to two providers, or silently degrades to a provider with incompatible behavior.
- For **Error codes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Clients parse human text, sensitive internals leak, or the same failure is represented differently across endpoints.
- For **Error propagation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for error propagation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Error Architecture**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Domain errors:** Locate every implementation path for domain errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Authorization errors:** Locate every implementation path for authorization errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Infrastructure errors:** Locate every implementation path for infrastructure errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Non-retryable errors:** Inject each error class and verify attempt count, schedule, deadline budget, duplicate behavior, and terminal routing.
- [ ] **Public vs internal error details:** Snapshot the public error schema, verify redaction, and test wrapping of database/provider errors without losing classification.
- [ ] **Sensitive-data masking:** Build a data-flow inventory and verify each copy's lawful purpose, encryption, access, retention, export, and deletion behavior.
- [ ] **Error correlation:** Locate every implementation path for error correlation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [012. Input Validation](012-input-validation.md) — layer: `primitives`; profile: `api`.
...
- [110. SDK / Client Design](../systems/110-sdk-client-design.md) — layer: `systems`; profile: `client`.
...
- [016. Pagination](016-pagination.md) — layer: `primitives`; profile: `api`.
...
**Paper metadata:** canonical subtopics: 20; layer: `primitives`; domain profile: `api`; verified through: `2026-08-17`.
