...
**Soft Delete / Hard Delete** exists to encode domain facts and invariants so valid states are easy to represent and invalid states are difficult to persist. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
- **SHOULD — Deleted-at fields:** Model deletion as explicit state with actor/reason/time, default query scoping, uniqueness behavior, reference policy, retention horizon, restoration rules, and irreversible purge workflow.
- **MUST — Unique constraints with soft deletion:** Enforce race-sensitive uniqueness in the authoritative database with the correct scope, normalization, null, tenant, and deletion semantics; translate violations into deterministic domain conflicts.
- **SHOULD — Restoration:** Model deletion as explicit state with actor/reason/time, default query scoping, uniqueness behavior, reference policy, retention horizon, restoration rules, and irreversible purge workflow.
- **MUST — Retention:** Define the exact semantics of **Retention** within Soft Delete / Hard Delete: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — References to deleted records:** Define the exact semantics of **References to deleted records** within Soft Delete / Hard Delete: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Privacy deletion:** Define the exact semantics of **Privacy deletion** within Soft Delete / Hard Delete: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.3. Unique constraints with soft deletion
...
- **MUST — engineering rule:** Define the exact semantics of **Retention** within Soft Delete / Hard Delete: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.9. References to deleted records

- **SHOULD — engineering rule:** Define the exact semantics of **References to deleted records** within Soft Delete / Hard Delete: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for references to deleted records is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for references to deleted records, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.11. Privacy deletion

- **MUST — engineering rule:** Define the exact semantics of **Privacy deletion** within Soft Delete / Hard Delete: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for privacy deletion is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for privacy deletion, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Soft Delete / Hard Delete path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Deleted-at fields`, `Unique constraints with soft deletion`, `Purging`. |
...
| Privacy / retention | Classify data produced or touched by Soft Delete / Hard Delete; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Soft Delete / Hard Delete** before choosing framework or provider mechanisms.
...
- **MUST** — For **Deleted-at fields**: Model deletion as explicit state with actor/reason/time, default query scoping, uniqueness behavior, reference policy, retention horizon, restoration rules, and irreversible purge workflow.
- **MUST** — For **Unique constraints with soft deletion**: Enforce race-sensitive uniqueness in the authoritative database with the correct scope, normalization, null, tenant, and deletion semantics; translate violations into deterministic domain conflicts.
- **MUST** — For **Restoration**: Model deletion as explicit state with actor/reason/time, default query scoping, uniqueness behavior, reference policy, retention horizon, restoration rules, and irreversible purge workflow.
- **MUST** — For **Retention**: Define the exact semantics of **Retention** within Soft Delete / Hard Delete: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Deleted-at fields:** Search every query/index/cache/export path for deletion semantics and test delete/restore/reuse races.
- [ ] **Unique constraints with soft deletion:** Run concurrent inserts and inspect the exact index/constraint definition, collation, partial predicate, and error mapping.
- [ ] **Restoration:** Search every query/index/cache/export path for deletion semantics and test delete/restore/reuse races.
...
- [ ] **References to deleted records:** Locate every implementation path for references to deleted records, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Privacy deletion:** Locate every implementation path for privacy deletion, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Deleted-at fields:** Deleted rows leak through joins/caches, block legitimate reuse, or are purged while still referenced or under legal hold.
- **Unique constraints with soft deletion:** Two concurrent check-then-insert paths create duplicates, or soft-deleted/null values behave differently than application assumptions.
- **Cascading soft delete:** Deleted rows leak through joins/caches, block legitimate reuse, or are purged while still referenced or under legal hold.
- **Purging:** Deleted rows leak through joins/caches, block legitimate reuse, or are purged while still referenced or under legal hold.
- **Orphan cleanup:** Partial failure or deletion leaves references that crash reads, leak data, or prevent retention enforcement.
- **References to deleted records:** A framework or provider default for references to deleted records is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Privacy deletion:** A framework or provider default for privacy deletion is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Soft Delete / Hard Delete** correct, and which component is authoritative for it?
...
- For **Deleted-at fields**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Deleted rows leak through joins/caches, block legitimate reuse, or are purged while still referenced or under legal hold.
- For **Unique constraints with soft deletion**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Two concurrent check-then-insert paths create duplicates, or soft-deleted/null values behave differently than application assumptions.
- For **Restoration**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Deleted rows leak through joins/caches, block legitimate reuse, or are purged while still referenced or under legal hold.
...
- For **References to deleted records**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for references to deleted records is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Soft Delete / Hard Delete**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Deleted-at fields:** Search every query/index/cache/export path for deletion semantics and test delete/restore/reuse races.
- [ ] **Unique constraints with soft deletion:** Run concurrent inserts and inspect the exact index/constraint definition, collation, partial predicate, and error mapping.
- [ ] **Cascading soft delete:** Search every query/index/cache/export path for deletion semantics and test delete/restore/reuse races.
- [ ] **Purging:** Search every query/index/cache/export path for deletion semantics and test delete/restore/reuse races.
- [ ] **Orphan cleanup:** Delete and restore principals/resources under concurrent writes, then run integrity queries for every relationship.
- [ ] **References to deleted records:** Locate every implementation path for references to deleted records, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Privacy deletion:** Locate every implementation path for privacy deletion, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
**Paper metadata:** canonical subtopics: 11; layer: `primitives`; domain profile: `data_model`; verified through: `2026-08-17`.
