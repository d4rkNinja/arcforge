---
paper_number: 17
title: "Filtering / Sorting / Query APIs"
layer: primitives
domain_profile: api
...
# 017. Filtering / Sorting / Query APIs
...
**Filtering**, **Sorting**, **Search parameters**, **Multiple filters**, **Range filters**, **Date filters**, **Null filters**, **Nested filters**, **Allowed fields**, **Operator validation**.

### Persistence and integrity

**Query complexity limits**, **Index-aware filtering**, **Stable sorting**, **User-controlled query safety**.

### Boundary of the paper

This paper treats **Filtering / Sorting / Query APIs** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Filtering:** Parse filters into a typed allowlisted AST, validate operators and complexity, bind parameters, combine mandatory tenant/auth predicates, and cap execution cost.
- **SHOULD — Multiple filters:** Parse filters into a typed allowlisted AST, validate operators and complexity, bind parameters, combine mandatory tenant/auth predicates, and cap execution cost.
- **SHOULD — Date filters:** Parse filters into a typed allowlisted AST, validate operators and complexity, bind parameters, combine mandatory tenant/auth predicates, and cap execution cost.
- **SHOULD — Allowed fields:** Parse filters into a typed allowlisted AST, validate operators and complexity, bind parameters, combine mandatory tenant/auth predicates, and cap execution cost.
- **SHOULD — Query complexity limits:** Parse filters into a typed allowlisted AST, validate operators and complexity, bind parameters, combine mandatory tenant/auth predicates, and cap execution cost.
- **SHOULD — User-controlled query safety:** Parse filters into a typed allowlisted AST, validate operators and complexity, bind parameters, combine mandatory tenant/auth predicates, and cap execution cost.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Search parameters** within Filtering / Sorting / Query APIs: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for search parameters is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for search parameters, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Multiple filters
...
### 7.10. Operator validation

- **MUST — engineering rule:** Define the exact semantics of **Operator validation** within Filtering / Sorting / Query APIs: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for operator validation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for operator validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Query complexity limits

- **SHOULD — engineering rule:** Parse filters into a typed allowlisted AST, validate operators and complexity, bind parameters, combine mandatory tenant/auth predicates, and cap execution cost.
- **Production failure mode:** Injection, authorization bypass, unbounded scans, or query-plan explosion occurs when raw user expressions reach the database.
- **Existing-codebase evidence:** Fuzz operators and nesting, verify mandatory predicates cannot be overridden, and benchmark worst allowed combinations.

### 7.12. Index-aware filtering
...
### 7.14. User-controlled query safety
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Filtering / Sorting / Query APIs path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Filtering`, `Multiple filters`, `Null filters`. |
...
| Privacy / retention | Classify data produced or touched by Filtering / Sorting / Query APIs; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Filtering / Sorting / Query APIs** before choosing framework or provider mechanisms.
...
- **MUST** — For **Filtering**: Parse filters into a typed allowlisted AST, validate operators and complexity, bind parameters, combine mandatory tenant/auth predicates, and cap execution cost.
- **MUST** — For **Multiple filters**: Parse filters into a typed allowlisted AST, validate operators and complexity, bind parameters, combine mandatory tenant/auth predicates, and cap execution cost.
- **MUST** — For **Date filters**: Parse filters into a typed allowlisted AST, validate operators and complexity, bind parameters, combine mandatory tenant/auth predicates, and cap execution cost.
- **MUST** — For **Allowed fields**: Parse filters into a typed allowlisted AST, validate operators and complexity, bind parameters, combine mandatory tenant/auth predicates, and cap execution cost.
...
- [ ] **Filtering:** Fuzz operators and nesting, verify mandatory predicates cannot be overridden, and benchmark worst allowed combinations.
- [ ] **Multiple filters:** Fuzz operators and nesting, verify mandatory predicates cannot be overridden, and benchmark worst allowed combinations.
- [ ] **Date filters:** Fuzz operators and nesting, verify mandatory predicates cannot be overridden, and benchmark worst allowed combinations.
- [ ] **Allowed fields:** Fuzz operators and nesting, verify mandatory predicates cannot be overridden, and benchmark worst allowed combinations.
- [ ] **Query complexity limits:** Fuzz operators and nesting, verify mandatory predicates cannot be overridden, and benchmark worst allowed combinations.
- [ ] **User-controlled query safety:** Fuzz operators and nesting, verify mandatory predicates cannot be overridden, and benchmark worst allowed combinations.
...
- **Filtering:** Injection, authorization bypass, unbounded scans, or query-plan explosion occurs when raw user expressions reach the database.
- **Search parameters:** A framework or provider default for search parameters is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Range filters:** Injection, authorization bypass, unbounded scans, or query-plan explosion occurs when raw user expressions reach the database.
- **Null filters:** Injection, authorization bypass, unbounded scans, or query-plan explosion occurs when raw user expressions reach the database.
- **Operator validation:** A framework or provider default for operator validation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Index-aware filtering:** Injection, authorization bypass, unbounded scans, or query-plan explosion occurs when raw user expressions reach the database.
- **User-controlled query safety:** Injection, authorization bypass, unbounded scans, or query-plan explosion occurs when raw user expressions reach the database.
...
- What exact invariant or user/system promise makes **Filtering / Sorting / Query APIs** correct, and which component is authoritative for it?
...
- For **Filtering**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Injection, authorization bypass, unbounded scans, or query-plan explosion occurs when raw user expressions reach the database.
- For **Multiple filters**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Injection, authorization bypass, unbounded scans, or query-plan explosion occurs when raw user expressions reach the database.
- For **Date filters**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Injection, authorization bypass, unbounded scans, or query-plan explosion occurs when raw user expressions reach the database.
- For **Allowed fields**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Injection, authorization bypass, unbounded scans, or query-plan explosion occurs when raw user expressions reach the database.
- For **Query complexity limits**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Injection, authorization bypass, unbounded scans, or query-plan explosion occurs when raw user expressions reach the database.
...
- [ ] Map every entry point for **Filtering / Sorting / Query APIs**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Filtering:** Fuzz operators and nesting, verify mandatory predicates cannot be overridden, and benchmark worst allowed combinations.
- [ ] **Search parameters:** Locate every implementation path for search parameters, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Range filters:** Fuzz operators and nesting, verify mandatory predicates cannot be overridden, and benchmark worst allowed combinations.
- [ ] **Null filters:** Fuzz operators and nesting, verify mandatory predicates cannot be overridden, and benchmark worst allowed combinations.
- [ ] **Operator validation:** Locate every implementation path for operator validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Index-aware filtering:** Fuzz operators and nesting, verify mandatory predicates cannot be overridden, and benchmark worst allowed combinations.
- [ ] **User-controlled query safety:** Fuzz operators and nesting, verify mandatory predicates cannot be overridden, and benchmark worst allowed combinations.
...
- [042. Search](../systems/042-search.md) — layer: `systems`; profile: `data_model`.
...
**Paper metadata:** canonical subtopics: 14; layer: `primitives`; domain profile: `api`; verified through: `2026-08-17`.
