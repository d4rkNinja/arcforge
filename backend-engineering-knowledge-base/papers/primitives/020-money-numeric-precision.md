...
> Keep this technical rather than financial-domain specific.
...
**Integer representation**, **Decimal representation**, **Floating-point risks**, **Fixed precision**, **Rounding**, **Rounding modes**, **Currency precision**, **Overflow**, **Underflow**, **Serialization precision**, **Database precision**, **Calculation determinism**.

### Boundary of the paper

This paper treats **Money / Numeric Precision** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Integer representation:** Define the exact semantics of **Integer representation** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Floating-point risks:** Define the exact semantics of **Floating-point risks** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Rounding modes:** Define the exact semantics of **Rounding modes** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Overflow:** Define the exact semantics of **Overflow** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Serialization precision:** Define the exact semantics of **Serialization precision** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Calculation determinism:** Define the exact semantics of **Calculation determinism** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.1. Integer representation

- **SHOULD — engineering rule:** Define the exact semantics of **Integer representation** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for integer representation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for integer representation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Decimal representation

- **SHOULD — engineering rule:** Define the exact semantics of **Decimal representation** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for decimal representation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for decimal representation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Floating-point risks

- **SHOULD — engineering rule:** Define the exact semantics of **Floating-point risks** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for floating-point risks is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for floating-point risks, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Fixed precision

- **SHOULD — engineering rule:** Define the exact semantics of **Fixed precision** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for fixed precision is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for fixed precision, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Rounding

- **SHOULD — engineering rule:** Define the exact semantics of **Rounding** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for rounding is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for rounding, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Rounding modes

- **SHOULD — engineering rule:** Define the exact semantics of **Rounding modes** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for rounding modes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for rounding modes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Currency precision

- **SHOULD — engineering rule:** Define the exact semantics of **Currency precision** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for currency precision is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for currency precision, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Overflow

- **SHOULD — engineering rule:** Define the exact semantics of **Overflow** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for overflow is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for overflow, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Underflow

- **SHOULD — engineering rule:** Define the exact semantics of **Underflow** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for underflow is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for underflow, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Numeric validation

- **MUST — engineering rule:** Define the exact semantics of **Numeric validation** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for numeric validation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for numeric validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Serialization precision

- **SHOULD — engineering rule:** Define the exact semantics of **Serialization precision** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for serialization precision is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for serialization precision, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.12. Database precision

- **SHOULD — engineering rule:** Define the exact semantics of **Database precision** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for database precision is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for database precision, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. Calculation determinism

- **SHOULD — engineering rule:** Define the exact semantics of **Calculation determinism** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for calculation determinism is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for calculation determinism, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST** — For **Integer representation**: Define the exact semantics of **Integer representation** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Floating-point risks**: Define the exact semantics of **Floating-point risks** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Rounding modes**: Define the exact semantics of **Rounding modes** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Overflow**: Define the exact semantics of **Overflow** within Money / Numeric Precision: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Integer representation:** Locate every implementation path for integer representation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Floating-point risks:** Locate every implementation path for floating-point risks, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Rounding modes:** Locate every implementation path for rounding modes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Overflow:** Locate every implementation path for overflow, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Serialization precision:** Locate every implementation path for serialization precision, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Calculation determinism:** Locate every implementation path for calculation determinism, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Integer representation:** A framework or provider default for integer representation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Floating-point risks:** A framework or provider default for floating-point risks is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Rounding:** A framework or provider default for rounding is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Currency precision:** A framework or provider default for currency precision is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Underflow:** A framework or provider default for underflow is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Serialization precision:** A framework or provider default for serialization precision is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Calculation determinism:** A framework or provider default for calculation determinism is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Integer representation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for integer representation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Floating-point risks**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for floating-point risks is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Rounding modes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for rounding modes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Overflow**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for overflow is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Serialization precision**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for serialization precision is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] **Integer representation:** Locate every implementation path for integer representation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Floating-point risks:** Locate every implementation path for floating-point risks, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Rounding:** Locate every implementation path for rounding, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Currency precision:** Locate every implementation path for currency precision, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Underflow:** Locate every implementation path for underflow, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Serialization precision:** Locate every implementation path for serialization precision, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Calculation determinism:** Locate every implementation path for calculation determinism, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [034. Immutable Data](034-immutable-data.md) — layer: `primitives`; profile: `data_model`.
...
**Paper metadata:** canonical subtopics: 13; layer: `primitives`; domain profile: `data_model`; verified through: `2026-08-17`.
