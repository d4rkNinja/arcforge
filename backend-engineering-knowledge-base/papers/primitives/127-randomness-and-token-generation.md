---
paper_number: 127
title: "Randomness & Token Generation"
layer: primitives
...
# 127. Randomness & Token Generation
...
**Cryptographic randomness**, **Collision probability**, **Guessability**, **Secure random APIs**, **Encoding**, **Hashing tokens at rest**.

### Boundary of the paper

This paper treats **Randomness & Token Generation** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **MUST — Cryptographic randomness:** Use well-reviewed libraries and current algorithms; define key purpose, scope, version, generation, storage, rotation, revocation, nonce uniqueness, and authenticated context.
- **SHOULD — Collision probability:** Define the exact semantics of **Collision probability** within Randomness & Token Generation: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Secure random APIs:** Define the exact semantics of **Secure random APIs** within Randomness & Token Generation: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Token length:** Define the exact semantics of **Token length** within Randomness & Token Generation: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Storage:** Define the exact semantics of **Storage** within Randomness & Token Generation: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Hashing tokens at rest:** Use well-reviewed libraries and current algorithms; define key purpose, scope, version, generation, storage, rotation, revocation, nonce uniqueness, and authenticated context.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Token entropy** within Randomness & Token Generation: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for token entropy is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for token entropy, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Collision probability

- **SHOULD — engineering rule:** Define the exact semantics of **Collision probability** within Randomness & Token Generation: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for collision probability is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for collision probability, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Guessability

- **SHOULD — engineering rule:** Define the exact semantics of **Guessability** within Randomness & Token Generation: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for guessability is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for guessability, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Secure random APIs

- **MUST — engineering rule:** Define the exact semantics of **Secure random APIs** within Randomness & Token Generation: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for secure random apis is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for secure random apis, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Token length

- **SHOULD — engineering rule:** Define the exact semantics of **Token length** within Randomness & Token Generation: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for token length is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for token length, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Encoding

- **SHOULD — engineering rule:** Define the exact semantics of **Encoding** within Randomness & Token Generation: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for encoding is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for encoding, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Storage

- **SHOULD — engineering rule:** Define the exact semantics of **Storage** within Randomness & Token Generation: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Randomness & Token Generation path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Cryptographic randomness`, `Collision probability`, `Secure random APIs`. |
...
| Privacy / retention | Classify data produced or touched by Randomness & Token Generation; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Randomness & Token Generation** before choosing framework or provider mechanisms.
...
- **MUST** — For **Cryptographic randomness**: Use well-reviewed libraries and current algorithms; define key purpose, scope, version, generation, storage, rotation, revocation, nonce uniqueness, and authenticated context.
- **MUST** — For **Collision probability**: Define the exact semantics of **Collision probability** within Randomness & Token Generation: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Secure random APIs**: Define the exact semantics of **Secure random APIs** within Randomness & Token Generation: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Token length**: Define the exact semantics of **Token length** within Randomness & Token Generation: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Cryptographic randomness:** Review against a cryptographic design document and test key rotation, corruption, wrong context/key, entropy failure, and legacy ciphertext.
- [ ] **Collision probability:** Locate every implementation path for collision probability, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Secure random APIs:** Locate every implementation path for secure random apis, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Token length:** Locate every implementation path for token length, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Hashing tokens at rest:** Review against a cryptographic design document and test key rotation, corruption, wrong context/key, entropy failure, and legacy ciphertext.
...
- **Cryptographic randomness:** Custom crypto, nonce reuse, weak randomness, key/algorithm confusion, or unauthenticated encryption destroys the intended guarantee.
- **Collision probability:** A framework or provider default for collision probability is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Guessability:** A framework or provider default for guessability is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Secure random APIs:** A framework or provider default for secure random apis is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Encoding:** A framework or provider default for encoding is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Hashing tokens at rest:** Custom crypto, nonce reuse, weak randomness, key/algorithm confusion, or unauthenticated encryption destroys the intended guarantee.
...
- What exact invariant or user/system promise makes **Randomness & Token Generation** correct, and which component is authoritative for it?
...
- For **Cryptographic randomness**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Custom crypto, nonce reuse, weak randomness, key/algorithm confusion, or unauthenticated encryption destroys the intended guarantee.
- For **Collision probability**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for collision probability is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Secure random APIs**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for secure random apis is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Token length**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for token length is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Storage**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for storage is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Randomness & Token Generation**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Cryptographic randomness:** Review against a cryptographic design document and test key rotation, corruption, wrong context/key, entropy failure, and legacy ciphertext.
- [ ] **Collision probability:** Locate every implementation path for collision probability, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Guessability:** Locate every implementation path for guessability, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Secure random APIs:** Locate every implementation path for secure random apis, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Encoding:** Locate every implementation path for encoding, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [126. Temporary Data](126-temporary-data.md) — layer: `primitives`; profile: `security`.
...
- [004. Authentication](../systems/004-authentication.md) — layer: `systems`; profile: `authentication`.
...
**Paper metadata:** canonical subtopics: 10; layer: `primitives`; domain profile: `security`; verified through: `2026-08-17`.
