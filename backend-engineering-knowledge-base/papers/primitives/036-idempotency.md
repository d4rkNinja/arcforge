...
**Idempotent API requests**, **Request fingerprinting**, **Response replay**.
...
**Idempotency keys**, **Key generation**, **Key scope**, **Concurrent duplicates**, **Failed first attempts**, **Retry safety**, **Job idempotency**, **Event idempotency**, **Webhook idempotency**.

### Boundary of the paper

This paper treats **Idempotency** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **MUST — Idempotent API requests:** Scope keys to caller and operation, bind them to a canonical request fingerprint, reserve atomically, persist terminal outcome, and distinguish in-progress, succeeded, retryable-failed, and permanently-failed states.
- **SHOULD — Key scope:** Define the exact semantics of **Key scope** within Idempotency: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Response replay:** Define the exact semantics of **Response replay** within Idempotency: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Concurrent duplicates:** Use barriers/hooks to force the critical interleaving, repeat across real database isolation, and assert the invariant rather than timing or one response.
- **SHOULD — Retry safety:** Classify retryable outcomes, cap attempts and elapsed time, use exponential backoff with jitter, honor provider pushback, and make side effects idempotent or detect ambiguous completion.
- **MUST — Webhook idempotency:** Model each delivery attempt with event ID, subscription version, endpoint, timeout, response class, next attempt, and terminal state. Bound retries and disable persistently failing or unsafe endpoints.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Key generation** within Idempotency: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for key generation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for key generation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Key scope

- **SHOULD — engineering rule:** Define the exact semantics of **Key scope** within Idempotency: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for key scope is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for key scope, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Request fingerprinting

- **SHOULD — engineering rule:** Define the exact semantics of **Request fingerprinting** within Idempotency: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for request fingerprinting is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for request fingerprinting, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Response replay

- **MUST — engineering rule:** Define the exact semantics of **Response replay** within Idempotency: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for response replay is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for response replay, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Storage

- **SHOULD — engineering rule:** Define the exact semantics of **Storage** within Idempotency: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for storage is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for storage, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.10. Failed first attempts

- **SHOULD — engineering rule:** Define the exact semantics of **Failed first attempts** within Idempotency: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for failed first attempts is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for failed first attempts, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.12. Job idempotency

- **MUST — engineering rule:** Define the exact semantics of **Job idempotency** within Idempotency: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for job idempotency is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for job idempotency, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. Event idempotency

- **MUST — engineering rule:** Define the exact semantics of **Event idempotency** within Idempotency: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for event idempotency is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for event idempotency, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.14. Webhook idempotency

- **MUST — engineering rule:** Model each delivery attempt with event ID, subscription version, endpoint, timeout, response class, next attempt, and terminal state. Bound retries and disable persistently failing or unsafe endpoints.
- **Production failure mode:** Slow consumers exhaust workers, duplicate/out-of-order deliveries corrupt state, or one endpoint causes unbounded retry cost.
- **Existing-codebase evidence:** Simulate timeout, 2xx-after-timeout, 4xx, 429, 5xx, DNS/TLS failure, endpoint changes, and replay.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Idempotency path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Idempotent API requests`, `Key scope`, `Storage`. |
...
| Privacy / retention | Classify data produced or touched by Idempotency; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Idempotency** before choosing framework or provider mechanisms.
...
- **MUST** — For **Idempotent API requests**: Scope keys to caller and operation, bind them to a canonical request fingerprint, reserve atomically, persist terminal outcome, and distinguish in-progress, succeeded, retryable-failed, and permanently-failed states.
- **MUST** — For **Key scope**: Define the exact semantics of **Key scope** within Idempotency: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Response replay**: Define the exact semantics of **Response replay** within Idempotency: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Concurrent duplicates**: Use barriers/hooks to force the critical interleaving, repeat across real database isolation, and assert the invariant rather than timing or one response.
...
- [ ] **Idempotent API requests:** Synchronize duplicate requests before first commit; test timeout-after-commit, key mismatch, expiry, and response replay.
- [ ] **Key scope:** Locate every implementation path for key scope, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Response replay:** Locate every implementation path for response replay, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Concurrent duplicates:** Instrument the exact read/check/write or lease boundary and run deterministic competing operations.
- [ ] **Retry safety:** Inject each error class and verify attempt count, schedule, deadline budget, duplicate behavior, and terminal routing.
- [ ] **Webhook idempotency:** Simulate timeout, 2xx-after-timeout, 4xx, 429, 5xx, DNS/TLS failure, endpoint changes, and replay.
...
- Lost updates from read-modify-write.
- Write skew across rows.
- Deadlocks retried at statement rather than transaction level.
- Side effect sent before commit then transaction rolls back.
- Expired lease holder continuing to write.
- **Idempotent API requests:** Two concurrent requests both execute, or the same key is reused with different parameters and returns the wrong result.
- **Key generation:** A framework or provider default for key generation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Request fingerprinting:** A framework or provider default for request fingerprinting is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Storage:** A framework or provider default for storage is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Failed first attempts:** A framework or provider default for failed first attempts is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Job idempotency:** A framework or provider default for job idempotency is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Webhook idempotency:** Slow consumers exhaust workers, duplicate/out-of-order deliveries corrupt state, or one endpoint causes unbounded retry cost.

## 18. AI coding-agent failure modes

An AI agent is especially likely to:

- Holding a transaction open across network calls.
- Retrying only the failed statement instead of the transaction.
- Assuming timeout means rollback.
...
- What exact invariant or user/system promise makes **Idempotency** correct, and which component is authoritative for it?
...
- For **Idempotent API requests**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Two concurrent requests both execute, or the same key is reused with different parameters and returns the wrong result.
- For **Key scope**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for key scope is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Response replay**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for response replay is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Concurrent duplicates**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A probabilistic sleep-based test passes while the actual race remains reachable.
- For **Retry safety**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Permanent failures loop forever, retries synchronize into a storm, or a timed-out successful operation is duplicated.
...
- [ ] Map every entry point for **Idempotency**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Idempotent API requests:** Synchronize duplicate requests before first commit; test timeout-after-commit, key mismatch, expiry, and response replay.
- [ ] **Key generation:** Locate every implementation path for key generation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Request fingerprinting:** Locate every implementation path for request fingerprinting, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Storage:** Locate every implementation path for storage, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Failed first attempts:** Locate every implementation path for failed first attempts, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Job idempotency:** Locate every implementation path for job idempotency, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Webhook idempotency:** Simulate timeout, 2xx-after-timeout, 4xx, 429, 5xx, DNS/TLS failure, endpoint changes, and replay.
...
- [035. State Machines](035-state-machines.md) — layer: `primitives`; profile: `transactions`.
- [103. Distributed Locks](../cross-cutting/103-distributed-locks.md) — layer: `cross-cutting`; profile: `transactions`.
- [025. Concurrency Control](025-concurrency-control.md) — layer: `primitives`; profile: `transactions`.
...
- [121. Ordering Guarantees](121-ordering-guarantees.md) — layer: `primitives`; profile: `transactions`.
...
- <a id="s116"></a> **[S116] Stripe API Documentation: Idempotent Requests.** Stripe; 2026; Current. [https://docs.stripe.com/api/idempotent_requests](https://docs.stripe.com/api/idempotent_requests) — Tags: idempotency, api, provider-quirks.

---

**Paper metadata:** canonical subtopics: 14; layer: `primitives`; domain profile: `transactions`; verified through: `2026-08-17`.
