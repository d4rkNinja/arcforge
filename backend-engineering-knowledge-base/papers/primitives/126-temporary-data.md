...
**Temporary tokens**, **Upload sessions**.

### State and lifecycle

**Draft states**, **Expiration**, **Cleanup**.
...
This paper treats **Temporary Data** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Upload sessions** within Temporary Data: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for upload sessions is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for upload sessions, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST — engineering rule:** Define the exact semantics of **Locks** within Temporary Data: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for locks is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for locks, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST — engineering rule:** Define the exact semantics of **Cleanup** within Temporary Data: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.9. Replay prevention

- **MUST — engineering rule:** Define the exact semantics of **Replay prevention** within Temporary Data: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for replay prevention is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for replay prevention, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Storage choice

- **SHOULD — engineering rule:** Define the exact semantics of **Storage choice** within Temporary Data: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for storage choice is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for storage choice, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Temporary Data path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Temporary tokens`, `OTPs`, `Locks`. |
...
| Privacy / retention | Classify data produced or touched by Temporary Data; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Temporary Data** before choosing framework or provider mechanisms.
...
- **Temporary tokens:** A code can be replayed, used for another purpose, brute-forced, or accepted after a newer code was issued.
...
- What exact invariant or user/system promise makes **Temporary Data** correct, and which component is authoritative for it?
...
- [ ] Map every entry point for **Temporary Data**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Temporary tokens:** Test wrong purpose, concurrent redemption, resend behavior, clock boundaries, attempt exhaustion, and cleanup.
- [ ] **OTPs:** Test wrong purpose, concurrent redemption, resend behavior, clock boundaries, attempt exhaustion, and cleanup.
- [ ] **Verification codes:** Test wrong purpose, concurrent redemption, resend behavior, clock boundaries, attempt exhaustion, and cleanup.
- [ ] **Locks:** Locate every implementation path for locks, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Expiration:** Test exact boundary times with controlled clocks, delayed cleanup, clock skew, and mass expiry.
- [ ] **Replay prevention:** Locate every implementation path for replay prevention, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Storage choice:** Locate every implementation path for storage choice, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [007. Sessions](../systems/007-sessions.md) — layer: `systems`; profile: `authentication`.
