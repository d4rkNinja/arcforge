...
**Validation**, **Per-item errors**.

### Persistence and integrity

**Atomic vs non-atomic**, **Transaction limits**.

### Concurrency and distributed behavior

**Bulk create**, **Bulk update**, **Partial success**, **Concurrency**.
...
This paper treats **Bulk Operations** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Bulk create:** Define the exact semantics of **Bulk create** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Bulk delete:** Define the exact semantics of **Bulk delete** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Atomic vs non-atomic:** Define the exact semantics of **Atomic vs non-atomic** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Validation:** Define the exact semantics of **Validation** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Concurrency:** Define the exact semantics of **Concurrency** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Rate limiting:** Choose identity/scope, algorithm, burst capacity, window clock, atomic update, distributed consistency, response headers, and fail behavior according to the abuse and availability threat.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Bulk create** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for bulk create is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for bulk create, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Bulk update

- **SHOULD — engineering rule:** Define the exact semantics of **Bulk update** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for bulk update is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for bulk update, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Bulk delete

- **SHOULD — engineering rule:** Define the exact semantics of **Bulk delete** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for bulk delete is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for bulk delete, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Partial success

- **SHOULD — engineering rule:** Define the exact semantics of **Partial success** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for partial success is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for partial success, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Atomic vs non-atomic

- **SHOULD — engineering rule:** Define the exact semantics of **Atomic vs non-atomic** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for atomic vs non-atomic is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for atomic vs non-atomic, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Validation

- **MUST — engineering rule:** Define the exact semantics of **Validation** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Per-item errors** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for per-item errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for per-item errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Concurrency

- **SHOULD — engineering rule:** Define the exact semantics of **Concurrency** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.9. Transaction limits

- **MUST — engineering rule:** Define the exact semantics of **Transaction limits** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for transaction limits is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for transaction limits, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Bulk Operations path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Bulk create`, `Bulk delete`, `Atomic vs non-atomic`. |
...
| Privacy / retention | Classify data produced or touched by Bulk Operations; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Bulk Operations** before choosing framework or provider mechanisms.
...
- **MUST** — For **Bulk create**: Define the exact semantics of **Bulk create** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Bulk delete**: Define the exact semantics of **Bulk delete** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Atomic vs non-atomic**: Define the exact semantics of **Atomic vs non-atomic** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Validation**: Define the exact semantics of **Validation** within Bulk Operations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Bulk create:** Locate every implementation path for bulk create, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Bulk delete:** Locate every implementation path for bulk delete, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Atomic vs non-atomic:** Locate every implementation path for atomic vs non-atomic, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Rate limiting:** Test simultaneous requests across nodes, clock skew, key eviction, backend failure, IPv6/proxy identity, and Retry-After semantics.
...
- **Bulk create:** A framework or provider default for bulk create is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Bulk delete:** A framework or provider default for bulk delete is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Partial success:** A framework or provider default for partial success is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Atomic vs non-atomic:** A framework or provider default for atomic vs non-atomic is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Per-item errors:** A framework or provider default for per-item errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Transaction limits:** A framework or provider default for transaction limits is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Rate limiting:** Window-boundary bursts, racey counters, NAT collateral damage, or fail-open behavior allow abuse; fail-closed can create an outage.
...
- What exact invariant or user/system promise makes **Bulk Operations** correct, and which component is authoritative for it?
...
- For **Bulk create**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for bulk create is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Bulk delete**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for bulk delete is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Atomic vs non-atomic**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for atomic vs non-atomic is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Bulk Operations**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Bulk create:** Locate every implementation path for bulk create, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Bulk delete:** Locate every implementation path for bulk delete, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Partial success:** Locate every implementation path for partial success, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Atomic vs non-atomic:** Locate every implementation path for atomic vs non-atomic, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Per-item errors:** Locate every implementation path for per-item errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Transaction limits:** Locate every implementation path for transaction limits, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Rate limiting:** Test simultaneous requests across nodes, clock skew, key eviction, backend failure, IPv6/proxy identity, and Retry-After semantics.
...
- [128. Email Delivery Infrastructure](../systems/128-email-delivery-infrastructure.md) — layer: `systems`; profile: `async`.
