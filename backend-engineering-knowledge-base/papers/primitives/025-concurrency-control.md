...
**Optimistic locking**, **Compare-and-swap**, **Pessimistic locking**, **Row locks**, **Advisory locks**, **Conditional updates**, **Mutexes**, **Semaphores**, **Distributed locks**.

### Boundary of the paper

This paper treats **Concurrency Control** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **MUST — Optimistic locking:** Include the expected version/state in the atomic write predicate, increment/change it in the same statement, and return a distinct conflict outcome when zero rows match.
- **SHOULD — Compare-and-swap:** Include the expected version/state in the atomic write predicate, increment/change it in the same statement, and return a distinct conflict outcome when zero rows match.
- **MUST — Advisory locks:** Define lock key, scope, order, timeout, transaction ownership, and behavior on crash. Verify the lock protects every competing code path.
- **SHOULD — Conditional updates:** Include the expected version/state in the atomic write predicate, increment/change it in the same statement, and return a distinct conflict outcome when zero rows match.
- **MUST — Distributed locks:** Use a lease only with an authority that issues monotonically increasing fencing tokens and a protected resource that rejects stale tokens. Renewal and ownership checks must be explicit.
- **SHOULD — Lease expiration:** Use a lease only with an authority that issues monotonically increasing fencing tokens and a protected resource that rejects stale tokens. Renewal and ownership checks must be explicit.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Atomic updates** within Concurrency Control: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for atomic updates is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for atomic updates, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Conditional updates
...
- **SHOULD — engineering rule:** Define the exact semantics of **Mutexes** within Concurrency Control: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for mutexes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for mutexes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Semaphores

- **SHOULD — engineering rule:** Define the exact semantics of **Semaphores** within Concurrency Control: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for semaphores is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for semaphores, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Distributed locks
...
- **MUST** — For **Compare-and-swap**: Include the expected version/state in the atomic write predicate, increment/change it in the same statement, and return a distinct conflict outcome when zero rows match.
...
- **MUST** — For **Conditional updates**: Include the expected version/state in the atomic write predicate, increment/change it in the same statement, and return a distinct conflict outcome when zero rows match.
...
- [ ] **Optimistic locking:** Synchronize two updates from the same version and prove exactly one commits.
- [ ] **Compare-and-swap:** Synchronize two updates from the same version and prove exactly one commits.
- [ ] **Advisory locks:** Enumerate all writers and test contention, cancellation, connection loss, and pool reuse.
- [ ] **Conditional updates:** Synchronize two updates from the same version and prove exactly one commits.
- [ ] **Distributed locks:** Pause the first holder past expiry, acquire with a second holder, then prove the resource rejects the first holder's later operation.
- [ ] **Lease expiration:** Pause the first holder past expiry, acquire with a second holder, then prove the resource rejects the first holder's later operation.
...
- **Optimistic locking:** A stale client overwrites a newer update or the application interprets a conflict as not-found/success.
- **Compare-and-swap:** A stale client overwrites a newer update or the application interprets a conflict as not-found/success.
- **Row locks:** A forgotten code path bypasses the lock, lock order deadlocks, or a session-scoped advisory lock leaks through pooling.
- **Atomic updates:** A framework or provider default for atomic updates is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Mutexes:** A framework or provider default for mutexes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Distributed locks:** A paused or partitioned former owner resumes after expiry and performs a stale write despite a new owner holding the lock.
- **Lease expiration:** A paused or partitioned former owner resumes after expiry and performs a stale write despite a new owner holding the lock.
...
- For **Optimistic locking**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A stale client overwrites a newer update or the application interprets a conflict as not-found/success.
- For **Compare-and-swap**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A stale client overwrites a newer update or the application interprets a conflict as not-found/success.
- For **Advisory locks**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A forgotten code path bypasses the lock, lock order deadlocks, or a session-scoped advisory lock leaks through pooling.
- For **Conditional updates**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A stale client overwrites a newer update or the application interprets a conflict as not-found/success.
- For **Distributed locks**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A paused or partitioned former owner resumes after expiry and performs a stale write despite a new owner holding the lock.
...
- [ ] **Optimistic locking:** Synchronize two updates from the same version and prove exactly one commits.
- [ ] **Compare-and-swap:** Synchronize two updates from the same version and prove exactly one commits.
- [ ] **Row locks:** Enumerate all writers and test contention, cancellation, connection loss, and pool reuse.
- [ ] **Atomic updates:** Locate every implementation path for atomic updates, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Mutexes:** Locate every implementation path for mutexes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Distributed locks:** Pause the first holder past expiry, acquire with a second holder, then prove the resource rejects the first holder's later operation.
- [ ] **Lease expiration:** Pause the first holder past expiry, acquire with a second holder, then prove the resource rejects the first holder's later operation.
...
- [102. Distributed Consensus](../cross-cutting/102-distributed-consensus.md) — layer: `cross-cutting`; profile: `transactions`.
...
**Paper metadata:** canonical subtopics: 13; layer: `primitives`; domain profile: `transactions`; verified through: `2026-08-17`.
