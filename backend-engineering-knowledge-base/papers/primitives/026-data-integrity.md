...
**Data Integrity** exists to encode domain facts and invariants so valid states are easy to represent and invalid states are difficult to persist. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Invariants**, **Referential integrity**, **Atomic changes**, **Partial writes**, **Stale writes**, **Corruption detection**, **Integrity validation**, **Repair jobs**, **Reconciliation**, **Derived-data rebuilding**.

### Concurrency and distributed behavior

**Duplicate prevention**, **Orphan prevention**, **Consistency checks**.

### Boundary of the paper

This paper treats **Data Integrity** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Invariants:** Define the exact semantics of **Invariants** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Duplicate prevention:** Enforce race-sensitive uniqueness in the authoritative database with the correct scope, normalization, null, tenant, and deletion semantics; translate violations into deterministic domain conflicts.
- **SHOULD — Partial writes:** Define the exact semantics of **Partial writes** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Corruption detection:** Define the exact semantics of **Corruption detection** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Reconciliation:** Define the exact semantics of **Reconciliation** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Derived-data rebuilding:** Define the exact semantics of **Derived-data rebuilding** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Invariants** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for invariants is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for invariants, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Referential integrity

- **MUST — engineering rule:** Define ownership and delete/update behavior explicitly. Use database enforcement where relationships share a transactional store; otherwise maintain tombstones, events, and reconciliation.
...
- **SHOULD — engineering rule:** Enforce race-sensitive uniqueness in the authoritative database with the correct scope, normalization, null, tenant, and deletion semantics; translate violations into deterministic domain conflicts.
- **Production failure mode:** Two concurrent check-then-insert paths create duplicates, or soft-deleted/null values behave differently than application assumptions.
- **Existing-codebase evidence:** Run concurrent inserts and inspect the exact index/constraint definition, collation, partial predicate, and error mapping.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Atomic changes** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for atomic changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for atomic changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Partial writes

- **SHOULD — engineering rule:** Define the exact semantics of **Partial writes** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for partial writes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for partial writes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Stale writes

- **SHOULD — engineering rule:** Define the exact semantics of **Stale writes** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for stale writes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for stale writes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Corruption detection

- **SHOULD — engineering rule:** Define the exact semantics of **Corruption detection** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for corruption detection is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for corruption detection, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Integrity validation

- **MUST — engineering rule:** Define the exact semantics of **Integrity validation** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for integrity validation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for integrity validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Repair jobs

- **SHOULD — engineering rule:** Define the exact semantics of **Repair jobs** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for repair jobs is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for repair jobs, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Reconciliation

- **SHOULD — engineering rule:** Define the exact semantics of **Reconciliation** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for reconciliation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for reconciliation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.12. Consistency checks

- **SHOULD — engineering rule:** Define the exact semantics of **Consistency checks** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for consistency checks is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for consistency checks, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. Derived-data rebuilding

- **SHOULD — engineering rule:** Define the exact semantics of **Derived-data rebuilding** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for derived-data rebuilding is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for derived-data rebuilding, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Data Integrity path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Invariants`, `Orphan prevention`, `Stale writes`. |
...
| Privacy / retention | Classify data produced or touched by Data Integrity; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Data Integrity** before choosing framework or provider mechanisms.
...
- **MUST** — For **Invariants**: Define the exact semantics of **Invariants** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Duplicate prevention**: Enforce race-sensitive uniqueness in the authoritative database with the correct scope, normalization, null, tenant, and deletion semantics; translate violations into deterministic domain conflicts.
- **MUST** — For **Partial writes**: Define the exact semantics of **Partial writes** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Corruption detection**: Define the exact semantics of **Corruption detection** within Data Integrity: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Invariants:** Locate every implementation path for invariants, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Duplicate prevention:** Run concurrent inserts and inspect the exact index/constraint definition, collation, partial predicate, and error mapping.
- [ ] **Partial writes:** Locate every implementation path for partial writes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Corruption detection:** Locate every implementation path for corruption detection, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Reconciliation:** Locate every implementation path for reconciliation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Derived-data rebuilding:** Locate every implementation path for derived-data rebuilding, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Invariants:** A framework or provider default for invariants is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Duplicate prevention:** Two concurrent check-then-insert paths create duplicates, or soft-deleted/null values behave differently than application assumptions.
- **Atomic changes:** A framework or provider default for atomic changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Stale writes:** A framework or provider default for stale writes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Integrity validation:** A framework or provider default for integrity validation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Reconciliation:** A framework or provider default for reconciliation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Derived-data rebuilding:** A framework or provider default for derived-data rebuilding is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Data Integrity** correct, and which component is authoritative for it?
...
- For **Invariants**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for invariants is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Duplicate prevention**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Two concurrent check-then-insert paths create duplicates, or soft-deleted/null values behave differently than application assumptions.
- For **Partial writes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for partial writes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Corruption detection**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for corruption detection is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Reconciliation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for reconciliation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Data Integrity**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Invariants:** Locate every implementation path for invariants, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Duplicate prevention:** Run concurrent inserts and inspect the exact index/constraint definition, collation, partial predicate, and error mapping.
- [ ] **Atomic changes:** Locate every implementation path for atomic changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Stale writes:** Locate every implementation path for stale writes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Integrity validation:** Locate every implementation path for integrity validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Reconciliation:** Locate every implementation path for reconciliation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Derived-data rebuilding:** Locate every implementation path for derived-data rebuilding, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [124. Data Reconciliation](124-data-reconciliation.md) — layer: `primitives`; profile: `data_ops`.
- [024. Concurrency Anomalies](024-concurrency-anomalies.md) — layer: `primitives`; profile: `transactions`.
- [023. Database Transactions](023-database-transactions.md) — layer: `primitives`; profile: `transactions`.
