...
**Identifiers** exists to encode domain facts and invariants so valid states are easy to represent and invalid states are difficult to persist. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Auto-increment IDs**, **UUIDv4**, **UUIDv7**, **ULID**, **Snowflake IDs**, **Database IDs**, **Public IDs**, **Private IDs**, **Sequential IDs**, **Human-readable IDs**, **Composite IDs**, **Collision handling**, **ID exposure**.
...
This paper treats **Identifiers** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
|---|---|---|
| Random vs time-ordered identifiers | Random UUIDs avoid ordering disclosure but fragment indexes; UUIDv7 improves locality while exposing rough creation time. | Choose according to index/write behavior, disclosure risk, and interoperability. |
...
### 7.1. Auto-increment IDs

- **SHOULD — engineering rule:** Define the exact semantics of **Auto-increment IDs** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for auto-increment ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for auto-increment ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. UUIDv4

- **SHOULD — engineering rule:** Define the exact semantics of **UUIDv4** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for uuidv4 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for uuidv4, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. UUIDv7

- **SHOULD — engineering rule:** Define the exact semantics of **UUIDv7** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for uuidv7 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for uuidv7, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. ULID

- **SHOULD — engineering rule:** Define the exact semantics of **ULID** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for ulid is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for ulid, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Snowflake IDs

- **SHOULD — engineering rule:** Define the exact semantics of **Snowflake IDs** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for snowflake ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for snowflake ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Database IDs

- **SHOULD — engineering rule:** Define the exact semantics of **Database IDs** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for database ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for database ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Public IDs

- **SHOULD — engineering rule:** Define the exact semantics of **Public IDs** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for public ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for public ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Private IDs

- **SHOULD — engineering rule:** Define the exact semantics of **Private IDs** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for private ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for private ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Sequential IDs

- **SHOULD — engineering rule:** Define the exact semantics of **Sequential IDs** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for sequential ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for sequential ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Human-readable IDs

- **SHOULD — engineering rule:** Define the exact semantics of **Human-readable IDs** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for human-readable ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for human-readable ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Composite IDs

- **SHOULD — engineering rule:** Define the exact semantics of **Composite IDs** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for composite ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for composite ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.12. Collision handling

- **SHOULD — engineering rule:** Define the exact semantics of **Collision handling** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for collision handling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for collision handling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. Enumeration prevention
...
- **SHOULD — engineering rule:** Define the exact semantics of **ID exposure** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for id exposure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for id exposure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.15. ID migration

- **SHOULD — engineering rule:** Define the exact semantics of **ID migration** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for id migration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for id migration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Identifiers path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Auto-increment IDs`, `Snowflake IDs`, `Private IDs`. |
...
| Privacy / retention | Classify data produced or touched by Identifiers; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Identifiers** before choosing framework or provider mechanisms.
...
- **MUST** — For **Auto-increment IDs**: Define the exact semantics of **Auto-increment IDs** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **ULID**: Define the exact semantics of **ULID** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Public IDs**: Define the exact semantics of **Public IDs** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Sequential IDs**: Define the exact semantics of **Sequential IDs** within Identifiers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **Random vs time-ordered identifiers** according to the stated trade-off: Choose according to index/write behavior, disclosure risk, and interoperability.
...
- [ ] **Auto-increment IDs:** Locate every implementation path for auto-increment ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **ULID:** Locate every implementation path for ulid, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Public IDs:** Locate every implementation path for public ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Sequential IDs:** Locate every implementation path for sequential ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Collision handling:** Locate every implementation path for collision handling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **ID migration:** Locate every implementation path for id migration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Auto-increment IDs:** A framework or provider default for auto-increment ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **UUIDv7:** A framework or provider default for uuidv7 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Database IDs:** A framework or provider default for database ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Private IDs:** A framework or provider default for private ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Human-readable IDs:** A framework or provider default for human-readable ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Enumeration prevention:** Attackers distribute attempts across keys, weaponize lockouts, or infer valid accounts from timing and error differences.
- **ID migration:** A framework or provider default for id migration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Identifiers** correct, and which component is authoritative for it?
...
- For **Auto-increment IDs**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for auto-increment ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **ULID**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for ulid is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Public IDs**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for public ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Sequential IDs**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for sequential ids is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Collision handling**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for collision handling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Identifiers**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Auto-increment IDs:** Locate every implementation path for auto-increment ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **UUIDv7:** Locate every implementation path for uuidv7, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Database IDs:** Locate every implementation path for database ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Private IDs:** Locate every implementation path for private ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Human-readable IDs:** Locate every implementation path for human-readable ids, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Enumeration prevention:** Replay distributed attack patterns and compare status, body, timing, notification, and recovery behavior for existing/non-existing targets.
- [ ] **ID migration:** Locate every implementation path for id migration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [127. Randomness & Token Generation](127-randomness-and-token-generation.md) — layer: `primitives`; profile: `security`.
...
- [032. Soft Delete / Hard Delete](032-soft-delete-hard-delete.md) — layer: `primitives`; profile: `data_model`.
...
**Paper metadata:** canonical subtopics: 15; layer: `primitives`; domain profile: `data_model`; verified through: `2026-08-17`.
