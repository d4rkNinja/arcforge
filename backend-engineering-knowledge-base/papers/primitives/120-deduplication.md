...
**Duplicate rows**, **Duplicate messages**, **Duplicate jobs**, **Duplicate webhooks**, **Natural keys**, **Fingerprints**, **Time-window dedupe**.

### Boundary of the paper

This paper treats **Deduplication** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Duplicate requests:** Define the exact semantics of **Duplicate requests** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Duplicate messages:** Define the exact semantics of **Duplicate messages** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Duplicate webhooks:** Model each delivery attempt with event ID, subscription version, endpoint, timeout, response class, next attempt, and terminal state. Bound retries and disable persistently failing or unsafe endpoints.
- **SHOULD — Natural keys:** Define the exact semantics of **Natural keys** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Fingerprints:** Define the exact semantics of **Fingerprints** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Cleanup:** Define the exact semantics of **Cleanup** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Duplicate requests** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Duplicate rows** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for duplicate rows is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for duplicate rows, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Duplicate messages

- **SHOULD — engineering rule:** Define the exact semantics of **Duplicate messages** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for duplicate messages is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for duplicate messages, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Duplicate jobs

- **SHOULD — engineering rule:** Define the exact semantics of **Duplicate jobs** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for duplicate jobs is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for duplicate jobs, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Duplicate webhooks

- **SHOULD — engineering rule:** Model each delivery attempt with event ID, subscription version, endpoint, timeout, response class, next attempt, and terminal state. Bound retries and disable persistently failing or unsafe endpoints.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Natural keys** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for natural keys is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for natural keys, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Unique constraints

- **MUST — engineering rule:** Enforce race-sensitive uniqueness in the authoritative database with the correct scope, normalization, null, tenant, and deletion semantics; translate violations into deterministic domain conflicts.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Fingerprints** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for fingerprints is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for fingerprints, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Time-window dedupe

- **SHOULD — engineering rule:** Define the exact semantics of **Time-window dedupe** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for time-window dedupe is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for time-window dedupe, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Cleanup

- **MUST — engineering rule:** Define the exact semantics of **Cleanup** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Deduplication path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Duplicate requests`, `Duplicate messages`, `Duplicate webhooks`. |
...
| Privacy / retention | Classify data produced or touched by Deduplication; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Deduplication** before choosing framework or provider mechanisms.
...
- **MUST** — For **Duplicate requests**: Define the exact semantics of **Duplicate requests** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Duplicate messages**: Define the exact semantics of **Duplicate messages** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Duplicate webhooks**: Model each delivery attempt with event ID, subscription version, endpoint, timeout, response class, next attempt, and terminal state. Bound retries and disable persistently failing or unsafe endpoints.
- **MUST** — For **Natural keys**: Define the exact semantics of **Natural keys** within Deduplication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Duplicate requests:** Locate every implementation path for duplicate requests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Duplicate messages:** Locate every implementation path for duplicate messages, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Duplicate webhooks:** Simulate timeout, 2xx-after-timeout, 4xx, 429, 5xx, DNS/TLS failure, endpoint changes, and replay.
- [ ] **Natural keys:** Locate every implementation path for natural keys, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Fingerprints:** Locate every implementation path for fingerprints, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Duplicate requests:** A framework or provider default for duplicate requests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Duplicate messages:** A framework or provider default for duplicate messages is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Duplicate jobs:** A framework or provider default for duplicate jobs is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Duplicate webhooks:** Slow consumers exhaust workers, duplicate/out-of-order deliveries corrupt state, or one endpoint causes unbounded retry cost.
- **Unique constraints:** Two concurrent check-then-insert paths create duplicates, or soft-deleted/null values behave differently than application assumptions.
- **Time-window dedupe:** A framework or provider default for time-window dedupe is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Cleanup:** A framework or provider default for cleanup is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Deduplication** correct, and which component is authoritative for it?
...
- For **Duplicate requests**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for duplicate requests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Duplicate messages**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for duplicate messages is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Duplicate webhooks**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Slow consumers exhaust workers, duplicate/out-of-order deliveries corrupt state, or one endpoint causes unbounded retry cost.
- For **Natural keys**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for natural keys is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Fingerprints**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for fingerprints is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Deduplication**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Duplicate requests:** Locate every implementation path for duplicate requests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Duplicate messages:** Locate every implementation path for duplicate messages, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Duplicate jobs:** Locate every implementation path for duplicate jobs, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Duplicate webhooks:** Simulate timeout, 2xx-after-timeout, 4xx, 429, 5xx, DNS/TLS failure, endpoint changes, and replay.
- [ ] **Unique constraints:** Run concurrent inserts and inspect the exact index/constraint definition, collation, partial predicate, and error mapping.
- [ ] **Time-window dedupe:** Locate every implementation path for time-window dedupe, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Cleanup:** Locate every implementation path for cleanup, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [119. Bulk Operations](119-bulk-operations.md) — layer: `primitives`; profile: `async`.
- [118. Batch Processing](118-batch-processing.md) — layer: `primitives`; profile: `async`.
...
- [129. Notification Infrastructure](../systems/129-notification-infrastructure.md) — layer: `systems`; profile: `async`.
...
**Paper metadata:** canonical subtopics: 10; layer: `primitives`; domain profile: `async`; verified through: `2026-08-17`.
