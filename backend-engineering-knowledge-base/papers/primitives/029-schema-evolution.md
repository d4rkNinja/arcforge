...
**Field renaming**, **Type changes**, **Nullable → required**, **Expand-and-contract**.

### Testing and evolution

**Additive changes**, **Destructive changes**, **Column changes**, **Default changes**, **Dual-read**, **Dual-write**, **Backward compatibility**, **Old-client compatibility**.

### Boundary of the paper

This paper treats **Schema Evolution** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Additive changes:** Define the exact semantics of **Additive changes** within Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Column changes:** Define the exact semantics of **Column changes** within Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Type changes:** Use expand-and-contract: add the new representation, deploy compatible readers/writers, backfill and verify, switch reads, stop old writes, then remove after the compatibility horizon.
- **SHOULD — Expand-and-contract:** Define the exact semantics of **Expand-and-contract** within Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Dual-write:** Assign a canonical truth, stable operation ID/version, ordering and conflict rules, compare outcomes, reconcile divergence, and define a finite exit criterion.
- **SHOULD — Old-client compatibility:** Define the exact semantics of **Old-client compatibility** within Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Additive changes** within Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for additive changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for additive changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Destructive changes
...
- **SHOULD — engineering rule:** Define the exact semantics of **Column changes** within Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for column changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for column changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Default changes** within Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for default changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for default changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Nullable → required

- **SHOULD — engineering rule:** Use expand-and-contract: add the new representation, deploy compatible readers/writers, backfill and verify, switch reads, stop old writes, then remove after the compatibility horizon.
- **Production failure mode:** Mixed versions lose data, defaults change meaning, or rollback becomes impossible.
- **Existing-codebase evidence:** Record per-stage invariants and gates; inspect telemetry proving old reads/writes have ceased before contraction.

### 7.8. Expand-and-contract

- **SHOULD — engineering rule:** Define the exact semantics of **Expand-and-contract** within Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for expand-and-contract is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for expand-and-contract, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.12. Old-client compatibility

- **SHOULD — engineering rule:** Define the exact semantics of **Old-client compatibility** within Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for old-client compatibility is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for old-client compatibility, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST** — For **Additive changes**: Define the exact semantics of **Additive changes** within Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Column changes**: Define the exact semantics of **Column changes** within Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MUST** — For **Expand-and-contract**: Define the exact semantics of **Expand-and-contract** within Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Additive changes:** Locate every implementation path for additive changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Column changes:** Locate every implementation path for column changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Type changes:** Record per-stage invariants and gates; inspect telemetry proving old reads/writes have ceased before contraction.
- [ ] **Expand-and-contract:** Locate every implementation path for expand-and-contract, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Dual-write:** Inject failure on each side and verify repair, metrics, replay, and rollback without duplicate side effects.
- [ ] **Old-client compatibility:** Locate every implementation path for old-client compatibility, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Additive changes:** A framework or provider default for additive changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Column changes:** A framework or provider default for column changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Type changes:** Mixed versions lose data, defaults change meaning, or rollback becomes impossible.
- **Nullable → required:** Mixed versions lose data, defaults change meaning, or rollback becomes impossible.
- **Expand-and-contract:** A framework or provider default for expand-and-contract is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Dual-write:** Two stores diverge after partial failure and operators cannot tell which copy is correct.
- **Old-client compatibility:** A framework or provider default for old-client compatibility is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Additive changes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for additive changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Column changes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for column changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Type changes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Mixed versions lose data, defaults change meaning, or rollback becomes impossible.
- For **Expand-and-contract**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for expand-and-contract is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Dual-write**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Two stores diverge after partial failure and operators cannot tell which copy is correct.
...
- [ ] **Column changes:** Locate every implementation path for column changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Type changes:** Record per-stage invariants and gates; inspect telemetry proving old reads/writes have ceased before contraction.
- [ ] **Nullable → required:** Record per-stage invariants and gates; inspect telemetry proving old reads/writes have ceased before contraction.
- [ ] **Expand-and-contract:** Locate every implementation path for expand-and-contract, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Dual-write:** Inject failure on each side and verify repair, metrics, replay, and rollback without duplicate side effects.
- [ ] **Old-client compatibility:** Locate every implementation path for old-client compatibility, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
