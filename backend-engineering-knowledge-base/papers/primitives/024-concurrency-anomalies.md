...
**Duplicate creation**, **Concurrent delete/update**, **Concurrent state transitions**.
...
**Lost updates**, **Dirty reads**, **Phantom reads**, **Write skew**, **Double writes**, **Check-then-act races**, **Read-modify-write races**.

### Boundary of the paper

This paper treats **Concurrency Anomalies** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Lost updates:** Replace separate read/check/write steps with a constraint, atomic conditional statement, lock, or serializable transaction that makes the decision and write indivisible.
- **SHOULD — Non-repeatable reads:** Select isolation by anomaly analysis, not name familiarity. Account for database-specific semantics and retry complete transactions on serialization failure.
- **SHOULD — Write skew:** Define the exact semantics of **Write skew** within Concurrency Anomalies: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Duplicate creation:** Replace separate read/check/write steps with a constraint, atomic conditional statement, lock, or serializable transaction that makes the decision and write indivisible.
- **SHOULD — Read-modify-write races:** Replace separate read/check/write steps with a constraint, atomic conditional statement, lock, or serializable transaction that makes the decision and write indivisible.
- **MUST — Concurrent state transitions:** Model transitions as commands with expected current state/version, guard evaluation, atomic state/history update, and idempotent side-effect triggering.
...
- **SHOULD — engineering rule:** Replace separate read/check/write steps with a constraint, atomic conditional statement, lock, or serializable transaction that makes the decision and write indivisible.
- **Production failure mode:** Concurrent requests both observe the same precondition and commit incompatible results.
- **Existing-codebase evidence:** Use a barrier to align competing requests immediately after the read and assert the invariant after both finish.

### 7.2. Dirty reads

- **SHOULD — engineering rule:** Define the exact semantics of **Dirty reads** within Concurrency Anomalies: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for dirty reads is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for dirty reads, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Non-repeatable reads
...
- **SHOULD — engineering rule:** Define the exact semantics of **Phantom reads** within Concurrency Anomalies: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for phantom reads is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for phantom reads, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Write skew

- **SHOULD — engineering rule:** Define the exact semantics of **Write skew** within Concurrency Anomalies: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for write skew is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for write skew, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Double writes

- **SHOULD — engineering rule:** Define the exact semantics of **Double writes** within Concurrency Anomalies: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for double writes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for double writes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Duplicate creation

- **SHOULD — engineering rule:** Replace separate read/check/write steps with a constraint, atomic conditional statement, lock, or serializable transaction that makes the decision and write indivisible.
- **Production failure mode:** Concurrent requests both observe the same precondition and commit incompatible results.
- **Existing-codebase evidence:** Use a barrier to align competing requests immediately after the read and assert the invariant after both finish.

### 7.8. Check-then-act races

- **SHOULD — engineering rule:** Replace separate read/check/write steps with a constraint, atomic conditional statement, lock, or serializable transaction that makes the decision and write indivisible.
- **Production failure mode:** Concurrent requests both observe the same precondition and commit incompatible results.
- **Existing-codebase evidence:** Use a barrier to align competing requests immediately after the read and assert the invariant after both finish.

### 7.9. Read-modify-write races

- **SHOULD — engineering rule:** Replace separate read/check/write steps with a constraint, atomic conditional statement, lock, or serializable transaction that makes the decision and write indivisible.
- **Production failure mode:** Concurrent requests both observe the same precondition and commit incompatible results.
- **Existing-codebase evidence:** Use a barrier to align competing requests immediately after the read and assert the invariant after both finish.

### 7.10. Concurrent delete/update
...
### 7.11. Concurrent state transitions
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Concurrency Anomalies path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Lost updates`, `Non-repeatable reads`, `Double writes`. |
...
| Privacy / retention | Classify data produced or touched by Concurrency Anomalies; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Concurrency Anomalies** before choosing framework or provider mechanisms.
...
- **MUST** — For **Lost updates**: Replace separate read/check/write steps with a constraint, atomic conditional statement, lock, or serializable transaction that makes the decision and write indivisible.
- **MUST** — For **Non-repeatable reads**: Select isolation by anomaly analysis, not name familiarity. Account for database-specific semantics and retry complete transactions on serialization failure.
- **MUST** — For **Write skew**: Define the exact semantics of **Write skew** within Concurrency Anomalies: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Duplicate creation**: Replace separate read/check/write steps with a constraint, atomic conditional statement, lock, or serializable transaction that makes the decision and write indivisible.
...
- [ ] **Lost updates:** Use a barrier to align competing requests immediately after the read and assert the invariant after both finish.
- [ ] **Non-repeatable reads:** Construct concurrent schedules that would violate the invariant and verify the chosen isolation/locking rejects one participant.
- [ ] **Write skew:** Locate every implementation path for write skew, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Duplicate creation:** Use a barrier to align competing requests immediately after the read and assert the invariant after both finish.
- [ ] **Read-modify-write races:** Use a barrier to align competing requests immediately after the read and assert the invariant after both finish.
- [ ] **Concurrent state transitions:** Generate the state-transition matrix and test every allowed/forbidden pair plus simultaneous conflicting commands.
...
- **Lost updates:** Concurrent requests both observe the same precondition and commit incompatible results.
- **Non-repeatable reads:** Lost updates, write skew, or phantoms violate invariants despite all statements being inside a transaction.
- **Phantom reads:** A framework or provider default for phantom reads is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Double writes:** A framework or provider default for double writes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Check-then-act races:** Concurrent requests both observe the same precondition and commit incompatible results.
- **Read-modify-write races:** Concurrent requests both observe the same precondition and commit incompatible results.
- **Concurrent state transitions:** Generic updates skip guards, concurrent transitions both succeed, or terminal records are silently reopened.
...
- What exact invariant or user/system promise makes **Concurrency Anomalies** correct, and which component is authoritative for it?
...
- For **Lost updates**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Concurrent requests both observe the same precondition and commit incompatible results.
- For **Non-repeatable reads**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Lost updates, write skew, or phantoms violate invariants despite all statements being inside a transaction.
- For **Write skew**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for write skew is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Duplicate creation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Concurrent requests both observe the same precondition and commit incompatible results.
- For **Read-modify-write races**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Concurrent requests both observe the same precondition and commit incompatible results.
...
- [ ] Map every entry point for **Concurrency Anomalies**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Lost updates:** Use a barrier to align competing requests immediately after the read and assert the invariant after both finish.
- [ ] **Non-repeatable reads:** Construct concurrent schedules that would violate the invariant and verify the chosen isolation/locking rejects one participant.
- [ ] **Phantom reads:** Locate every implementation path for phantom reads, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Double writes:** Locate every implementation path for double writes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Check-then-act races:** Use a barrier to align competing requests immediately after the read and assert the invariant after both finish.
- [ ] **Read-modify-write races:** Use a barrier to align competing requests immediately after the read and assert the invariant after both finish.
- [ ] **Concurrent state transitions:** Generate the state-transition matrix and test every allowed/forbidden pair plus simultaneous conflicting commands.
...
- [092. Concurrency Testing](../cross-cutting/092-concurrency-testing.md) — layer: `cross-cutting`; profile: `testing`.
...
- [100. Replication](../cross-cutting/100-replication.md) — layer: `cross-cutting`; profile: `transactions`.
...
**Paper metadata:** canonical subtopics: 11; layer: `primitives`; domain profile: `transactions`; verified through: `2026-08-17`.
