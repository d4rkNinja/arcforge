...
**Database Constraints** exists to encode domain facts and invariants so valid states are easy to represent and invalid states are difficult to persist. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Primary keys**, **Foreign keys**, **Unique constraints**, **Composite uniqueness**, **Check constraints**, **Not-null constraints**, **Referential integrity**, **Cascade behavior**, **Restrict behavior**, **Default values**, **Data invariants**, **Application validation vs DB constraints**, **Constraint race conditions**.

### Boundary of the paper

This paper treats **Database Constraints** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Primary keys:** Define the exact semantics of **Primary keys** within Database Constraints: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Unique constraints:** Enforce race-sensitive uniqueness in the authoritative database with the correct scope, normalization, null, tenant, and deletion semantics; translate violations into deterministic domain conflicts.
- **MUST — Not-null constraints:** Encode stable local invariants in the database and mirror them in application validation for usable errors. Roll out stricter constraints only after measuring and repairing existing data.
- **SHOULD — Cascade behavior:** Choose cascade, restrict, nullify, soft-delete propagation, or asynchronous cleanup from ownership and audit requirements; never accept ORM defaults without review.
- **SHOULD — Data invariants:** Encode stable local invariants in the database and mirror them in application validation for usable errors. Roll out stricter constraints only after measuring and repairing existing data.
- **MUST — Constraint race conditions:** Use barriers/hooks to force the critical interleaving, repeat across real database isolation, and assert the invariant rather than timing or one response.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Primary keys** within Database Constraints: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for primary keys is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for primary keys, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST — engineering rule:** Encode stable local invariants in the database and mirror them in application validation for usable errors. Roll out stricter constraints only after measuring and repairing existing data.
- **Production failure mode:** A second writer or migration bypasses application validation and persists an impossible state.
- **Existing-codebase evidence:** Compare domain rules to schema constraints and run constraint validation against the full production distribution before enforcement.

### 7.6. Not-null constraints
...
### 7.8. Cascade behavior

- **SHOULD — engineering rule:** Choose cascade, restrict, nullify, soft-delete propagation, or asynchronous cleanup from ownership and audit requirements; never accept ORM defaults without review.
- **Production failure mode:** A parent deletion unexpectedly erases shared data or leaves a massive locking cascade and orphaned external resources.
- **Existing-codebase evidence:** Model deletion fanout, transaction size, lock impact, restoration, and audit expectations with production-scale fixtures.

### 7.9. Restrict behavior
...
- **SHOULD — engineering rule:** Define the exact semantics of **Default values** within Database Constraints: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for default values is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for default values, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Data invariants

- **SHOULD — engineering rule:** Encode stable local invariants in the database and mirror them in application validation for usable errors. Roll out stricter constraints only after measuring and repairing existing data.
- **Production failure mode:** A second writer or migration bypasses application validation and persists an impossible state.
- **Existing-codebase evidence:** Compare domain rules to schema constraints and run constraint validation against the full production distribution before enforcement.

### 7.12. Application validation vs DB constraints

- **MUST — engineering rule:** Define the exact semantics of **Application validation vs DB constraints** within Database Constraints: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for application validation vs db constraints is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for application validation vs db constraints, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. Constraint race conditions

- **MUST — engineering rule:** Use barriers/hooks to force the critical interleaving, repeat across real database isolation, and assert the invariant rather than timing or one response.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Database Constraints path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Primary keys`, `Composite uniqueness`, `Referential integrity`. |
...
| Privacy / retention | Classify data produced or touched by Database Constraints; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Database Constraints** before choosing framework or provider mechanisms.
...
- **MUST** — For **Primary keys**: Define the exact semantics of **Primary keys** within Database Constraints: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Unique constraints**: Enforce race-sensitive uniqueness in the authoritative database with the correct scope, normalization, null, tenant, and deletion semantics; translate violations into deterministic domain conflicts.
- **MUST** — For **Not-null constraints**: Encode stable local invariants in the database and mirror them in application validation for usable errors. Roll out stricter constraints only after measuring and repairing existing data.
- **MUST** — For **Cascade behavior**: Choose cascade, restrict, nullify, soft-delete propagation, or asynchronous cleanup from ownership and audit requirements; never accept ORM defaults without review.
...
- [ ] **Primary keys:** Locate every implementation path for primary keys, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Not-null constraints:** Compare domain rules to schema constraints and run constraint validation against the full production distribution before enforcement.
- [ ] **Cascade behavior:** Model deletion fanout, transaction size, lock impact, restoration, and audit expectations with production-scale fixtures.
- [ ] **Data invariants:** Compare domain rules to schema constraints and run constraint validation against the full production distribution before enforcement.
- [ ] **Constraint race conditions:** Instrument the exact read/check/write or lease boundary and run deterministic competing operations.
...
- **Primary keys:** A framework or provider default for primary keys is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Check constraints:** A second writer or migration bypasses application validation and persists an impossible state.
- **Referential integrity:** Partial failure or deletion leaves references that crash reads, leak data, or prevent retention enforcement.
- **Restrict behavior:** A parent deletion unexpectedly erases shared data or leaves a massive locking cascade and orphaned external resources.
- **Data invariants:** A second writer or migration bypasses application validation and persists an impossible state.
- **Constraint race conditions:** A probabilistic sleep-based test passes while the actual race remains reachable.
...
- What exact invariant or user/system promise makes **Database Constraints** correct, and which component is authoritative for it?
...
- For **Primary keys**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for primary keys is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Unique constraints**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Two concurrent check-then-insert paths create duplicates, or soft-deleted/null values behave differently than application assumptions.
- For **Not-null constraints**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A second writer or migration bypasses application validation and persists an impossible state.
- For **Cascade behavior**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A parent deletion unexpectedly erases shared data or leaves a massive locking cascade and orphaned external resources.
- For **Data invariants**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A second writer or migration bypasses application validation and persists an impossible state.
...
- [ ] Map every entry point for **Database Constraints**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Primary keys:** Locate every implementation path for primary keys, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Unique constraints:** Run concurrent inserts and inspect the exact index/constraint definition, collation, partial predicate, and error mapping.
- [ ] **Check constraints:** Compare domain rules to schema constraints and run constraint validation against the full production distribution before enforcement.
- [ ] **Referential integrity:** Delete and restore principals/resources under concurrent writes, then run integrity queries for every relationship.
- [ ] **Restrict behavior:** Model deletion fanout, transaction size, lock impact, restoration, and audit expectations with production-scale fixtures.
- [ ] **Data invariants:** Compare domain rules to schema constraints and run constraint validation against the full production distribution before enforcement.
- [ ] **Constraint race conditions:** Instrument the exact read/check/write or lease boundary and run deterministic competing operations.
