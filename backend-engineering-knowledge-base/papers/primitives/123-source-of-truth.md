...
**Source of Truth** exists to encode domain facts and invariants so valid states are easy to represent and invalid states are difficult to persist. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Canonical data ownership**, **Ownership boundaries**.

### Persistence and integrity

**Primary source**, **Derived copies**, **Search indexes**, **Caches**, **Conflict resolution**.
...
This paper treats **Source of Truth** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **MUST — Canonical data ownership:** Assign one owner for invariants and writes, expose a narrow versioned interface, enforce dependency direction mechanically, and communicate cross-boundary facts through explicit calls/events.
- **SHOULD — Replicas:** Define the exact semantics of **Replicas** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Derived copies:** Define the exact semantics of **Derived copies** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Caches:** Define the exact semantics of **Caches** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Synchronization:** Define the exact semantics of **Synchronization** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Ownership boundaries:** Define the exact semantics of **Ownership boundaries** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Primary source** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for primary source is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for primary source, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Replicas

- **SHOULD — engineering rule:** Define the exact semantics of **Replicas** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for replicas is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for replicas, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Derived copies

- **SHOULD — engineering rule:** Define the exact semantics of **Derived copies** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for derived copies is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for derived copies, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Caches** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for caches is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for caches, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Synchronization

- **SHOULD — engineering rule:** Define the exact semantics of **Synchronization** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for synchronization is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for synchronization, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Conflict resolution

- **SHOULD — engineering rule:** Define the exact semantics of **Conflict resolution** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.9. Ownership boundaries

- **MUST — engineering rule:** Define the exact semantics of **Ownership boundaries** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for ownership boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for ownership boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Source of Truth path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Canonical data ownership`, `Replicas`, `Search indexes`. |
...
| Privacy / retention | Classify data produced or touched by Source of Truth; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Source of Truth** before choosing framework or provider mechanisms.
...
- **MUST** — For **Canonical data ownership**: Assign one owner for invariants and writes, expose a narrow versioned interface, enforce dependency direction mechanically, and communicate cross-boundary facts through explicit calls/events.
- **MUST** — For **Replicas**: Define the exact semantics of **Replicas** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Derived copies**: Define the exact semantics of **Derived copies** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Caches**: Define the exact semantics of **Caches** within Source of Truth: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Canonical data ownership:** Build an import/dependency graph, map table writes, and flag cross-boundary direct access.
- [ ] **Replicas:** Locate every implementation path for replicas, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Derived copies:** Locate every implementation path for derived copies, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Caches:** Locate every implementation path for caches, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Synchronization:** Locate every implementation path for synchronization, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Ownership boundaries:** Locate every implementation path for ownership boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Canonical data ownership:** Modules share tables/models/utilities until changes require lockstep deployment and no team can reason locally.
- **Primary source:** A framework or provider default for primary source is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Derived copies:** A framework or provider default for derived copies is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Search indexes:** An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
- **Caches:** A framework or provider default for caches is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Ownership boundaries:** A framework or provider default for ownership boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Source of Truth** correct, and which component is authoritative for it?
...
- For **Canonical data ownership**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Modules share tables/models/utilities until changes require lockstep deployment and no team can reason locally.
- For **Replicas**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for replicas is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Derived copies**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for derived copies is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Caches**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for caches is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Synchronization**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for synchronization is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Source of Truth**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Canonical data ownership:** Build an import/dependency graph, map table writes, and flag cross-boundary direct access.
- [ ] **Primary source:** Locate every implementation path for primary source, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Derived copies:** Locate every implementation path for derived copies, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Search indexes:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
- [ ] **Caches:** Locate every implementation path for caches, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
**Paper metadata:** canonical subtopics: 9; layer: `primitives`; domain profile: `data_model`; verified through: `2026-08-17`.
