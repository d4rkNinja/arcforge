---
paper_number: 70
title: "API / Event Schema Evolution"
layer: primitives
...
# 070. API / Event Schema Evolution
...
**Optional fields**, **Required fields**, **Field renaming**, **Enum changes**, **Unknown fields**.

### Testing and evolution

**JSON evolution**, **Protobuf evolution**, **Old consumers**, **New consumers**, **Compatibility testing**.

### Boundary of the paper

This paper treats **API / Event Schema Evolution** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — JSON evolution:** Define the exact semantics of **JSON evolution** within API / Event Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Optional fields:** Specify presence, null, empty, whitespace-only, default, and unknown-field semantics separately for create, replace, and partial update.
- **SHOULD — Field removal:** Use expand-and-contract: add the new representation, deploy compatible readers/writers, backfill and verify, switch reads, stop old writes, then remove after the compatibility horizon.
- **SHOULD — Enum changes:** Define the exact semantics of **Enum changes** within API / Event Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Old consumers:** Define the exact semantics of **Old consumers** within API / Event Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Compatibility testing:** Define the exact semantics of **Compatibility testing** within API / Event Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **JSON evolution** within API / Event Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for json evolution is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for json evolution, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Protobuf evolution

- **SHOULD — engineering rule:** Define the exact semantics of **Protobuf evolution** within API / Event Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for protobuf evolution is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for protobuf evolution, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Enum changes** within API / Event Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for enum changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for enum changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Old consumers** within API / Event Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for old consumers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for old consumers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. New consumers

- **SHOULD — engineering rule:** Define the exact semantics of **New consumers** within API / Event Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for new consumers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for new consumers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Compatibility testing

- **SHOULD — engineering rule:** Define the exact semantics of **Compatibility testing** within API / Event Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for compatibility testing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for compatibility testing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every API / Event Schema Evolution path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `JSON evolution`, `Optional fields`, `Field renaming`. |
...
| Privacy / retention | Classify data produced or touched by API / Event Schema Evolution; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **API / Event Schema Evolution** before choosing framework or provider mechanisms.
...
- **MUST** — For **JSON evolution**: Define the exact semantics of **JSON evolution** within API / Event Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Optional fields**: Specify presence, null, empty, whitespace-only, default, and unknown-field semantics separately for create, replace, and partial update.
- **MUST** — For **Field removal**: Use expand-and-contract: add the new representation, deploy compatible readers/writers, backfill and verify, switch reads, stop old writes, then remove after the compatibility horizon.
- **MUST** — For **Enum changes**: Define the exact semantics of **Enum changes** within API / Event Schema Evolution: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **JSON evolution:** Locate every implementation path for json evolution, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Optional fields:** Build a contract matrix for omitted/null/empty/zero/false/unknown values and verify serialization round trips.
- [ ] **Field removal:** Record per-stage invariants and gates; inspect telemetry proving old reads/writes have ceased before contraction.
- [ ] **Enum changes:** Locate every implementation path for enum changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Old consumers:** Locate every implementation path for old consumers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Compatibility testing:** Locate every implementation path for compatibility testing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **JSON evolution:** A framework or provider default for json evolution is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Optional fields:** Clients cannot clear a field, defaults overwrite explicit values, or silent unknown fields hide typos and compatibility bugs.
...
- **Field renaming:** Mixed versions lose data, defaults change meaning, or rollback becomes impossible.
...
- **Old consumers:** A framework or provider default for old consumers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Compatibility testing:** A framework or provider default for compatibility testing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **API / Event Schema Evolution** correct, and which component is authoritative for it?
...
- For **JSON evolution**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for json evolution is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Optional fields**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Clients cannot clear a field, defaults overwrite explicit values, or silent unknown fields hide typos and compatibility bugs.
- For **Field removal**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Mixed versions lose data, defaults change meaning, or rollback becomes impossible.
- For **Enum changes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for enum changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Old consumers**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for old consumers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **API / Event Schema Evolution**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Field renaming:** Record per-stage invariants and gates; inspect telemetry proving old reads/writes have ceased before contraction.
...
**Paper metadata:** canonical subtopics: 11; layer: `primitives`; domain profile: `migration`; verified through: `2026-08-17`.
