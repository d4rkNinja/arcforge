---
paper_number: 31
title: "Data Migrations & Backfills"
layer: primitives
...
**Historical data migration**, **Backfills**, **Batch processing**, **Chunking**, **Resume support**, **Idempotent migration**, **Progress tracking**, **Reconciliation**, **Production load control**, **Dual systems during migration**.

### Boundary of the paper

This paper treats **Data Migrations & Backfills** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Historical data migration:** Define the exact semantics of **Historical data migration** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Batch processing:** Define the exact semantics of **Batch processing** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MUST — Validation:** Define the exact semantics of **Validation** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Reconciliation:** Define the exact semantics of **Reconciliation** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Dual systems during migration:** Define the exact semantics of **Dual systems during migration** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.1. Historical data migration

- **SHOULD — engineering rule:** Define the exact semantics of **Historical data migration** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for historical data migration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for historical data migration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Batch processing** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for batch processing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for batch processing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.6. Idempotent migration

- **MUST — engineering rule:** Define the exact semantics of **Idempotent migration** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for idempotent migration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for idempotent migration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST — engineering rule:** Define the exact semantics of **Validation** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Reconciliation** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.11. Production load control

- **SHOULD — engineering rule:** Define the exact semantics of **Production load control** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for production load control is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for production load control, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.12. Dual systems during migration

- **SHOULD — engineering rule:** Define the exact semantics of **Dual systems during migration** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for dual systems during migration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for dual systems during migration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Data Migrations & Backfills path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Historical data migration`, `Chunking`, `Progress tracking`. |
...
| Privacy / retention | Classify data produced or touched by Data Migrations & Backfills; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Data Migrations & Backfills** before choosing framework or provider mechanisms.
...
- **MUST** — For **Historical data migration**: Define the exact semantics of **Historical data migration** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Batch processing**: Define the exact semantics of **Batch processing** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Resume support**: Process bounded deterministic chunks with checkpoints after durable commit, idempotent writes, rate/resource limits, pause/resume, version guards, and validation samples plus full reconciliation.
- **MUST** — For **Validation**: Define the exact semantics of **Validation** within Data Migrations & Backfills: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Historical data migration:** Locate every implementation path for historical data migration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Batch processing:** Locate every implementation path for batch processing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Resume support:** Kill workers at every boundary, mutate rows during backfill, resume from checkpoints, and compare source/target counts and checksums.
...
- [ ] **Dual systems during migration:** Locate every implementation path for dual systems during migration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Historical data migration:** A framework or provider default for historical data migration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Batch processing:** A framework or provider default for batch processing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Resume support:** Restart repeats or skips rows, live writes are overwritten by stale data, or the job saturates production.
- **Progress tracking:** Restart repeats or skips rows, live writes are overwritten by stale data, or the job saturates production.
...
- **Dual systems during migration:** A framework or provider default for dual systems during migration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Data Migrations & Backfills** correct, and which component is authoritative for it?
...
- For **Historical data migration**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for historical data migration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Batch processing**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for batch processing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Resume support**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Restart repeats or skips rows, live writes are overwritten by stale data, or the job saturates production.
...
- [ ] Map every entry point for **Data Migrations & Backfills**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Batch processing:** Locate every implementation path for batch processing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Resume support:** Kill workers at every boundary, mutate rows during backfill, resume from checkpoints, and compare source/target counts and checksums.
- [ ] **Progress tracking:** Kill workers at every boundary, mutate rows during backfill, resume from checkpoints, and compare source/target counts and checksums.
- [ ] **Validation:** Locate every implementation path for validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Reconciliation:** Locate every implementation path for reconciliation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Dual systems during migration:** Locate every implementation path for dual systems during migration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
**Paper metadata:** canonical subtopics: 12; layer: `primitives`; domain profile: `migration`; verified through: `2026-08-17`.
