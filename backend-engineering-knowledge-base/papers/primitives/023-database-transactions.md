...
**Transaction boundaries**, **Repeatable read**, **Transaction retries**, **Long transactions**, **Nested transactions**, **Transaction scope**, **Transaction + network call**, **Transaction + queue publishing**.

### Concurrency and distributed behavior

**ACID**, **Commit**, **Isolation levels**, **Read committed**, **Serializable**, **Snapshot isolation**, **Deadlocks**, **Lock timeouts**, **Savepoints**.
...
This paper treats **Database Transactions** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — ACID:** Define the exact semantics of **ACID** within Database Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — Serializable:** Select isolation by anomaly analysis, not name familiarity. Account for database-specific semantics and retry complete transactions on serialization failure.
- **MUST — Deadlocks:** Use a consistent lock acquisition order, short transactions, bounded waits, and whole-transaction retries with jitter and attempt limits.
- **SHOULD — Savepoints:** Define the exact semantics of **Savepoints** within Database Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Transaction + queue publishing:** Define the exact semantics of **Transaction + queue publishing** within Database Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **ACID** within Database Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for acid is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for acid, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Transaction boundaries

- **MUST — engineering rule:** Define the exact semantics of **Transaction boundaries** within Database Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Commit** within Database Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for commit is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for commit, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Select isolation by anomaly analysis, not name familiarity. Account for database-specific semantics and retry complete transactions on serialization failure.
- **Production failure mode:** Lost updates, write skew, or phantoms violate invariants despite all statements being inside a transaction.
- **Existing-codebase evidence:** Construct concurrent schedules that would violate the invariant and verify the chosen isolation/locking rejects one participant.
...
### 7.9. Snapshot isolation

- **MUST — engineering rule:** Select isolation by anomaly analysis, not name familiarity. Account for database-specific semantics and retry complete transactions on serialization failure.
- **Production failure mode:** Lost updates, write skew, or phantoms violate invariants despite all statements being inside a transaction.
- **Existing-codebase evidence:** Construct concurrent schedules that would violate the invariant and verify the chosen isolation/locking rejects one participant.

### 7.10. Transaction retries

- **MUST — engineering rule:** Define the exact semantics of **Transaction retries** within Database Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for transaction retries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for transaction retries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Deadlocks

- **MUST — engineering rule:** Use a consistent lock acquisition order, short transactions, bounded waits, and whole-transaction retries with jitter and attempt limits.
...
- **Production failure mode:** A framework or provider default for long transactions is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for long transactions, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.14. Nested transactions

- **MUST — engineering rule:** Define the exact semantics of **Nested transactions** within Database Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for nested transactions is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for nested transactions, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.15. Savepoints

- **SHOULD — engineering rule:** Define the exact semantics of **Savepoints** within Database Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for savepoints is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for savepoints, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.16. Transaction scope
...
- **Production failure mode:** A repository transaction omits one required write, or locks are held while calling a slow provider.
- **Existing-codebase evidence:** Document the invariant and map every read/write/side effect to before, inside, or after commit.

### 7.17. Transaction + network call

- **MUST — engineering rule:** Define the exact semantics of **Transaction + network call** within Database Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for transaction + network call is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for transaction + network call, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.18. Transaction + queue publishing

- **MUST — engineering rule:** Define the exact semantics of **Transaction + queue publishing** within Database Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for transaction + queue publishing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for transaction + queue publishing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Database Transactions path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `ACID`, `Isolation levels`, `Snapshot isolation`. |
...
| Privacy / retention | Classify data produced or touched by Database Transactions; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Database Transactions** before choosing framework or provider mechanisms.
...
- **MUST** — For **ACID**: Define the exact semantics of **ACID** within Database Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MUST** — For **Serializable**: Select isolation by anomaly analysis, not name familiarity. Account for database-specific semantics and retry complete transactions on serialization failure.
- **MUST** — For **Deadlocks**: Use a consistent lock acquisition order, short transactions, bounded waits, and whole-transaction retries with jitter and attempt limits.
...
- [ ] **ACID:** Locate every implementation path for acid, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Rollback:** Perform a production-like rollback drill after generating new-version data and partially completed work.
- [ ] **Serializable:** Construct concurrent schedules that would violate the invariant and verify the chosen isolation/locking rejects one participant.
- [ ] **Deadlocks:** Force reversed lock order in tests and verify classification, rollback, retry, metrics, and user-visible outcome.
- [ ] **Savepoints:** Locate every implementation path for savepoints, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Transaction + queue publishing:** Locate every implementation path for transaction + queue publishing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **ACID:** A framework or provider default for acid is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Rollback:** Code rolls back while schema/data/side effects do not, creating a second outage or corruption.
- **Repeatable read:** Lost updates, write skew, or phantoms violate invariants despite all statements being inside a transaction.
- **Snapshot isolation:** Lost updates, write skew, or phantoms violate invariants despite all statements being inside a transaction.
- **Lock timeouts:** Requests wait indefinitely, downstream work continues after callers leave, or nested timeouts exceed the original budget.
- **Savepoints:** A framework or provider default for savepoints is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Transaction + queue publishing:** A framework or provider default for transaction + queue publishing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Database Transactions** correct, and which component is authoritative for it?
...
- For **ACID**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for acid is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Serializable**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Lost updates, write skew, or phantoms violate invariants despite all statements being inside a transaction.
- For **Deadlocks**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Workers hang, retry only the failed statement, or create a synchronized retry storm.
- For **Savepoints**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for savepoints is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Database Transactions**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **ACID:** Locate every implementation path for acid, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Rollback:** Perform a production-like rollback drill after generating new-version data and partially completed work.
- [ ] **Repeatable read:** Construct concurrent schedules that would violate the invariant and verify the chosen isolation/locking rejects one participant.
- [ ] **Snapshot isolation:** Construct concurrent schedules that would violate the invariant and verify the chosen isolation/locking rejects one participant.
- [ ] **Lock timeouts:** Inject slow dependencies at each hop and verify cancellation reaches database, network calls, streams, and workers.
- [ ] **Savepoints:** Locate every implementation path for savepoints, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Transaction + queue publishing:** Locate every implementation path for transaction + queue publishing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
**Paper metadata:** canonical subtopics: 18; layer: `primitives`; domain profile: `transactions`; verified through: `2026-08-17`.
