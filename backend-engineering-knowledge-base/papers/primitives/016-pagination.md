...
**Offset pagination**, **Page-number pagination**, **Cursor pagination**, **Keyset pagination**, **Cursor encoding**, **Cursor validation**, **Maximum page size**, **Default limits**, **Total counts**, **Expensive counts**, **Deep pagination**.

### Persistence and integrity

**Stable ordering**, **Missing-record prevention**.

### Concurrency and distributed behavior

**Duplicate prevention**, **Concurrent writes**.

### Boundary of the paper

This paper treats **Pagination** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Offset pagination:** Use a total deterministic order with a unique tie-breaker; encode cursor position and query shape opaquely; validate limits and preserve authorization/filter semantics across pages.
- **SHOULD — Keyset pagination:** Use a total deterministic order with a unique tie-breaker; encode cursor position and query shape opaquely; validate limits and preserve authorization/filter semantics across pages.
- **SHOULD — Missing-record prevention:** Define the exact semantics of **Missing-record prevention** within Pagination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Cursor encoding:** Use a total deterministic order with a unique tie-breaker; encode cursor position and query shape opaquely; validate limits and preserve authorization/filter semantics across pages.
- **SHOULD — Default limits:** Define the exact semantics of **Default limits** within Pagination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Deep pagination:** Use a total deterministic order with a unique tie-breaker; encode cursor position and query shape opaquely; validate limits and preserve authorization/filter semantics across pages.
...
|---|---|---|
| Offset vs cursor/keyset pagination | Offset is simple and supports random access but degrades and shifts under writes; keyset is stable and efficient but requires deterministic sortable keys. | Prefer keyset for large mutable datasets; use offset for bounded/admin datasets when trade-offs are explicit. |
...
### 7.7. Missing-record prevention

- **SHOULD — engineering rule:** Define the exact semantics of **Missing-record prevention** within Pagination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for missing-record prevention is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for missing-record prevention, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Concurrent writes
...
### 7.10. Cursor validation

- **MUST — engineering rule:** Use a total deterministic order with a unique tie-breaker; encode cursor position and query shape opaquely; validate limits and preserve authorization/filter semantics across pages.
- **Production failure mode:** Concurrent inserts/updates cause duplicates or omissions, cursors are tampered with, or deep offsets exhaust the database.
- **Existing-codebase evidence:** Paginate while mutating boundary rows and verify every eligible record appears at most once under the documented consistency model.

### 7.11. Maximum page size
...
- **SHOULD — engineering rule:** Define the exact semantics of **Default limits** within Pagination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for default limits is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for default limits, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. Total counts

- **SHOULD — engineering rule:** Define the exact semantics of **Total counts** within Pagination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for total counts is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for total counts, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.14. Expensive counts

- **SHOULD — engineering rule:** Define the exact semantics of **Expensive counts** within Pagination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for expensive counts is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for expensive counts, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.15. Deep pagination
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Pagination path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Offset pagination`, `Stable ordering`, `Concurrent writes`. |
...
| Privacy / retention | Classify data produced or touched by Pagination; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Pagination** before choosing framework or provider mechanisms.
...
- **MUST** — For **Offset pagination**: Use a total deterministic order with a unique tie-breaker; encode cursor position and query shape opaquely; validate limits and preserve authorization/filter semantics across pages.
- **MUST** — For **Keyset pagination**: Use a total deterministic order with a unique tie-breaker; encode cursor position and query shape opaquely; validate limits and preserve authorization/filter semantics across pages.
- **MUST** — For **Missing-record prevention**: Define the exact semantics of **Missing-record prevention** within Pagination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Cursor encoding**: Use a total deterministic order with a unique tie-breaker; encode cursor position and query shape opaquely; validate limits and preserve authorization/filter semantics across pages.
...
- **MAY** — Choose **Offset vs cursor/keyset pagination** according to the stated trade-off: Prefer keyset for large mutable datasets; use offset for bounded/admin datasets when trade-offs are explicit.
...
- [ ] **Offset pagination:** Paginate while mutating boundary rows and verify every eligible record appears at most once under the documented consistency model.
- [ ] **Keyset pagination:** Paginate while mutating boundary rows and verify every eligible record appears at most once under the documented consistency model.
- [ ] **Missing-record prevention:** Locate every implementation path for missing-record prevention, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Cursor encoding:** Paginate while mutating boundary rows and verify every eligible record appears at most once under the documented consistency model.
- [ ] **Default limits:** Locate every implementation path for default limits, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Deep pagination:** Paginate while mutating boundary rows and verify every eligible record appears at most once under the documented consistency model.
...
- **Offset pagination:** Concurrent inserts/updates cause duplicates or omissions, cursors are tampered with, or deep offsets exhaust the database.
- **Cursor pagination:** Concurrent inserts/updates cause duplicates or omissions, cursors are tampered with, or deep offsets exhaust the database.
...
- **Concurrent writes:** A probabilistic sleep-based test passes while the actual race remains reachable.
- **Cursor validation:** Concurrent inserts/updates cause duplicates or omissions, cursors are tampered with, or deep offsets exhaust the database.
- **Total counts:** A framework or provider default for total counts is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Deep pagination:** Concurrent inserts/updates cause duplicates or omissions, cursors are tampered with, or deep offsets exhaust the database.
...
- What exact invariant or user/system promise makes **Pagination** correct, and which component is authoritative for it?
...
- For **Offset pagination**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Concurrent inserts/updates cause duplicates or omissions, cursors are tampered with, or deep offsets exhaust the database.
- For **Keyset pagination**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Concurrent inserts/updates cause duplicates or omissions, cursors are tampered with, or deep offsets exhaust the database.
- For **Missing-record prevention**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for missing-record prevention is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Cursor encoding**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Concurrent inserts/updates cause duplicates or omissions, cursors are tampered with, or deep offsets exhaust the database.
- For **Default limits**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for default limits is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Pagination**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Offset pagination:** Paginate while mutating boundary rows and verify every eligible record appears at most once under the documented consistency model.
- [ ] **Cursor pagination:** Paginate while mutating boundary rows and verify every eligible record appears at most once under the documented consistency model.
...
- [ ] **Concurrent writes:** Instrument the exact read/check/write or lease boundary and run deterministic competing operations.
- [ ] **Cursor validation:** Paginate while mutating boundary rows and verify every eligible record appears at most once under the documented consistency model.
- [ ] **Total counts:** Locate every implementation path for total counts, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Deep pagination:** Paginate while mutating boundary rows and verify every eligible record appears at most once under the documented consistency model.
...
**Paper metadata:** canonical subtopics: 15; layer: `primitives`; domain profile: `api`; verified through: `2026-08-17`.
