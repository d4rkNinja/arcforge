...
**HTTP compression**, **Gzip**, **Brotli**, **Payload compression**, **CPU trade-offs**, **Compression bombs**, **Minimum payload thresholds**.

### Boundary of the paper

This paper treats **Compression** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — HTTP compression:** Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **SHOULD — Gzip:** Define the exact semantics of **Gzip** within Compression: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Payload compression:** Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **SHOULD — Storage compression:** Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **SHOULD — Compression bombs:** Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **SHOULD — Minimum payload thresholds:** Define the exact semantics of **Minimum payload thresholds** within Compression: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Gzip** within Compression: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for gzip is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for gzip, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Brotli

- **SHOULD — engineering rule:** Define the exact semantics of **Brotli** within Compression: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for brotli is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for brotli, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Payload compression

- **SHOULD — engineering rule:** Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **Production failure mode:** Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
- **Existing-codebase evidence:** Fuzz representative parsers, enforce pixel/duration/frame limits, kill hung workers, and reprocess after pipeline version changes.

### 7.5. Storage compression
...
### 7.7. Compression bombs

- **SHOULD — engineering rule:** Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **Production failure mode:** Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
- **Existing-codebase evidence:** Fuzz representative parsers, enforce pixel/duration/frame limits, kill hung workers, and reprocess after pipeline version changes.

### 7.8. Minimum payload thresholds

- **SHOULD — engineering rule:** Define the exact semantics of **Minimum payload thresholds** within Compression: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for minimum payload thresholds is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for minimum payload thresholds, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Compression path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `HTTP compression`, `Brotli`, `Storage compression`. |
...
| Privacy / retention | Classify data produced or touched by Compression; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Compression** before choosing framework or provider mechanisms.
...
- **MUST** — For **HTTP compression**: Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **MUST** — For **Gzip**: Define the exact semantics of **Gzip** within Compression: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Payload compression**: Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **MUST** — For **Storage compression**: Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
...
- [ ] **HTTP compression:** Fuzz representative parsers, enforce pixel/duration/frame limits, kill hung workers, and reprocess after pipeline version changes.
- [ ] **Gzip:** Locate every implementation path for gzip, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Payload compression:** Fuzz representative parsers, enforce pixel/duration/frame limits, kill hung workers, and reprocess after pipeline version changes.
- [ ] **Storage compression:** Fuzz representative parsers, enforce pixel/duration/frame limits, kill hung workers, and reprocess after pipeline version changes.
- [ ] **Compression bombs:** Fuzz representative parsers, enforce pixel/duration/frame limits, kill hung workers, and reprocess after pipeline version changes.
- [ ] **Minimum payload thresholds:** Locate every implementation path for minimum payload thresholds, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **HTTP compression:** Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
- **Gzip:** A framework or provider default for gzip is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Brotli:** A framework or provider default for brotli is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Storage compression:** Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
- **CPU trade-offs:** A micro-optimization shifts pressure to memory, database, or network and worsens production tails.
- **Compression bombs:** Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
- **Minimum payload thresholds:** A framework or provider default for minimum payload thresholds is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Compression** correct, and which component is authoritative for it?
...
- For **HTTP compression**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
- For **Gzip**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for gzip is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Payload compression**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
- For **Storage compression**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
- For **Compression bombs**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
...
- [ ] Map every entry point for **Compression**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **HTTP compression:** Fuzz representative parsers, enforce pixel/duration/frame limits, kill hung workers, and reprocess after pipeline version changes.
- [ ] **Gzip:** Locate every implementation path for gzip, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Brotli:** Locate every implementation path for brotli, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Storage compression:** Fuzz representative parsers, enforce pixel/duration/frame limits, kill hung workers, and reprocess after pipeline version changes.
- [ ] **CPU trade-offs:** Capture before/after profiles and end-to-end metrics with equivalent workloads and statistical confidence.
- [ ] **Compression bombs:** Fuzz representative parsers, enforce pixel/duration/frame limits, kill hung workers, and reprocess after pipeline version changes.
- [ ] **Minimum payload thresholds:** Locate every implementation path for minimum payload thresholds, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [039. Quotas](039-quotas.md) — layer: `primitives`; profile: `cache`.
...
**Paper metadata:** canonical subtopics: 8; layer: `primitives`; domain profile: `performance`; verified through: `2026-08-17`.
