...
**Request parsing**, **Request validation**, **Request context**, **Correlation ID**, **Request ID**, **Deadlines**, **Cancellation**, **Context propagation**, **Request body limits**, **Content-type validation**, **Response construction**, **Response headers**.
...
This paper treats **Request Lifecycle** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Request parsing:** Define the exact semantics of **Request parsing** within Request Lifecycle: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Authentication middleware:** Define the exact semantics of **Authentication middleware** within Request Lifecycle: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Request ID:** Accept externally supplied identifiers only under a bounded format and generate a trusted internal identifier when absent; propagate across logs, traces, jobs, events, and outbound calls.
- **SHOULD — Cancellation:** Establish an end-to-end deadline, allocate smaller downstream budgets, propagate cancellation, and stop expensive or side-effecting work when the result is no longer useful unless explicitly designed otherwise.
- **MUST — Content-type validation:** Treat declared type, filename extension, and content signature as independent signals; parse with a hardened library and apply an allowlist based on intended processing.
- **MUST — Request cleanup:** Define the exact semantics of **Request cleanup** within Request Lifecycle: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Request parsing** within Request Lifecycle: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for request parsing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for request parsing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Request validation

- **MUST — engineering rule:** Define the exact semantics of **Request validation** within Request Lifecycle: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for request validation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for request validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Request context

- **SHOULD — engineering rule:** Define the exact semantics of **Request context** within Request Lifecycle: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MUST — engineering rule:** Define the exact semantics of **Authentication middleware** within Request Lifecycle: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for authentication middleware is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for authentication middleware, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Authorization middleware

- **MUST — engineering rule:** Define the exact semantics of **Authorization middleware** within Request Lifecycle: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for authorization middleware is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for authorization middleware, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Correlation ID

- **SHOULD — engineering rule:** Accept externally supplied identifiers only under a bounded format and generate a trusted internal identifier when absent; propagate across logs, traces, jobs, events, and outbound calls.
- **Production failure mode:** Unbounded or malicious IDs poison logs, collide across tenants, or causal chains disappear at async boundaries.
- **Existing-codebase evidence:** Follow one request across all components and test malformed, duplicate, overlong, and attacker-controlled values.
...
### 7.14. Response construction

- **SHOULD — engineering rule:** Define the exact semantics of **Response construction** within Request Lifecycle: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for response construction is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for response construction, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.15. Response headers
...
### 7.16. Request cleanup

- **MUST — engineering rule:** Define the exact semantics of **Request cleanup** within Request Lifecycle: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for request cleanup is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for request cleanup, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Request Lifecycle path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Request parsing`, `Authorization middleware`, `Timeouts`. |
...
| Privacy / retention | Classify data produced or touched by Request Lifecycle; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Request Lifecycle** before choosing framework or provider mechanisms.
...
- **MUST** — For **Request parsing**: Define the exact semantics of **Request parsing** within Request Lifecycle: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Authentication middleware**: Define the exact semantics of **Authentication middleware** within Request Lifecycle: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Request ID**: Accept externally supplied identifiers only under a bounded format and generate a trusted internal identifier when absent; propagate across logs, traces, jobs, events, and outbound calls.
- **MUST** — For **Cancellation**: Establish an end-to-end deadline, allocate smaller downstream budgets, propagate cancellation, and stop expensive or side-effecting work when the result is no longer useful unless explicitly designed otherwise.
...
- [ ] **Request parsing:** Locate every implementation path for request parsing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Authentication middleware:** Locate every implementation path for authentication middleware, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Request ID:** Follow one request across all components and test malformed, duplicate, overlong, and attacker-controlled values.
...
- [ ] **Content-type validation:** Use malformed, polyglot, spoofed-extension, and parser-differential fixtures through the entire upload/download path.
- [ ] **Request cleanup:** Locate every implementation path for request cleanup, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Request parsing:** A framework or provider default for request parsing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Correlation ID:** Unbounded or malicious IDs poison logs, collide across tenants, or causal chains disappear at async boundaries.
...
- **Context propagation:** Causal chains break, retries look like unrelated requests, or sensitive data is exported to telemetry vendors.
- **Content-type validation:** A polyglot or mislabeled file reaches an unsafe parser or is served with active content semantics.
- **Request cleanup:** A framework or provider default for request cleanup is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Request Lifecycle** correct, and which component is authoritative for it?
...
- For **Request parsing**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for request parsing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Authentication middleware**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for authentication middleware is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Request ID**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Unbounded or malicious IDs poison logs, collide across tenants, or causal chains disappear at async boundaries.
- For **Cancellation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Requests wait indefinitely, downstream work continues after callers leave, or nested timeouts exceed the original budget.
- For **Content-type validation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A polyglot or mislabeled file reaches an unsafe parser or is served with active content semantics.
...
- [ ] Map every entry point for **Request Lifecycle**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Correlation ID:** Follow one request across all components and test malformed, duplicate, overlong, and attacker-controlled values.
- [ ] **Timeouts:** Inject slow dependencies at each hop and verify cancellation reaches database, network calls, streams, and workers.
- [ ] **Context propagation:** Trace a request through database, cache, queue, worker, and provider; verify parent/link semantics and redaction.
- [ ] **Content-type validation:** Use malformed, polyglot, spoofed-extension, and parser-differential fixtures through the entire upload/download path.
- [ ] **Request cleanup:** Locate every implementation path for request cleanup, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
**Paper metadata:** canonical subtopics: 16; layer: `primitives`; domain profile: `api`; verified through: `2026-08-17`.
