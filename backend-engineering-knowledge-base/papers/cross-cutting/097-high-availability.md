...
**High Availability** exists to restore correct service and data after deletion, corruption, regional failure, or operator error within explicit recovery objectives. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Redundancy**, **Active/passive**, **Active/active**, **Multi-zone**, **Multi-region**, **Automated failover**, **Health checks**, **Failback**, **Dependency redundancy**.

### Boundary of the paper

This paper treats **High Availability** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Redundancy:** Define the exact semantics of **Redundancy** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MAY — Active/active:** Define the exact semantics of **Active/active** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MAY — Multi-region:** Define the exact semantics of **Multi-region** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Automated failover:** Define the exact semantics of **Automated failover** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Failback:** Define what can be reversed, what requires compensation or forward repair, and how data written by the new version remains readable. Rehearse the exact control-plane and data-plane sequence.
- **SHOULD — Data replication:** Specify write acknowledgment, read consistency, lag bounds, failover fencing, conflict handling, and behavior during partitions. Do not equate replication with backup.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Redundancy** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for redundancy is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for redundancy, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Active/passive

- **SHOULD — engineering rule:** Define the exact semantics of **Active/passive** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY — engineering rule:** Define the exact semantics of **Active/active** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Multi-zone** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for multi-zone is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for multi-zone, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Multi-region

- **MAY — engineering rule:** Define the exact semantics of **Multi-region** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for multi-region is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for multi-region, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Automated failover

- **SHOULD — engineering rule:** Define the exact semantics of **Automated failover** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for automated failover is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for automated failover, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.9. Dependency redundancy

- **SHOULD — engineering rule:** Define the exact semantics of **Dependency redundancy** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for dependency redundancy is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for dependency redundancy, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every High Availability path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Redundancy`, `Active/active`, `Multi-region`. |
...
| Privacy / retention | Classify data produced or touched by High Availability; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **High Availability** before choosing framework or provider mechanisms.
...
- **MUST** — For **Redundancy**: Define the exact semantics of **Redundancy** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Active/active**: Define the exact semantics of **Active/active** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Multi-region**: Define the exact semantics of **Multi-region** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Automated failover**: Define the exact semantics of **Automated failover** within High Availability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Redundancy:** Locate every implementation path for redundancy, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Multi-region:** Locate every implementation path for multi-region, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Automated failover:** Locate every implementation path for automated failover, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Failback:** Perform a production-like rollback drill after generating new-version data and partially completed work.
- [ ] **Data replication:** Inject lag, leader loss, partition, and failback; verify documented read/write guarantees and conflict outcomes.
...
- **Redundancy:** A framework or provider default for redundancy is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Multi-zone:** A framework or provider default for multi-zone is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Multi-region:** A framework or provider default for multi-region is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Health checks:** A dependency outage triggers restart storms, or a process receives traffic before initialization and migrations are complete.
- **Dependency redundancy:** A framework or provider default for dependency redundancy is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Data replication:** Stale reads violate workflows, failover loses acknowledged writes, or split-brain accepts conflicting updates.
...
- What exact invariant or user/system promise makes **High Availability** correct, and which component is authoritative for it?
...
- For **Redundancy**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for redundancy is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Multi-region**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for multi-region is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Automated failover**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for automated failover is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Failback**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Code rolls back while schema/data/side effects do not, creating a second outage or corruption.
...
- [ ] Map every entry point for **High Availability**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Redundancy:** Locate every implementation path for redundancy, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Active/active:** Locate every implementation path for active/active, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Multi-zone:** Locate every implementation path for multi-zone, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Multi-region:** Locate every implementation path for multi-region, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Health checks:** Fail each dependency separately and verify probe response, routing, restart behavior, and probe cost under fleet scale.
- [ ] **Dependency redundancy:** Locate every implementation path for dependency redundancy, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Data replication:** Inject lag, leader loss, partition, and failback; verify documented read/write guarantees and conflict outcomes.
...
- [082. Service Discovery](../systems/082-service-discovery.md) — layer: `systems`; profile: `runtime`.
