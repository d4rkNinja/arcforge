...
**Leader election**, **Quorums**, **Raft**, **Paxos concepts**, **Replicated logs**, **Membership changes**, **Split brain**, **Linearizability**, **Consensus limitations**.

### Boundary of the paper

This paper treats **Distributed Consensus** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Raft:** Use a proven implementation and understand quorum membership, term/epoch, log commitment, joint configuration, disk durability, and failure detector limits. Fence old leaders at external resources.
- **SHOULD — Paxos concepts:** Use a proven implementation and understand quorum membership, term/epoch, log commitment, joint configuration, disk durability, and failure detector limits. Fence old leaders at external resources.
- **SHOULD — Membership changes:** Define the exact semantics of **Membership changes** within Distributed Consensus: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Split brain:** Use a proven implementation and understand quorum membership, term/epoch, log commitment, joint configuration, disk durability, and failure detector limits. Fence old leaders at external resources.
- **SHOULD — Consensus limitations:** Use a proven implementation and understand quorum membership, term/epoch, log commitment, joint configuration, disk durability, and failure detector limits. Fence old leaders at external resources.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Replicated logs** within Distributed Consensus: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for replicated logs is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for replicated logs, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Membership changes

- **SHOULD — engineering rule:** Define the exact semantics of **Membership changes** within Distributed Consensus: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for membership changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for membership changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Linearizability** within Distributed Consensus: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Distributed Consensus path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Leader election`, `Raft`, `Replicated logs`. |
...
| Privacy / retention | Classify data produced or touched by Distributed Consensus; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Distributed Consensus** before choosing framework or provider mechanisms.
...
- **MUST** — For **Leader election**: Use a proven implementation and understand quorum membership, term/epoch, log commitment, joint configuration, disk durability, and failure detector limits. Fence old leaders at external resources.
- **MUST** — For **Raft**: Use a proven implementation and understand quorum membership, term/epoch, log commitment, joint configuration, disk durability, and failure detector limits. Fence old leaders at external resources.
- **MUST** — For **Paxos concepts**: Use a proven implementation and understand quorum membership, term/epoch, log commitment, joint configuration, disk durability, and failure detector limits. Fence old leaders at external resources.
- **MUST** — For **Membership changes**: Define the exact semantics of **Membership changes** within Distributed Consensus: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Raft:** Test minority/majority partitions, membership change, disk loss, delayed messages, and recovery without manual log surgery.
- [ ] **Paxos concepts:** Test minority/majority partitions, membership change, disk loss, delayed messages, and recovery without manual log surgery.
- [ ] **Membership changes:** Locate every implementation path for membership changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Split brain:** Test minority/majority partitions, membership change, disk loss, delayed messages, and recovery without manual log surgery.
- [ ] **Consensus limitations:** Test minority/majority partitions, membership change, disk loss, delayed messages, and recovery without manual log surgery.
...
- **Quorums:** Operational changes break quorum, stale leaders serve writes, or engineers assume consensus keeps the service available through every partition.
- **Paxos concepts:** Operational changes break quorum, stale leaders serve writes, or engineers assume consensus keeps the service available through every partition.
- **Replicated logs:** A framework or provider default for replicated logs is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Membership changes:** A framework or provider default for membership changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Consensus limitations:** Operational changes break quorum, stale leaders serve writes, or engineers assume consensus keeps the service available through every partition.
...
- What exact invariant or user/system promise makes **Distributed Consensus** correct, and which component is authoritative for it?
...
- For **Raft**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Operational changes break quorum, stale leaders serve writes, or engineers assume consensus keeps the service available through every partition.
- For **Paxos concepts**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Operational changes break quorum, stale leaders serve writes, or engineers assume consensus keeps the service available through every partition.
- For **Membership changes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for membership changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Split brain**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Operational changes break quorum, stale leaders serve writes, or engineers assume consensus keeps the service available through every partition.
...
- [ ] Map every entry point for **Distributed Consensus**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Leader election:** Test minority/majority partitions, membership change, disk loss, delayed messages, and recovery without manual log surgery.
- [ ] **Quorums:** Test minority/majority partitions, membership change, disk loss, delayed messages, and recovery without manual log surgery.
- [ ] **Paxos concepts:** Test minority/majority partitions, membership change, disk loss, delayed messages, and recovery without manual log surgery.
- [ ] **Replicated logs:** Locate every implementation path for replicated logs, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Membership changes:** Locate every implementation path for membership changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [103. Distributed Locks](103-distributed-locks.md) — layer: `cross-cutting`; profile: `transactions`.
- [100. Replication](100-replication.md) — layer: `cross-cutting`; profile: `transactions`.
- [099. Consistency Models](099-consistency-models.md) — layer: `cross-cutting`; profile: `transactions`.
