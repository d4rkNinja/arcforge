...
**Leader/follower**, **Multi-leader**, **Leaderless**, **Synchronous replication**, **Asynchronous replication**, **Replica lag**, **Conflict resolution**, **Replication topology**.
...
This paper treats **Replication** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Leader/follower:** Specify write acknowledgment, read consistency, lag bounds, failover fencing, conflict handling, and behavior during partitions. Do not equate replication with backup.
- **SHOULD — Leaderless:** Specify write acknowledgment, read consistency, lag bounds, failover fencing, conflict handling, and behavior during partitions. Do not equate replication with backup.
- **SHOULD — Synchronous replication:** Specify write acknowledgment, read consistency, lag bounds, failover fencing, conflict handling, and behavior during partitions. Do not equate replication with backup.
- **SHOULD — Replica lag:** Define which reads may be stale and how read-your-writes is achieved after mutation. Route authorization, uniqueness, and state-transition checks to an appropriately consistent source.
- **SHOULD — Conflict resolution:** Define the exact semantics of **Conflict resolution** within Replication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Replication topology:** Specify write acknowledgment, read consistency, lag bounds, failover fencing, conflict handling, and behavior during partitions. Do not equate replication with backup.
...
|---|---|---|
| Leader/follower vs multi-leader/leaderless replication | Single-leader simplifies conflict ordering; multi-writer models improve locality/availability but require conflict semantics. | Choose from write geography, consistency, and conflict tolerance. |
...
### 7.4. Synchronous replication

- **SHOULD — engineering rule:** Specify write acknowledgment, read consistency, lag bounds, failover fencing, conflict handling, and behavior during partitions. Do not equate replication with backup.
- **Production failure mode:** Stale reads violate workflows, failover loses acknowledged writes, or split-brain accepts conflicting updates.
- **Existing-codebase evidence:** Inject lag, leader loss, partition, and failback; verify documented read/write guarantees and conflict outcomes.

### 7.5. Asynchronous replication
...
- **SHOULD — engineering rule:** Define the exact semantics of **Conflict resolution** within Replication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Failover** within Replication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Replication path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Leader/follower`, `Leaderless`, `Asynchronous replication`. |
...
| Privacy / retention | Classify data produced or touched by Replication; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Replication** before choosing framework or provider mechanisms.
...
- **MUST** — For **Leader/follower**: Specify write acknowledgment, read consistency, lag bounds, failover fencing, conflict handling, and behavior during partitions. Do not equate replication with backup.
- **MUST** — For **Leaderless**: Specify write acknowledgment, read consistency, lag bounds, failover fencing, conflict handling, and behavior during partitions. Do not equate replication with backup.
- **MUST** — For **Synchronous replication**: Specify write acknowledgment, read consistency, lag bounds, failover fencing, conflict handling, and behavior during partitions. Do not equate replication with backup.
- **MUST** — For **Replica lag**: Define which reads may be stale and how read-your-writes is achieved after mutation. Route authorization, uniqueness, and state-transition checks to an appropriately consistent source.
...
- **MAY** — Choose **Leader/follower vs multi-leader/leaderless replication** according to the stated trade-off: Choose from write geography, consistency, and conflict tolerance.
...
- [ ] **Leader/follower:** Inject lag, leader loss, partition, and failback; verify documented read/write guarantees and conflict outcomes.
- [ ] **Leaderless:** Inject lag, leader loss, partition, and failback; verify documented read/write guarantees and conflict outcomes.
- [ ] **Synchronous replication:** Inject lag, leader loss, partition, and failback; verify documented read/write guarantees and conflict outcomes.
- [ ] **Replica lag:** Inject lag and failover; test post-write reads, monotonic pagination, and fallback to primary.
...
- [ ] **Replication topology:** Inject lag, leader loss, partition, and failback; verify documented read/write guarantees and conflict outcomes.
...
- **Leader/follower:** Stale reads violate workflows, failover loses acknowledged writes, or split-brain accepts conflicting updates.
- **Multi-leader:** Stale reads violate workflows, failover loses acknowledged writes, or split-brain accepts conflicting updates.
- **Synchronous replication:** Stale reads violate workflows, failover loses acknowledged writes, or split-brain accepts conflicting updates.
- **Asynchronous replication:** Stale reads violate workflows, failover loses acknowledged writes, or split-brain accepts conflicting updates.
- **Replica lag:** A just-written record appears missing, a revoked permission remains active, or a stale check authorizes an invalid transition.
...
- **Replication topology:** Stale reads violate workflows, failover loses acknowledged writes, or split-brain accepts conflicting updates.
...
- What exact invariant or user/system promise makes **Replication** correct, and which component is authoritative for it?
...
- For **Leader/follower**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Stale reads violate workflows, failover loses acknowledged writes, or split-brain accepts conflicting updates.
- For **Leaderless**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Stale reads violate workflows, failover loses acknowledged writes, or split-brain accepts conflicting updates.
- For **Synchronous replication**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Stale reads violate workflows, failover loses acknowledged writes, or split-brain accepts conflicting updates.
- For **Replica lag**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A just-written record appears missing, a revoked permission remains active, or a stale check authorizes an invalid transition.
...
- [ ] Map every entry point for **Replication**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Leader/follower:** Inject lag, leader loss, partition, and failback; verify documented read/write guarantees and conflict outcomes.
- [ ] **Multi-leader:** Inject lag, leader loss, partition, and failback; verify documented read/write guarantees and conflict outcomes.
- [ ] **Synchronous replication:** Inject lag, leader loss, partition, and failback; verify documented read/write guarantees and conflict outcomes.
- [ ] **Asynchronous replication:** Inject lag, leader loss, partition, and failback; verify documented read/write guarantees and conflict outcomes.
- [ ] **Replica lag:** Inject lag and failover; test post-write reads, monotonic pagination, and fallback to primary.
