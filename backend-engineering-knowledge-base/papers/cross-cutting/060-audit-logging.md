...
**Audit Logging** exists to make system behavior, causality, saturation, and security-relevant actions reconstructable without leaking sensitive data. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Immutable audit logs**, **Audit search**, **Sensitive-field masking**.

### Operations and observability

**Actor**, **Action**, **Resource**, **Timestamp**, **Before/after data**, **Admin actions**, **Tamper resistance**.

### Boundary of the paper

This paper treats **Audit Logging** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Actor:** Record real actor, effective actor, action, resource, tenant, timestamp, request/trace, policy context, outcome, and appropriately minimized change details in tamper-resistant storage.
- **SHOULD — Timestamp:** Define the exact semantics of **Timestamp** within Audit Logging: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Before/after data:** Record real actor, effective actor, action, resource, tenant, timestamp, request/trace, policy context, outcome, and appropriately minimized change details in tamper-resistant storage.
- **SHOULD — Admin actions:** Record real actor, effective actor, action, resource, tenant, timestamp, request/trace, policy context, outcome, and appropriately minimized change details in tamper-resistant storage.
- **MUST — Audit retention:** Record real actor, effective actor, action, resource, tenant, timestamp, request/trace, policy context, outcome, and appropriately minimized change details in tamper-resistant storage.
- **SHOULD — Tamper resistance:** Prevent in-place mutation at the write boundary; represent corrections as new records linked to prior versions and protect ordering/identity. Tamper evidence must cover canonical serialization and key lifecycle.
...
|---|---|---|
| Mutable log store vs append-only/tamper-evident audit store | Ordinary logs are searchable and cheap but mutable/sampled; stronger audit storage improves evidence at cost and complexity. | Separate audit guarantees from application logging according to threat and compliance needs. |
...
- **SHOULD — engineering rule:** Record real actor, effective actor, action, resource, tenant, timestamp, request/trace, policy context, outcome, and appropriately minimized change details in tamper-resistant storage.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Action** within Audit Logging: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for action is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for action, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Resource

- **SHOULD — engineering rule:** Define the exact semantics of **Resource** within Audit Logging: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for resource is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for resource, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Timestamp

- **SHOULD — engineering rule:** Define the exact semantics of **Timestamp** within Audit Logging: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for timestamp is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for timestamp, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Request context

- **SHOULD — engineering rule:** Define the exact semantics of **Request context** within Audit Logging: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for request context is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for request context, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Before/after data

- **SHOULD — engineering rule:** Record real actor, effective actor, action, resource, tenant, timestamp, request/trace, policy context, outcome, and appropriately minimized change details in tamper-resistant storage.
- **Production failure mode:** Security-sensitive changes cannot be attributed, audit data leaks secrets, or failed attempts vanish.
- **Existing-codebase evidence:** Exercise successful/failed/admin/impersonated/bulk actions and verify atomicity, retention, search, export, and access controls.

### 7.7. Authentication events

- **MUST — engineering rule:** Define the exact semantics of **Authentication events** within Audit Logging: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for authentication events is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for authentication events, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Authorization events

- **MUST — engineering rule:** Define the exact semantics of **Authorization events** within Audit Logging: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for authorization events is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for authorization events, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.10. Immutable audit logs

- **MUST — engineering rule:** Prevent in-place mutation at the write boundary; represent corrections as new records linked to prior versions and protect ordering/identity. Tamper evidence must cover canonical serialization and key lifecycle.
- **Production failure mode:** An update erases history, sequence gaps go unnoticed, or a hash chain provides false assurance because an attacker can rewrite both data and hashes.
- **Existing-codebase evidence:** Attempt every mutation path, verify correction lineage, and test independent verification/export of the chain.

### 7.11. Audit retention
...
### 7.14. Tamper resistance

- **SHOULD — engineering rule:** Prevent in-place mutation at the write boundary; represent corrections as new records linked to prior versions and protect ordering/identity. Tamper evidence must cover canonical serialization and key lifecycle.
- **Production failure mode:** An update erases history, sequence gaps go unnoticed, or a hash chain provides false assurance because an attacker can rewrite both data and hashes.
- **Existing-codebase evidence:** Attempt every mutation path, verify correction lineage, and test independent verification/export of the chain.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Audit Logging path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Actor`, `Timestamp`, `Authentication events`. |
...
| Privacy / retention | Classify data produced or touched by Audit Logging; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Audit Logging** before choosing framework or provider mechanisms.
...
- **MUST** — For **Actor**: Record real actor, effective actor, action, resource, tenant, timestamp, request/trace, policy context, outcome, and appropriately minimized change details in tamper-resistant storage.
- **MUST** — For **Timestamp**: Define the exact semantics of **Timestamp** within Audit Logging: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Before/after data**: Record real actor, effective actor, action, resource, tenant, timestamp, request/trace, policy context, outcome, and appropriately minimized change details in tamper-resistant storage.
- **MUST** — For **Admin actions**: Record real actor, effective actor, action, resource, tenant, timestamp, request/trace, policy context, outcome, and appropriately minimized change details in tamper-resistant storage.
...
- **MAY** — Choose **Mutable log store vs append-only/tamper-evident audit store** according to the stated trade-off: Separate audit guarantees from application logging according to threat and compliance needs.
...
- [ ] **Actor:** Exercise successful/failed/admin/impersonated/bulk actions and verify atomicity, retention, search, export, and access controls.
- [ ] **Timestamp:** Locate every implementation path for timestamp, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Before/after data:** Exercise successful/failed/admin/impersonated/bulk actions and verify atomicity, retention, search, export, and access controls.
- [ ] **Admin actions:** Exercise successful/failed/admin/impersonated/bulk actions and verify atomicity, retention, search, export, and access controls.
- [ ] **Audit retention:** Exercise successful/failed/admin/impersonated/bulk actions and verify atomicity, retention, search, export, and access controls.
- [ ] **Tamper resistance:** Attempt every mutation path, verify correction lineage, and test independent verification/export of the chain.
...
- **Actor:** Security-sensitive changes cannot be attributed, audit data leaks secrets, or failed attempts vanish.
- **Resource:** A framework or provider default for resource is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Request context:** A framework or provider default for request context is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Authentication events:** A framework or provider default for authentication events is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Immutable audit logs:** An update erases history, sequence gaps go unnoticed, or a hash chain provides false assurance because an attacker can rewrite both data and hashes.
- **Audit search:** Security-sensitive changes cannot be attributed, audit data leaks secrets, or failed attempts vanish.
- **Tamper resistance:** An update erases history, sequence gaps go unnoticed, or a hash chain provides false assurance because an attacker can rewrite both data and hashes.
...
- What exact invariant or user/system promise makes **Audit Logging** correct, and which component is authoritative for it?
...
- For **Actor**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Security-sensitive changes cannot be attributed, audit data leaks secrets, or failed attempts vanish.
- For **Timestamp**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for timestamp is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Before/after data**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Security-sensitive changes cannot be attributed, audit data leaks secrets, or failed attempts vanish.
- For **Admin actions**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Security-sensitive changes cannot be attributed, audit data leaks secrets, or failed attempts vanish.
- For **Audit retention**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Security-sensitive changes cannot be attributed, audit data leaks secrets, or failed attempts vanish.
...
- [ ] Map every entry point for **Audit Logging**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Actor:** Exercise successful/failed/admin/impersonated/bulk actions and verify atomicity, retention, search, export, and access controls.
- [ ] **Resource:** Locate every implementation path for resource, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Request context:** Locate every implementation path for request context, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Authentication events:** Locate every implementation path for authentication events, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Immutable audit logs:** Attempt every mutation path, verify correction lineage, and test independent verification/export of the chain.
- [ ] **Audit search:** Exercise successful/failed/admin/impersonated/bulk actions and verify atomicity, retention, search, export, and access controls.
- [ ] **Tamper resistance:** Attempt every mutation path, verify correction lineage, and test independent verification/export of the chain.
...
- [009. Users & Account Lifecycle](../systems/009-users-and-account-lifecycle.md) — layer: `systems`; profile: `identity`.
...
**Paper metadata:** canonical subtopics: 14; layer: `cross-cutting`; domain profile: `observability`; verified through: `2026-08-17`.
