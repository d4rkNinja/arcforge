...
**Partitioning**, **Sharding**, **Replication**, **Hot partitions**.

### Operations and observability

**Vertical scaling**, **Horizontal scaling**, **Read scaling**, **Write scaling**, **Autoscaling**, **Hot keys**, **Fanout**, **Capacity planning**.

### Boundary of the paper

This paper treats **Scalability** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Vertical scaling:** Define the exact semantics of **Vertical scaling** within Scalability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Stateless services:** Define the exact semantics of **Stateless services** within Scalability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Replication:** Specify write acknowledgment, read consistency, lag bounds, failover fencing, conflict handling, and behavior during partitions. Do not equate replication with backup.
- **SHOULD — Write scaling:** Define the exact semantics of **Write scaling** within Scalability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Hot keys:** Use request coalescing/singleflight, bounded locking with fencing where needed, jittered expiry, negative caching, admission control, and precomputation for known hot objects.
- **SHOULD — Capacity planning:** Model realistic arrival rate, concurrency, payload/cardinality/skew, cache warmth, failures, and think time; observe tail latency, errors, saturation, queues, and cost through steady and recovery periods.
...
|---|---|---|
| Vertical vs horizontal scaling | Vertical scaling is operationally simple but bounded; horizontal scaling requires partitionable state and coordination. | Use measured bottlenecks and design stateful components for partitioning before scale forces it. |
...
- **SHOULD — engineering rule:** Define the exact semantics of **Vertical scaling** within Scalability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for vertical scaling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for vertical scaling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Horizontal scaling

- **SHOULD — engineering rule:** Define the exact semantics of **Horizontal scaling** within Scalability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Stateless services** within Scalability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for stateless services is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for stateless services, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Read scaling** within Scalability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for read scaling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for read scaling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Write scaling

- **SHOULD — engineering rule:** Define the exact semantics of **Write scaling** within Scalability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for write scaling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for write scaling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Autoscaling

- **SHOULD — engineering rule:** Define the exact semantics of **Autoscaling** within Scalability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Fanout** within Scalability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for fanout is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for fanout, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. Capacity planning
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Scalability path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Vertical scaling`, `Partitioning`, `Read scaling`. |
...
| Privacy / retention | Classify data produced or touched by Scalability; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Scalability** before choosing framework or provider mechanisms.
...
- **MUST** — For **Vertical scaling**: Define the exact semantics of **Vertical scaling** within Scalability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Stateless services**: Define the exact semantics of **Stateless services** within Scalability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Replication**: Specify write acknowledgment, read consistency, lag bounds, failover fencing, conflict handling, and behavior during partitions. Do not equate replication with backup.
- **MUST** — For **Write scaling**: Define the exact semantics of **Write scaling** within Scalability: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **Vertical vs horizontal scaling** according to the stated trade-off: Use measured bottlenecks and design stateful components for partitioning before scale forces it.
...
- [ ] **Vertical scaling:** Locate every implementation path for vertical scaling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Stateless services:** Locate every implementation path for stateless services, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Replication:** Inject lag, leader loss, partition, and failback; verify documented read/write guarantees and conflict outcomes.
- [ ] **Write scaling:** Locate every implementation path for write scaling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Hot keys:** Load-test synchronized expiry and high-cardinality misses while monitoring source QPS and waiter limits.
- [ ] **Capacity planning:** Run baseline, breakpoint, spike, and soak experiments with production-like data and explicit stop conditions.
...
- **Vertical scaling:** A framework or provider default for vertical scaling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Stateless services:** A framework or provider default for stateless services is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Sharding:** Monotonic or celebrity keys overload one shard, cross-shard queries become the norm, or resharding blocks writes.
- **Read scaling:** A framework or provider default for read scaling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Capacity planning:** A closed-loop benchmark hides overload, averages hide tail latency, or a short test misses leaks and compaction effects.
...
- What exact invariant or user/system promise makes **Scalability** correct, and which component is authoritative for it?
...
- For **Vertical scaling**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for vertical scaling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Stateless services**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for stateless services is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Replication**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Stale reads violate workflows, failover loses acknowledged writes, or split-brain accepts conflicting updates.
- For **Write scaling**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for write scaling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Hot keys**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A popular miss fans out to the source and causes cascading failure; malicious distinct misses defeat the cache.
...
- [ ] Map every entry point for **Scalability**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Vertical scaling:** Locate every implementation path for vertical scaling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Stateless services:** Locate every implementation path for stateless services, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Sharding:** Replay skewed production-like distributions and rehearse split/move/failover with concurrent reads and writes.
- [ ] **Read scaling:** Locate every implementation path for read scaling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
**Paper metadata:** canonical subtopics: 13; layer: `cross-cutting`; domain profile: `performance`; verified through: `2026-08-17`.
