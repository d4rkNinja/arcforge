...
**Backup** exists to restore correct service and data after deletion, corruption, regional failure, or operator error within explicit recovery objectives. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Full backups**, **Incremental backups**, **Snapshots**, **PITR**, **Backup verification**, **Backup failures**, **Cross-region copies**.
...
This paper treats **Backup** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Full backups:** Define the exact semantics of **Full backups** within Backup: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Snapshots:** Define the exact semantics of **Snapshots** within Backup: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Encryption:** Use well-reviewed libraries and current algorithms; define key purpose, scope, version, generation, storage, rotation, revocation, nonce uniqueness, and authenticated context.
- **MUST — Retention:** Define the exact semantics of **Retention** within Backup: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Backup failures:** Define the exact semantics of **Backup failures** within Backup: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Restore testing:** Automate integrity checks and regularly restore into an isolated environment, then validate application-level invariants, keys, object files, search indexes, and external reconciliation against RPO/RTO.
...
|---|---|---|
| Full vs incremental backup | Full backups simplify restore; incrementals reduce transfer/storage but create chains and dependency on logs. | Use complementary strategies and test complete restoration regularly. |
| Provider snapshot vs application-consistent backup | Provider snapshots are operationally efficient; application checkpoints coordinate multiple stores but need orchestration. | Combine storage-native protection with application-level restore validation and cross-store positions. |
...
- **SHOULD — engineering rule:** Define the exact semantics of **Full backups** within Backup: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for full backups is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for full backups, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Incremental backups

- **SHOULD — engineering rule:** Define the exact semantics of **Incremental backups** within Backup: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for incremental backups is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for incremental backups, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Snapshots

- **SHOULD — engineering rule:** Define the exact semantics of **Snapshots** within Backup: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for snapshots is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for snapshots, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. PITR

- **SHOULD — engineering rule:** Define the exact semantics of **PITR** within Backup: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for pitr is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for pitr, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST — engineering rule:** Define the exact semantics of **Retention** within Backup: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MUST — engineering rule:** Automate integrity checks and regularly restore into an isolated environment, then validate application-level invariants, keys, object files, search indexes, and external reconciliation against RPO/RTO.
- **Production failure mode:** Backups report success but are corrupt, missing dependencies, too slow, or unusable with current credentials/schema.
- **Existing-codebase evidence:** Run timed restore drills from each backup class and document measured recovery point, recovery time, and validation evidence.

### 7.8. Backup failures

- **SHOULD — engineering rule:** Define the exact semantics of **Backup failures** within Backup: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for backup failures is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for backup failures, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Cross-region copies

- **SHOULD — engineering rule:** Define the exact semantics of **Cross-region copies** within Backup: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for cross-region copies is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for cross-region copies, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Restore testing

- **SHOULD — engineering rule:** Automate integrity checks and regularly restore into an isolated environment, then validate application-level invariants, keys, object files, search indexes, and external reconciliation against RPO/RTO.
- **Production failure mode:** Backups report success but are corrupt, missing dependencies, too slow, or unusable with current credentials/schema.
- **Existing-codebase evidence:** Run timed restore drills from each backup class and document measured recovery point, recovery time, and validation evidence.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Backup path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Full backups`, `Snapshots`, `Encryption`. |
...
| Privacy / retention | Classify data produced or touched by Backup; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Backup** before choosing framework or provider mechanisms.
...
- **MUST** — For **Full backups**: Define the exact semantics of **Full backups** within Backup: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Snapshots**: Define the exact semantics of **Snapshots** within Backup: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Encryption**: Use well-reviewed libraries and current algorithms; define key purpose, scope, version, generation, storage, rotation, revocation, nonce uniqueness, and authenticated context.
- **MUST** — For **Retention**: Define the exact semantics of **Retention** within Backup: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **Full vs incremental backup** according to the stated trade-off: Use complementary strategies and test complete restoration regularly.
- **MAY** — Choose **Provider snapshot vs application-consistent backup** according to the stated trade-off: Combine storage-native protection with application-level restore validation and cross-store positions.
...
- [ ] **Full backups:** Locate every implementation path for full backups, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Snapshots:** Locate every implementation path for snapshots, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Encryption:** Review against a cryptographic design document and test key rotation, corruption, wrong context/key, entropy failure, and legacy ciphertext.
...
- [ ] **Backup failures:** Locate every implementation path for backup failures, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Restore testing:** Run timed restore drills from each backup class and document measured recovery point, recovery time, and validation evidence.
...
- **Full backups:** A framework or provider default for full backups is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Snapshots:** A framework or provider default for snapshots is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **PITR:** A framework or provider default for pitr is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Encryption:** Custom crypto, nonce reuse, weak randomness, key/algorithm confusion, or unauthenticated encryption destroys the intended guarantee.
- **Backup verification:** Backups report success but are corrupt, missing dependencies, too slow, or unusable with current credentials/schema.
- **Cross-region copies:** A framework or provider default for cross-region copies is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Restore testing:** Backups report success but are corrupt, missing dependencies, too slow, or unusable with current credentials/schema.
...
- What exact invariant or user/system promise makes **Backup** correct, and which component is authoritative for it?
...
- For **Full backups**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for full backups is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Snapshots**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for snapshots is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Encryption**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Custom crypto, nonce reuse, weak randomness, key/algorithm confusion, or unauthenticated encryption destroys the intended guarantee.
- For **Retention**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for retention is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Backup failures**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for backup failures is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Backup**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Full backups:** Locate every implementation path for full backups, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Snapshots:** Locate every implementation path for snapshots, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **PITR:** Locate every implementation path for pitr, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Encryption:** Review against a cryptographic design document and test key rotation, corruption, wrong context/key, entropy failure, and legacy ciphertext.
- [ ] **Backup verification:** Run timed restore drills from each backup class and document measured recovery point, recovery time, and validation evidence.
- [ ] **Cross-region copies:** Locate every implementation path for cross-region copies, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Restore testing:** Run timed restore drills from each backup class and document measured recovery point, recovery time, and validation evidence.
...
- [030. Database Migrations](030-database-migrations.md) — layer: `cross-cutting`; profile: `migration`.
