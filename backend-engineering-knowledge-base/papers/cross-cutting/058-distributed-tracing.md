...
**Distributed Tracing** exists to make system behavior, causality, saturation, and security-relevant actions reconstructable without leaking sensitive data. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Queue spans**, **Async trace propagation**.
...
**Trace IDs**, **Span IDs**, **Parent/child spans**, **Context propagation**, **DB spans**, **Cache spans**, **Sampling**.

### Boundary of the paper

This paper treats **Distributed Tracing** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Trace IDs:** Accept externally supplied identifiers only under a bounded format and generate a trusted internal identifier when absent; propagate across logs, traces, jobs, events, and outbound calls.
- **SHOULD — Parent/child spans:** Propagate standard trace context across synchronous and async boundaries, create spans around meaningful remote/queued work, record bounded attributes, and avoid secrets or large payloads.
- **SHOULD — DB spans:** Propagate standard trace context across synchronous and async boundaries, create spans around meaningful remote/queued work, record bounded attributes, and avoid secrets or large payloads.
- **SHOULD — Queue spans:** Propagate standard trace context across synchronous and async boundaries, create spans around meaningful remote/queued work, record bounded attributes, and avoid secrets or large payloads.
- **SHOULD — Async trace propagation:** Propagate standard trace context across synchronous and async boundaries, create spans around meaningful remote/queued work, record bounded attributes, and avoid secrets or large payloads.
- **MUST — Trace privacy:** Propagate standard trace context across synchronous and async boundaries, create spans around meaningful remote/queued work, record bounded attributes, and avoid secrets or large payloads.
...
### 7.8. External API spans

- **SHOULD — engineering rule:** Propagate standard trace context across synchronous and async boundaries, create spans around meaningful remote/queued work, record bounded attributes, and avoid secrets or large payloads.
- **Production failure mode:** Causal chains break, retries look like unrelated requests, or sensitive data is exported to telemetry vendors.
- **Existing-codebase evidence:** Trace a request through database, cache, queue, worker, and provider; verify parent/link semantics and redaction.

### 7.9. Async trace propagation
...
- **SHOULD — engineering rule:** Define the exact semantics of **Sampling** within Distributed Tracing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for sampling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for sampling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Trace privacy

- **MUST — engineering rule:** Propagate standard trace context across synchronous and async boundaries, create spans around meaningful remote/queued work, record bounded attributes, and avoid secrets or large payloads.
- **Production failure mode:** Causal chains break, retries look like unrelated requests, or sensitive data is exported to telemetry vendors.
- **Existing-codebase evidence:** Trace a request through database, cache, queue, worker, and provider; verify parent/link semantics and redaction.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Distributed Tracing path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Trace IDs`, `Parent/child spans`, `Cache spans`. |
...
| Privacy / retention | Classify data produced or touched by Distributed Tracing; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Distributed Tracing** before choosing framework or provider mechanisms.
...
- **MUST** — For **Trace IDs**: Accept externally supplied identifiers only under a bounded format and generate a trusted internal identifier when absent; propagate across logs, traces, jobs, events, and outbound calls.
- **MUST** — For **Parent/child spans**: Propagate standard trace context across synchronous and async boundaries, create spans around meaningful remote/queued work, record bounded attributes, and avoid secrets or large payloads.
- **MUST** — For **DB spans**: Propagate standard trace context across synchronous and async boundaries, create spans around meaningful remote/queued work, record bounded attributes, and avoid secrets or large payloads.
- **MUST** — For **Queue spans**: Propagate standard trace context across synchronous and async boundaries, create spans around meaningful remote/queued work, record bounded attributes, and avoid secrets or large payloads.
...
- [ ] **Parent/child spans:** Trace a request through database, cache, queue, worker, and provider; verify parent/link semantics and redaction.
- [ ] **DB spans:** Trace a request through database, cache, queue, worker, and provider; verify parent/link semantics and redaction.
- [ ] **Queue spans:** Trace a request through database, cache, queue, worker, and provider; verify parent/link semantics and redaction.
- [ ] **Async trace propagation:** Trace a request through database, cache, queue, worker, and provider; verify parent/link semantics and redaction.
- [ ] **Trace privacy:** Trace a request through database, cache, queue, worker, and provider; verify parent/link semantics and redaction.
...
- **Parent/child spans:** Causal chains break, retries look like unrelated requests, or sensitive data is exported to telemetry vendors.
...
- **Cache spans:** Causal chains break, retries look like unrelated requests, or sensitive data is exported to telemetry vendors.
- **External API spans:** Causal chains break, retries look like unrelated requests, or sensitive data is exported to telemetry vendors.
- **Async trace propagation:** Causal chains break, retries look like unrelated requests, or sensitive data is exported to telemetry vendors.
- **Trace privacy:** Causal chains break, retries look like unrelated requests, or sensitive data is exported to telemetry vendors.
...
- What exact invariant or user/system promise makes **Distributed Tracing** correct, and which component is authoritative for it?
...
- For **Trace IDs**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Unbounded or malicious IDs poison logs, collide across tenants, or causal chains disappear at async boundaries.
- For **Parent/child spans**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Causal chains break, retries look like unrelated requests, or sensitive data is exported to telemetry vendors.
- For **DB spans**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Causal chains break, retries look like unrelated requests, or sensitive data is exported to telemetry vendors.
- For **Queue spans**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Causal chains break, retries look like unrelated requests, or sensitive data is exported to telemetry vendors.
- For **Async trace propagation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Causal chains break, retries look like unrelated requests, or sensitive data is exported to telemetry vendors.
...
- [ ] Map every entry point for **Distributed Tracing**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Cache spans:** Trace a request through database, cache, queue, worker, and provider; verify parent/link semantics and redaction.
- [ ] **External API spans:** Trace a request through database, cache, queue, worker, and provider; verify parent/link semantics and redaction.
- [ ] **Async trace propagation:** Trace a request through database, cache, queue, worker, and provider; verify parent/link semantics and redaction.
- [ ] **Trace privacy:** Trace a request through database, cache, queue, worker, and provider; verify parent/link semantics and redaction.
...
**Paper metadata:** canonical subtopics: 11; layer: `cross-cutting`; domain profile: `observability`; verified through: `2026-08-17`.
