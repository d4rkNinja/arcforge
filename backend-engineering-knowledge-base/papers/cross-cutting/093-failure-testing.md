...
**Failure Testing** exists to produce evidence that observable behavior and invariants survive realistic inputs, concurrency, failures, and change. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Queue unavailable**, **Provider timeout**, **Network partition**, **Duplicate delivery**, **Out-of-order delivery**.

### Testing and evolution

**Redis unavailable**, **Provider 500**, **Disk full**, **Slow dependency**, **Worker crash**, **Partial writes**.

### Boundary of the paper

This paper treats **Failure Testing** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Database unavailable:** Define the exact semantics of **Database unavailable** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Queue unavailable:** Define the exact semantics of **Queue unavailable** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Provider 500:** Define the exact semantics of **Provider 500** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Slow dependency:** Define the exact semantics of **Slow dependency** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Partial writes:** Define the exact semantics of **Partial writes** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Out-of-order delivery:** State the ordering scope, assign monotonic per-aggregate/version metadata where needed, and make consumers reject, buffer, or reconcile stale/out-of-order items.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Database unavailable** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for database unavailable is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for database unavailable, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Redis unavailable

- **SHOULD — engineering rule:** Define the exact semantics of **Redis unavailable** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for redis unavailable is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for redis unavailable, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Queue unavailable

- **SHOULD — engineering rule:** Define the exact semantics of **Queue unavailable** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for queue unavailable is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for queue unavailable, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Provider 500** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for provider 500 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for provider 500, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Disk full** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for disk full is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for disk full, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Slow dependency

- **SHOULD — engineering rule:** Define the exact semantics of **Slow dependency** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for slow dependency is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for slow dependency, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Worker crash

- **SHOULD — engineering rule:** Define the exact semantics of **Worker crash** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for worker crash is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for worker crash, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Partial writes

- **SHOULD — engineering rule:** Define the exact semantics of **Partial writes** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.11. Duplicate delivery

- **SHOULD — engineering rule:** Define the exact semantics of **Duplicate delivery** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for duplicate delivery is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for duplicate delivery, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.12. Out-of-order delivery
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Failure Testing path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Database unavailable`, `Provider timeout`, `Disk full`. |
...
| Privacy / retention | Classify data produced or touched by Failure Testing; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Failure Testing** before choosing framework or provider mechanisms.
...
- **MUST** — For **Database unavailable**: Define the exact semantics of **Database unavailable** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Queue unavailable**: Define the exact semantics of **Queue unavailable** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Provider 500**: Define the exact semantics of **Provider 500** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Slow dependency**: Define the exact semantics of **Slow dependency** within Failure Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Database unavailable:** Locate every implementation path for database unavailable, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Queue unavailable:** Locate every implementation path for queue unavailable, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Provider 500:** Locate every implementation path for provider 500, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Slow dependency:** Locate every implementation path for slow dependency, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Out-of-order delivery:** Deliver permutations, duplicates, gaps, and partition rebalances; verify deterministic final state.
...
- Tests passing against mocks but failing under real isolation.
...
- **Database unavailable:** A framework or provider default for database unavailable is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Queue unavailable:** A framework or provider default for queue unavailable is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Provider 500:** A framework or provider default for provider 500 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Disk full:** A framework or provider default for disk full is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Slow dependency:** A framework or provider default for slow dependency is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Partial writes:** A framework or provider default for partial writes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Out-of-order delivery:** Parallel partitions or retries apply an older event after a newer one and regress state.
...
- Using sleeps for concurrency.
- Asserting implementation call order instead of outcomes.
...
- What exact invariant or user/system promise makes **Failure Testing** correct, and which component is authoritative for it?
...
- For **Database unavailable**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for database unavailable is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Queue unavailable**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for queue unavailable is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Provider 500**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for provider 500 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Slow dependency**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for slow dependency is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Failure Testing**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Queue unavailable:** Locate every implementation path for queue unavailable, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Provider 500:** Locate every implementation path for provider 500, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Disk full:** Locate every implementation path for disk full, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Slow dependency:** Locate every implementation path for slow dependency, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Partial writes:** Locate every implementation path for partial writes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Out-of-order delivery:** Deliver permutations, duplicates, gaps, and partition rebalances; verify deterministic final state.
...
- [092. Concurrency Testing](092-concurrency-testing.md) — layer: `cross-cutting`; profile: `testing`.
...
- [091. Test Data](091-test-data.md) — layer: `cross-cutting`; profile: `testing`.
...
**Paper metadata:** canonical subtopics: 12; layer: `cross-cutting`; domain profile: `testing`; verified through: `2026-08-17`.
