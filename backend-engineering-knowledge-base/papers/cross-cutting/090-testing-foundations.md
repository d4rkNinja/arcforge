...
**Unit tests**, **Integration tests**, **API tests**, **Database tests**, **Contract tests**, **End-to-end tests**, **Property-based tests**, **Fuzz tests**, **Mutation tests**, **Performance tests**, **Security tests**, **Chaos tests**.

### Boundary of the paper

This paper treats **Testing Foundations** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Unit tests:** Place each invariant at the lowest test layer that faithfully includes the mechanism that can violate it; use real databases/protocols for constraints, isolation, serialization, and provider contracts.
- **SHOULD — API tests:** Define the exact semantics of **API tests** within Testing Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Contract tests:** Place each invariant at the lowest test layer that faithfully includes the mechanism that can violate it; use real databases/protocols for constraints, isolation, serialization, and provider contracts.
- **SHOULD — Fuzz tests:** Define the exact semantics of **Fuzz tests** within Testing Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Performance tests:** Define the exact semantics of **Performance tests** within Testing Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Chaos tests:** Define the exact semantics of **Chaos tests** within Testing Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **API tests** within Testing Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for api tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for api tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Database tests

- **SHOULD — engineering rule:** Define the exact semantics of **Database tests** within Testing Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for database tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for database tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Property-based tests** within Testing Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for property-based tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for property-based tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Fuzz tests

- **SHOULD — engineering rule:** Define the exact semantics of **Fuzz tests** within Testing Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for fuzz tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for fuzz tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Mutation tests

- **SHOULD — engineering rule:** Define the exact semantics of **Mutation tests** within Testing Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for mutation tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for mutation tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Performance tests

- **SHOULD — engineering rule:** Define the exact semantics of **Performance tests** within Testing Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for performance tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for performance tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Security tests

- **MUST — engineering rule:** Define the exact semantics of **Security tests** within Testing Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for security tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for security tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.12. Chaos tests

- **SHOULD — engineering rule:** Define the exact semantics of **Chaos tests** within Testing Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for chaos tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for chaos tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST** — For **API tests**: Define the exact semantics of **API tests** within Testing Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Contract tests**: Place each invariant at the lowest test layer that faithfully includes the mechanism that can violate it; use real databases/protocols for constraints, isolation, serialization, and provider contracts.
- **MUST** — For **Fuzz tests**: Define the exact semantics of **Fuzz tests** within Testing Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Unit tests:** Map each production failure mode to at least one faithful test and identify which dependencies are real versus simulated.
- [ ] **API tests:** Locate every implementation path for api tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Contract tests:** Map each production failure mode to at least one faithful test and identify which dependencies are real versus simulated.
- [ ] **Fuzz tests:** Locate every implementation path for fuzz tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Performance tests:** Locate every implementation path for performance tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Chaos tests:** Locate every implementation path for chaos tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Unit tests:** Mocks confirm implementation assumptions rather than real behavior and regressions escape at integration boundaries.
- **API tests:** A framework or provider default for api tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Contract tests:** Mocks confirm implementation assumptions rather than real behavior and regressions escape at integration boundaries.
- **Property-based tests:** A framework or provider default for property-based tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Fuzz tests:** A framework or provider default for fuzz tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Performance tests:** A framework or provider default for performance tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Chaos tests:** A framework or provider default for chaos tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Unit tests**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Mocks confirm implementation assumptions rather than real behavior and regressions escape at integration boundaries.
- For **API tests**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for api tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Contract tests**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Mocks confirm implementation assumptions rather than real behavior and regressions escape at integration boundaries.
- For **Fuzz tests**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for fuzz tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Performance tests**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for performance tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] **API tests:** Locate every implementation path for api tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Contract tests:** Map each production failure mode to at least one faithful test and identify which dependencies are real versus simulated.
- [ ] **Property-based tests:** Locate every implementation path for property-based tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Fuzz tests:** Locate every implementation path for fuzz tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Performance tests:** Locate every implementation path for performance tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Chaos tests:** Locate every implementation path for chaos tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
