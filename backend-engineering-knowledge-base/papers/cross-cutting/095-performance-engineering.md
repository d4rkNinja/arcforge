...
**Garbage collection**, **Database queries**.
...
**CPU**, **Memory**, **Allocations**, **Network calls**, **Compression**, **Batching**, **Caching**, **Connection pooling**, **Profiling**, **Flame graphs**.

### Boundary of the paper

This paper treats **Performance Engineering** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Garbage collection:** Measure under representative load before changing code; correlate profiles with request classes, queueing, GC pauses, I/O, and tail latency. Preserve correctness and observability while optimizing.
- **SHOULD — Network calls:** Define the exact semantics of **Network calls** within Performance Engineering: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Batching:** Define the exact semantics of **Batching** within Performance Engineering: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Connection pooling:** Size pools from downstream capacity and instance count, bound acquisition time, instrument wait/usage, validate connections, and always release through structured cleanup/cancellation.
- **SHOULD — Flame graphs:** Measure under representative load before changing code; correlate profiles with request classes, queueing, GC pauses, I/O, and tail latency. Preserve correctness and observability while optimizing.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Database queries** within Performance Engineering: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for database queries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for database queries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Network calls

- **SHOULD — engineering rule:** Define the exact semantics of **Network calls** within Performance Engineering: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for network calls is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for network calls, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Serialization

- **SHOULD — engineering rule:** Define the exact semantics of **Serialization** within Performance Engineering: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **Production failure mode:** Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
- **Existing-codebase evidence:** Fuzz representative parsers, enforce pixel/duration/frame limits, kill hung workers, and reprocess after pipeline version changes.

### 7.9. Batching

- **SHOULD — engineering rule:** Define the exact semantics of **Batching** within Performance Engineering: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for batching is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for batching, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Caching

- **SHOULD — engineering rule:** Define the exact semantics of **Caching** within Performance Engineering: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.11. Connection pooling

- **SHOULD — engineering rule:** Size pools from downstream capacity and instance count, bound acquisition time, instrument wait/usage, validate connections, and always release through structured cleanup/cancellation.
- **Production failure mode:** Scale-out opens more connections than the database can serve, or leaked/slow operations make every request wait indefinitely.
- **Existing-codebase evidence:** Load-test at maximum replicas, inject slow queries/network resets, and assert bounded wait plus zero leaked checkouts.

### 7.12. Async processing

- **SHOULD — engineering rule:** Define the exact semantics of **Async processing** within Performance Engineering: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for async processing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for async processing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Performance Engineering path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `CPU`, `Garbage collection`, `Serialization`. |
...
| Privacy / retention | Classify data produced or touched by Performance Engineering; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Performance Engineering** before choosing framework or provider mechanisms.
...
- **MUST** — For **Garbage collection**: Measure under representative load before changing code; correlate profiles with request classes, queueing, GC pauses, I/O, and tail latency. Preserve correctness and observability while optimizing.
- **MUST** — For **Network calls**: Define the exact semantics of **Network calls** within Performance Engineering: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Batching**: Define the exact semantics of **Batching** within Performance Engineering: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Garbage collection:** Capture before/after profiles and end-to-end metrics with equivalent workloads and statistical confidence.
- [ ] **Network calls:** Locate every implementation path for network calls, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Batching:** Locate every implementation path for batching, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Connection pooling:** Load-test at maximum replicas, inject slow queries/network resets, and assert bounded wait plus zero leaked checkouts.
- [ ] **Flame graphs:** Capture before/after profiles and end-to-end metrics with equivalent workloads and statistical confidence.
...
- **Allocations:** A micro-optimization shifts pressure to memory, database, or network and worsens production tails.
- **Database queries:** A framework or provider default for database queries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Caching:** A framework or provider default for caching is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Async processing:** A framework or provider default for async processing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Flame graphs:** A micro-optimization shifts pressure to memory, database, or network and worsens production tails.
...
- What exact invariant or user/system promise makes **Performance Engineering** correct, and which component is authoritative for it?
...
- For **Garbage collection**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A micro-optimization shifts pressure to memory, database, or network and worsens production tails.
- For **Network calls**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for network calls is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Batching**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for batching is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Connection pooling**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Scale-out opens more connections than the database can serve, or leaked/slow operations make every request wait indefinitely.
...
- [ ] Map every entry point for **Performance Engineering**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **CPU:** Capture before/after profiles and end-to-end metrics with equivalent workloads and statistical confidence.
- [ ] **Allocations:** Capture before/after profiles and end-to-end metrics with equivalent workloads and statistical confidence.
- [ ] **Database queries:** Locate every implementation path for database queries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Caching:** Locate every implementation path for caching, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Async processing:** Locate every implementation path for async processing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Flame graphs:** Capture before/after profiles and end-to-end metrics with equivalent workloads and statistical confidence.
...
**Paper metadata:** canonical subtopics: 14; layer: `cross-cutting`; domain profile: `performance`; verified through: `2026-08-17`.
