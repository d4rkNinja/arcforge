...
**Output encoding**, **Least privilege**, **Default deny**, **Defense in depth**, **Secure defaults**, **Trust boundaries**, **Attack surface**, **Threat modeling**, **Secure failure**, **Sensitive-data handling**.

### Boundary of the paper

This paper treats **Security Fundamentals** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **MUST — Input validation:** Define the exact semantics of **Input validation** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Least privilege:** Define the exact semantics of **Least privilege** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Defense in depth:** Define the exact semantics of **Defense in depth** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Trust boundaries:** Define the exact semantics of **Trust boundaries** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Threat modeling:** Define the exact semantics of **Threat modeling** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Sensitive-data handling:** Classify fields and flows, collect the minimum, limit purpose and access, use irreversible anonymization only when re-identification risk is acceptably low, and propagate deletion/retention to derived systems and backups.
...
- **MUST — engineering rule:** Define the exact semantics of **Input validation** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for input validation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for input validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Output encoding

- **SHOULD — engineering rule:** Define the exact semantics of **Output encoding** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for output encoding is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for output encoding, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Least privilege

- **SHOULD — engineering rule:** Define the exact semantics of **Least privilege** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for least privilege is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for least privilege, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Defense in depth** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for defense in depth is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for defense in depth, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Secure defaults

- **SHOULD — engineering rule:** Define the exact semantics of **Secure defaults** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for secure defaults is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for secure defaults, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Trust boundaries

- **SHOULD — engineering rule:** Define the exact semantics of **Trust boundaries** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for trust boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for trust boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Attack surface

- **SHOULD — engineering rule:** Define the exact semantics of **Attack surface** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for attack surface is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for attack surface, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Threat modeling

- **SHOULD — engineering rule:** Define the exact semantics of **Threat modeling** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for threat modeling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for threat modeling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Secure failure

- **SHOULD — engineering rule:** Define the exact semantics of **Secure failure** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for secure failure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for secure failure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Sensitive-data handling
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Security Fundamentals path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Input validation`, `Least privilege`, `Secure defaults`. |
...
| Privacy / retention | Classify data produced or touched by Security Fundamentals; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Security Fundamentals** before choosing framework or provider mechanisms.
...
- **MUST** — For **Input validation**: Define the exact semantics of **Input validation** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Least privilege**: Define the exact semantics of **Least privilege** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Defense in depth**: Define the exact semantics of **Defense in depth** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Trust boundaries**: Define the exact semantics of **Trust boundaries** within Security Fundamentals: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Input validation:** Locate every implementation path for input validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Least privilege:** Locate every implementation path for least privilege, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Defense in depth:** Locate every implementation path for defense in depth, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Trust boundaries:** Locate every implementation path for trust boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Threat modeling:** Locate every implementation path for threat modeling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Sensitive-data handling:** Build a data-flow inventory and verify each copy's lawful purpose, encryption, access, retention, export, and deletion behavior.
...
- **Input validation:** A framework or provider default for input validation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Least privilege:** A framework or provider default for least privilege is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Default deny:** A newly added endpoint or malformed policy input becomes implicitly allowed.
- **Secure defaults:** A framework or provider default for secure defaults is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Attack surface:** A framework or provider default for attack surface is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Threat modeling:** A framework or provider default for threat modeling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Sensitive-data handling:** Sensitive data spreads into logs, caches, search, analytics, and backups where deletion and access controls are weaker.
...
- What exact invariant or user/system promise makes **Security Fundamentals** correct, and which component is authoritative for it?
...
- For **Input validation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for input validation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Least privilege**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for least privilege is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Defense in depth**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for defense in depth is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Trust boundaries**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for trust boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Threat modeling**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for threat modeling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Security Fundamentals**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Input validation:** Locate every implementation path for input validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Least privilege:** Locate every implementation path for least privilege, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Default deny:** Add tests for absent policy, unknown enum values, policy engine outage, stale cache, and new routes without annotations.
- [ ] **Secure defaults:** Locate every implementation path for secure defaults, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Attack surface:** Locate every implementation path for attack surface, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Threat modeling:** Locate every implementation path for threat modeling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Sensitive-data handling:** Build a data-flow inventory and verify each copy's lawful purpose, encryption, access, retention, export, and deletion behavior.
...
- [112. Internal Admin Operations](../systems/112-internal-admin-operations.md) — layer: `systems`; profile: `authorization`.
...
- [008. Authorization](../systems/008-authorization.md) — layer: `systems`; profile: `authorization`.
...
**Paper metadata:** canonical subtopics: 11; layer: `cross-cutting`; domain profile: `security`; verified through: `2026-08-17`.
