...
**User targeting**, **Tenant targeting**.

### State and lifecycle

**Default states**, **Cleanup**.

### Security, privacy, and abuse

**Boolean flags**, **Kill switches**, **Dependencies**, **Failure behavior**, **Flag auditing**.
...
This paper treats **Feature Flags** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Boolean flags:** Define the exact semantics of **Boolean flags** within Feature Flags: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — User targeting:** Give flags typed ownership, safe defaults, evaluation context, audit, expiry, dependency rules, and deterministic assignment. Separate release flags from durable configuration.
- **SHOULD — Kill switches:** Give flags typed ownership, safe defaults, evaluation context, audit, expiry, dependency rules, and deterministic assignment. Separate release flags from durable configuration.
- **SHOULD — Default states:** Define the exact semantics of **Default states** within Feature Flags: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Flag auditing:** Record real actor, effective actor, action, resource, tenant, timestamp, request/trace, policy context, outcome, and appropriately minimized change details in tamper-resistant storage.
...
|---|---|---|
| Boolean vs targeted/progressive flag | Boolean flags are simple; percentage/targeted rollout reduces blast radius but needs stable assignment, audit, and evaluation availability. | Keep defaults safe, decouple deploy from release, and remove stale flags. |
...
- **SHOULD — engineering rule:** Define the exact semantics of **Boolean flags** within Feature Flags: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for boolean flags is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for boolean flags, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Percentage rollout
...
- **SHOULD — engineering rule:** Define the exact semantics of **Dependencies** within Feature Flags: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for dependencies is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for dependencies, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Default states

- **SHOULD — engineering rule:** Define the exact semantics of **Default states** within Feature Flags: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for default states is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for default states, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Failure behavior

- **SHOULD — engineering rule:** Define the exact semantics of **Failure behavior** within Feature Flags: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for failure behavior is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for failure behavior, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST — engineering rule:** Define the exact semantics of **Cleanup** within Feature Flags: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Feature Flags path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Boolean flags`, `User targeting`, `Dependencies`. |
...
| Privacy / retention | Classify data produced or touched by Feature Flags; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Feature Flags** before choosing framework or provider mechanisms.
...
- **MUST** — For **Boolean flags**: Define the exact semantics of **Boolean flags** within Feature Flags: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **User targeting**: Give flags typed ownership, safe defaults, evaluation context, audit, expiry, dependency rules, and deterministic assignment. Separate release flags from durable configuration.
- **MUST** — For **Kill switches**: Give flags typed ownership, safe defaults, evaluation context, audit, expiry, dependency rules, and deterministic assignment. Separate release flags from durable configuration.
- **MUST** — For **Default states**: Define the exact semantics of **Default states** within Feature Flags: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **Boolean vs targeted/progressive flag** according to the stated trade-off: Keep defaults safe, decouple deploy from release, and remove stale flags.
...
- [ ] **Boolean flags:** Locate every implementation path for boolean flags, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **User targeting:** Test provider outage, missing/invalid values, rollout stability, rollback, authorization of changes, and cleanup.
- [ ] **Kill switches:** Test provider outage, missing/invalid values, rollout stability, rollback, authorization of changes, and cleanup.
- [ ] **Default states:** Locate every implementation path for default states, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Flag auditing:** Exercise successful/failed/admin/impersonated/bulk actions and verify atomicity, retention, search, export, and access controls.
...
- **Boolean flags:** A framework or provider default for boolean flags is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **User targeting:** A missing flag service changes behavior unsafely, cohorts flap, or stale dead flags permanently fork code paths.
- **Tenant targeting:** A missing filter, reused cache key, delayed job, or admin path reads or writes another tenant's data.
- **Dependencies:** A framework or provider default for dependencies is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Failure behavior:** A framework or provider default for failure behavior is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Flag auditing:** Security-sensitive changes cannot be attributed, audit data leaks secrets, or failed attempts vanish.
...
- What exact invariant or user/system promise makes **Feature Flags** correct, and which component is authoritative for it?
...
- For **Boolean flags**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for boolean flags is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **User targeting**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A missing flag service changes behavior unsafely, cohorts flap, or stale dead flags permanently fork code paths.
- For **Kill switches**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A missing flag service changes behavior unsafely, cohorts flap, or stale dead flags permanently fork code paths.
- For **Default states**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for default states is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Flag auditing**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Security-sensitive changes cannot be attributed, audit data leaks secrets, or failed attempts vanish.
...
- [ ] Map every entry point for **Feature Flags**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Boolean flags:** Locate every implementation path for boolean flags, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **User targeting:** Test provider outage, missing/invalid values, rollout stability, rollback, authorization of changes, and cleanup.
- [ ] **Tenant targeting:** Search for data-access methods that accept no tenant scope; run mutation and property tests that swap tenant identifiers at every boundary.
- [ ] **Dependencies:** Locate every implementation path for dependencies, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Failure behavior:** Locate every implementation path for failure behavior, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Flag auditing:** Exercise successful/failed/admin/impersonated/bulk actions and verify atomicity, retention, search, export, and access controls.
- [ ] **Rollback:** Perform a production-like rollback drill after generating new-version data and partially completed work.
