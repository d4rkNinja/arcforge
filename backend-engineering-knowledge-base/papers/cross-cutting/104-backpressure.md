...
**Backpressure** exists to bound the blast radius and resource cost of partial failure while preserving useful service. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Queue backpressure**, **Bounded queues**, **Retry interactions**.

### Operations and observability

**HTTP backpressure**, **Stream backpressure**, **Worker saturation**, **Producer throttling**, **Load shedding**, **Memory protection**.

### Boundary of the paper

This paper treats **Backpressure** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Queue backpressure:** Define the exact semantics of **Queue backpressure** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Stream backpressure:** Define the exact semantics of **Stream backpressure** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Worker saturation:** Define the exact semantics of **Worker saturation** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Bounded queues:** Define the exact semantics of **Bounded queues** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Load shedding:** Define the exact semantics of **Load shedding** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Memory protection:** Measure under representative load before changing code; correlate profiles with request classes, queueing, GC pauses, I/O, and tail latency. Preserve correctness and observability while optimizing.
...
### 7.1. Queue backpressure

- **SHOULD — engineering rule:** Define the exact semantics of **Queue backpressure** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for queue backpressure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for queue backpressure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. HTTP backpressure

- **SHOULD — engineering rule:** Define the exact semantics of **HTTP backpressure** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for http backpressure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for http backpressure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Stream backpressure

- **SHOULD — engineering rule:** Define the exact semantics of **Stream backpressure** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for stream backpressure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for stream backpressure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Worker saturation

- **SHOULD — engineering rule:** Define the exact semantics of **Worker saturation** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for worker saturation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for worker saturation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Producer throttling
...
- **SHOULD — engineering rule:** Define the exact semantics of **Bounded queues** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for bounded queues is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for bounded queues, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Load shedding

- **SHOULD — engineering rule:** Define the exact semantics of **Load shedding** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for load shedding is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for load shedding, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Retry interactions
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Backpressure path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Queue backpressure`, `Stream backpressure`, `Producer throttling`. |
...
| Privacy / retention | Classify data produced or touched by Backpressure; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Backpressure** before choosing framework or provider mechanisms.
...
- **MUST** — For **Queue backpressure**: Define the exact semantics of **Queue backpressure** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Stream backpressure**: Define the exact semantics of **Stream backpressure** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Worker saturation**: Define the exact semantics of **Worker saturation** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Bounded queues**: Define the exact semantics of **Bounded queues** within Backpressure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Queue backpressure:** Locate every implementation path for queue backpressure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Stream backpressure:** Locate every implementation path for stream backpressure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Worker saturation:** Locate every implementation path for worker saturation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Bounded queues:** Locate every implementation path for bounded queues, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Load shedding:** Locate every implementation path for load shedding, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Memory protection:** Capture before/after profiles and end-to-end metrics with equivalent workloads and statistical confidence.
...
- **Queue backpressure:** A framework or provider default for queue backpressure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **HTTP backpressure:** A framework or provider default for http backpressure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Worker saturation:** A framework or provider default for worker saturation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Producer throttling:** Data remains valid too long, expires simultaneously causing a stampede, or code assumes expired records are immediately deleted.
- **Bounded queues:** A framework or provider default for bounded queues is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Retry interactions:** Permanent failures loop forever, retries synchronize into a storm, or a timed-out successful operation is duplicated.
- **Memory protection:** A micro-optimization shifts pressure to memory, database, or network and worsens production tails.
...
- What exact invariant or user/system promise makes **Backpressure** correct, and which component is authoritative for it?
...
- For **Queue backpressure**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for queue backpressure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Stream backpressure**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for stream backpressure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Worker saturation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for worker saturation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Bounded queues**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for bounded queues is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Load shedding**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for load shedding is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Backpressure**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Queue backpressure:** Locate every implementation path for queue backpressure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **HTTP backpressure:** Locate every implementation path for http backpressure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Worker saturation:** Locate every implementation path for worker saturation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Producer throttling:** Test exact boundary times with controlled clocks, delayed cleanup, clock skew, and mass expiry.
- [ ] **Bounded queues:** Locate every implementation path for bounded queues, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Retry interactions:** Inject each error class and verify attempt count, schedule, deadline budget, duplicate behavior, and terminal routing.
- [ ] **Memory protection:** Capture before/after profiles and end-to-end metrics with equivalent workloads and statistical confidence.
...
- [050. Realtime Communication](../systems/050-realtime-communication.md) — layer: `systems`; profile: `api`.
...
**Paper metadata:** canonical subtopics: 9; layer: `cross-cutting`; domain profile: `resilience`; verified through: `2026-08-17`.
