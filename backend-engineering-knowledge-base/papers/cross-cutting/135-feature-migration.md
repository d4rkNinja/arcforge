...
**Old implementation**, **New implementation**, **Shadow traffic**, **Dual writes**, **Dual reads**, **Comparison**, **Gradual migration**, **Compatibility**.

### Boundary of the paper

This paper treats **Feature Migration** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Old implementation:** Define the exact semantics of **Old implementation** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Shadow traffic:** Assign a canonical truth, stable operation ID/version, ordering and conflict rules, compare outcomes, reconcile divergence, and define a finite exit criterion.
- **SHOULD — Dual reads:** Define the exact semantics of **Dual reads** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Comparison:** Define the exact semantics of **Comparison** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — Compatibility:** Define the exact semantics of **Compatibility** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.1. Old implementation

- **SHOULD — engineering rule:** Define the exact semantics of **Old implementation** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for old implementation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for old implementation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. New implementation

- **SHOULD — engineering rule:** Define the exact semantics of **New implementation** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for new implementation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for new implementation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Dual writes** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for dual writes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for dual writes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Dual reads

- **SHOULD — engineering rule:** Define the exact semantics of **Dual reads** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for dual reads is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for dual reads, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Comparison

- **SHOULD — engineering rule:** Define the exact semantics of **Comparison** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for comparison is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for comparison, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Gradual migration

- **SHOULD — engineering rule:** Define the exact semantics of **Gradual migration** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for gradual migration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for gradual migration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST — engineering rule:** Define the exact semantics of **Cleanup** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Compatibility** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for compatibility is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for compatibility, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Feature Migration path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Old implementation`, `Shadow traffic`, `Dual reads`. |
...
| Privacy / retention | Classify data produced or touched by Feature Migration; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Feature Migration** before choosing framework or provider mechanisms.
...
- **MUST** — For **Old implementation**: Define the exact semantics of **Old implementation** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Shadow traffic**: Assign a canonical truth, stable operation ID/version, ordering and conflict rules, compare outcomes, reconcile divergence, and define a finite exit criterion.
- **MUST** — For **Dual reads**: Define the exact semantics of **Dual reads** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Comparison**: Define the exact semantics of **Comparison** within Feature Migration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Old implementation:** Locate every implementation path for old implementation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Shadow traffic:** Inject failure on each side and verify repair, metrics, replay, and rollback without duplicate side effects.
- [ ] **Dual reads:** Locate every implementation path for dual reads, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Comparison:** Locate every implementation path for comparison, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Rollback:** Perform a production-like rollback drill after generating new-version data and partially completed work.
- [ ] **Compatibility:** Locate every implementation path for compatibility, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Old implementation:** A framework or provider default for old implementation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Shadow traffic:** Two stores diverge after partial failure and operators cannot tell which copy is correct.
- **Dual writes:** A framework or provider default for dual writes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Dual reads:** A framework or provider default for dual reads is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Gradual migration:** A framework or provider default for gradual migration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Compatibility:** A framework or provider default for compatibility is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Feature Migration** correct, and which component is authoritative for it?
...
- For **Old implementation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for old implementation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Shadow traffic**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Two stores diverge after partial failure and operators cannot tell which copy is correct.
- For **Dual reads**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for dual reads is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Comparison**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for comparison is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Feature Migration**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Old implementation:** Locate every implementation path for old implementation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Shadow traffic:** Inject failure on each side and verify repair, metrics, replay, and rollback without duplicate side effects.
- [ ] **Dual writes:** Locate every implementation path for dual writes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Dual reads:** Locate every implementation path for dual reads, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Gradual migration:** Locate every implementation path for gradual migration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [134. Zero-Downtime Changes](134-zero-downtime-changes.md) — layer: `cross-cutting`; profile: `migration`.
- [136. Legacy-System Integration](../systems/136-legacy-system-integration.md) — layer: `systems`; profile: `migration`.
- [068. Feature Flags](068-feature-flags.md) — layer: `cross-cutting`; profile: `security`.
...
- [072. Data Synchronization](../primitives/072-data-synchronization.md) — layer: `primitives`; profile: `migration`.
...
**Paper metadata:** canonical subtopics: 10; layer: `cross-cutting`; domain profile: `migration`; verified through: `2026-08-17`.
