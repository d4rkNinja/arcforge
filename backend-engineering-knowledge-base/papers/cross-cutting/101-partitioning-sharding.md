...
**Range partitioning**, **Directory-based partitioning**, **Resharding**, **Rebalancing**, **Hot shards**, **Cross-shard queries**, **Shard routing**.
...
This paper treats **Partitioning / Sharding** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Hash partitioning:** State the ordering scope, assign monotonic per-aggregate/version metadata where needed, and make consumers reject, buffer, or reconcile stale/out-of-order items.
- **MUST — Tenant partitioning:** Derive tenant context from an authenticated, authorized binding; propagate it explicitly through queries, caches, jobs, events, files, metrics, and audit records. Make unscoped access difficult or impossible.
- **SHOULD — Resharding:** Choose a key from access patterns, cardinality, locality, tenant size, growth, and rebalancing needs; design routing metadata and cross-shard operations before scale forces migration.
- **SHOULD — Rebalancing:** Choose a key from access patterns, cardinality, locality, tenant size, growth, and rebalancing needs; design routing metadata and cross-shard operations before scale forces migration.
- **SHOULD — Cross-shard queries:** Choose a key from access patterns, cardinality, locality, tenant size, growth, and rebalancing needs; design routing metadata and cross-shard operations before scale forces migration.
- **SHOULD — Shard routing:** Choose a key from access patterns, cardinality, locality, tenant size, growth, and rebalancing needs; design routing metadata and cross-shard operations before scale forces migration.
...
|---|---|---|
| Hash vs range partitioning | Hash balances uniform load but scatters range queries; range preserves locality but creates hot ranges. | Base the key on workload distribution and future resharding, not only current row counts. |
...
### 7.4. Directory-based partitioning
...
### 7.8. Cross-shard queries

- **SHOULD — engineering rule:** Choose a key from access patterns, cardinality, locality, tenant size, growth, and rebalancing needs; design routing metadata and cross-shard operations before scale forces migration.
- **Production failure mode:** Monotonic or celebrity keys overload one shard, cross-shard queries become the norm, or resharding blocks writes.
- **Existing-codebase evidence:** Replay skewed production-like distributions and rehearse split/move/failover with concurrent reads and writes.

### 7.9. Cross-shard transactions

- **MUST — engineering rule:** Choose a key from access patterns, cardinality, locality, tenant size, growth, and rebalancing needs; design routing metadata and cross-shard operations before scale forces migration.
- **Production failure mode:** Monotonic or celebrity keys overload one shard, cross-shard queries become the norm, or resharding blocks writes.
- **Existing-codebase evidence:** Replay skewed production-like distributions and rehearse split/move/failover with concurrent reads and writes.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Partitioning / Sharding path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Hash partitioning`, `Tenant partitioning`, `Resharding`. |
...
| Privacy / retention | Classify data produced or touched by Partitioning / Sharding; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Partitioning / Sharding** before choosing framework or provider mechanisms.
...
- **MUST** — For **Hash partitioning**: State the ordering scope, assign monotonic per-aggregate/version metadata where needed, and make consumers reject, buffer, or reconcile stale/out-of-order items.
- **MUST** — For **Tenant partitioning**: Derive tenant context from an authenticated, authorized binding; propagate it explicitly through queries, caches, jobs, events, files, metrics, and audit records. Make unscoped access difficult or impossible.
- **MUST** — For **Resharding**: Choose a key from access patterns, cardinality, locality, tenant size, growth, and rebalancing needs; design routing metadata and cross-shard operations before scale forces migration.
- **MUST** — For **Rebalancing**: Choose a key from access patterns, cardinality, locality, tenant size, growth, and rebalancing needs; design routing metadata and cross-shard operations before scale forces migration.
...
- **MAY** — Choose **Hash vs range partitioning** according to the stated trade-off: Base the key on workload distribution and future resharding, not only current row counts.
...
- [ ] **Hash partitioning:** Deliver permutations, duplicates, gaps, and partition rebalances; verify deterministic final state.
- [ ] **Tenant partitioning:** Search for data-access methods that accept no tenant scope; run mutation and property tests that swap tenant identifiers at every boundary.
- [ ] **Resharding:** Replay skewed production-like distributions and rehearse split/move/failover with concurrent reads and writes.
- [ ] **Rebalancing:** Replay skewed production-like distributions and rehearse split/move/failover with concurrent reads and writes.
- [ ] **Cross-shard queries:** Replay skewed production-like distributions and rehearse split/move/failover with concurrent reads and writes.
- [ ] **Shard routing:** Replay skewed production-like distributions and rehearse split/move/failover with concurrent reads and writes.
...
- **Hash partitioning:** Parallel partitions or retries apply an older event after a newer one and regress state.
- **Tenant partitioning:** A missing filter, reused cache key, delayed job, or admin path reads or writes another tenant's data.
- **Directory-based partitioning:** Parallel partitions or retries apply an older event after a newer one and regress state.
- **Resharding:** Monotonic or celebrity keys overload one shard, cross-shard queries become the norm, or resharding blocks writes.
- **Hot shards:** Monotonic or celebrity keys overload one shard, cross-shard queries become the norm, or resharding blocks writes.
- **Cross-shard transactions:** Monotonic or celebrity keys overload one shard, cross-shard queries become the norm, or resharding blocks writes.
- **Shard routing:** Monotonic or celebrity keys overload one shard, cross-shard queries become the norm, or resharding blocks writes.
...
- What exact invariant or user/system promise makes **Partitioning / Sharding** correct, and which component is authoritative for it?
...
- For **Hash partitioning**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Parallel partitions or retries apply an older event after a newer one and regress state.
- For **Tenant partitioning**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A missing filter, reused cache key, delayed job, or admin path reads or writes another tenant's data.
- For **Resharding**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Monotonic or celebrity keys overload one shard, cross-shard queries become the norm, or resharding blocks writes.
- For **Rebalancing**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Monotonic or celebrity keys overload one shard, cross-shard queries become the norm, or resharding blocks writes.
- For **Cross-shard queries**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Monotonic or celebrity keys overload one shard, cross-shard queries become the norm, or resharding blocks writes.
...
- [ ] Map every entry point for **Partitioning / Sharding**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Hash partitioning:** Deliver permutations, duplicates, gaps, and partition rebalances; verify deterministic final state.
- [ ] **Tenant partitioning:** Search for data-access methods that accept no tenant scope; run mutation and property tests that swap tenant identifiers at every boundary.
- [ ] **Directory-based partitioning:** Deliver permutations, duplicates, gaps, and partition rebalances; verify deterministic final state.
- [ ] **Resharding:** Replay skewed production-like distributions and rehearse split/move/failover with concurrent reads and writes.
- [ ] **Hot shards:** Replay skewed production-like distributions and rehearse split/move/failover with concurrent reads and writes.
- [ ] **Cross-shard transactions:** Replay skewed production-like distributions and rehearse split/move/failover with concurrent reads and writes.
- [ ] **Shard routing:** Replay skewed production-like distributions and rehearse split/move/failover with concurrent reads and writes.
...
**Paper metadata:** canonical subtopics: 10; layer: `cross-cutting`; domain profile: `transactions`; verified through: `2026-08-17`.
