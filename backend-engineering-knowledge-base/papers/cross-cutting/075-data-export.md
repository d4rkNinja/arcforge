...
**Data Export** exists to restore correct service and data after deletion, corruption, regional failure, or operator error within explicit recovery objectives. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**CSV**, **JSON**, **Streaming exports**, **Large exports**, **Background generation**, **Expiring downloads**.

### Boundary of the paper

This paper treats **Data Export** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — CSV:** Define the exact semantics of **CSV** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Streaming exports:** Define the exact semantics of **Streaming exports** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Background generation:** Define the exact semantics of **Background generation** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Temporary files:** Define the exact semantics of **Temporary files** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Permission checks:** Define authorization as `(principal, action, resource, context) -> decision + reason + policy version`. Enforce at the owning boundary and include tenant/resource state, not only route-level roles.
- **SHOULD — Export consistency:** Define the exact semantics of **Export consistency** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **CSV** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for csv is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for csv, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. JSON

- **SHOULD — engineering rule:** Define the exact semantics of **JSON** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.3. Streaming exports

- **SHOULD — engineering rule:** Define the exact semantics of **Streaming exports** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for streaming exports is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for streaming exports, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Large exports

- **SHOULD — engineering rule:** Define the exact semantics of **Large exports** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for large exports is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for large exports, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Background generation

- **SHOULD — engineering rule:** Define the exact semantics of **Background generation** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for background generation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for background generation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Temporary files

- **SHOULD — engineering rule:** Define the exact semantics of **Temporary files** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for temporary files is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for temporary files, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Expiring downloads

- **SHOULD — engineering rule:** Define the exact semantics of **Expiring downloads** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for expiring downloads is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for expiring downloads, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.10. Export consistency

- **SHOULD — engineering rule:** Define the exact semantics of **Export consistency** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for export consistency is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for export consistency, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Data Export path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `CSV`, `Streaming exports`, `Background generation`. |
...
| Privacy / retention | Classify data produced or touched by Data Export; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Data Export** before choosing framework or provider mechanisms.
...
- **MUST** — For **CSV**: Define the exact semantics of **CSV** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Streaming exports**: Define the exact semantics of **Streaming exports** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Background generation**: Define the exact semantics of **Background generation** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Temporary files**: Define the exact semantics of **Temporary files** within Data Export: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **CSV:** Locate every implementation path for csv, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Streaming exports:** Locate every implementation path for streaming exports, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Background generation:** Locate every implementation path for background generation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Temporary files:** Locate every implementation path for temporary files, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Permission checks:** Trace each read/write from externally supplied identifier to the final query/serializer and verify denial before data exposure or mutation.
- [ ] **Export consistency:** Locate every implementation path for export consistency, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **CSV:** A framework or provider default for csv is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Streaming exports:** A framework or provider default for streaming exports is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Large exports:** A framework or provider default for large exports is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Background generation:** A framework or provider default for background generation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Expiring downloads:** A framework or provider default for expiring downloads is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Sensitive-data filtering:** Injection, authorization bypass, unbounded scans, or query-plan explosion occurs when raw user expressions reach the database.
- **Export consistency:** A framework or provider default for export consistency is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Data Export** correct, and which component is authoritative for it?
...
- For **CSV**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for csv is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Streaming exports**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for streaming exports is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Background generation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for background generation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Temporary files**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for temporary files is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Permission checks**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: An authenticated caller accesses another object, hidden field, or state transition because lookup and authorization are separated or policy inputs are incomplete.
...
- [ ] Map every entry point for **Data Export**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **CSV:** Locate every implementation path for csv, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Streaming exports:** Locate every implementation path for streaming exports, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Large exports:** Locate every implementation path for large exports, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Background generation:** Locate every implementation path for background generation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Expiring downloads:** Locate every implementation path for expiring downloads, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Sensitive-data filtering:** Fuzz operators and nesting, verify mandatory predicates cannot be overridden, and benchmark worst allowed combinations.
- [ ] **Export consistency:** Locate every implementation path for export consistency, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [074. Data Import](074-data-import.md) — layer: `cross-cutting`; profile: `backup_dr`.
- [040. File Handling](../systems/040-file-handling.md) — layer: `systems`; profile: `data_model`.
- [119. Bulk Operations](../primitives/119-bulk-operations.md) — layer: `primitives`; profile: `async`.
- [031. Data Migrations & Backfills](../primitives/031-data-migrations-and-backfills.md) — layer: `primitives`; profile: `migration`.
...
- [132. Multi-Region Systems](132-multi-region-systems.md) — layer: `cross-cutting`; profile: `backup_dr`.
...
**Paper metadata:** canonical subtopics: 10; layer: `cross-cutting`; domain profile: `backup_dr`; verified through: `2026-08-17`.
