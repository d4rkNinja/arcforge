...
**Graceful degradation**, **Retry control**, **Timeout control**.

### Operations and observability

**Partial failure**, **Fallbacks**, **Bulkheads**, **Load shedding**, **Backpressure**, **Failure isolation**, **Dependency isolation**, **Brownout modes**, **Emergency switches**.

### Boundary of the paper

This paper treats **Resilience** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Partial failure:** Define the exact semantics of **Partial failure** within Resilience: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Fallbacks:** Define the exact semantics of **Fallbacks** within Resilience: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Load shedding:** Define the exact semantics of **Load shedding** within Resilience: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Dependency isolation:** Select isolation by anomaly analysis, not name familiarity. Account for database-specific semantics and retry complete transactions on serialization failure.
- **MUST — Timeout control:** Establish an end-to-end deadline, allocate smaller downstream budgets, propagate cancellation, and stop expensive or side-effecting work when the result is no longer useful unless explicitly designed otherwise.
- **SHOULD — Emergency switches:** Define the exact semantics of **Emergency switches** within Resilience: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Partial failure** within Resilience: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Fallbacks** within Resilience: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for fallbacks is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for fallbacks, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Bulkheads

- **SHOULD — engineering rule:** Define the exact semantics of **Bulkheads** within Resilience: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for bulkheads is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for bulkheads, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Load shedding

- **SHOULD — engineering rule:** Define the exact semantics of **Load shedding** within Resilience: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Backpressure** within Resilience: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Brownout modes** within Resilience: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for brownout modes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for brownout modes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.12. Emergency switches

- **SHOULD — engineering rule:** Define the exact semantics of **Emergency switches** within Resilience: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for emergency switches is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for emergency switches, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST** — For **Partial failure**: Define the exact semantics of **Partial failure** within Resilience: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Fallbacks**: Define the exact semantics of **Fallbacks** within Resilience: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Load shedding**: Define the exact semantics of **Load shedding** within Resilience: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Partial failure:** Locate every implementation path for partial failure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Fallbacks:** Locate every implementation path for fallbacks, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Dependency isolation:** Construct concurrent schedules that would violate the invariant and verify the chosen isolation/locking rejects one participant.
- [ ] **Timeout control:** Inject slow dependencies at each hop and verify cancellation reaches database, network calls, streams, and workers.
- [ ] **Emergency switches:** Locate every implementation path for emergency switches, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Partial failure:** A framework or provider default for partial failure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Fallbacks:** A framework or provider default for fallbacks is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Load shedding:** A framework or provider default for load shedding is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Failure isolation:** Lost updates, write skew, or phantoms violate invariants despite all statements being inside a transaction.
- **Dependency isolation:** Lost updates, write skew, or phantoms violate invariants despite all statements being inside a transaction.
- **Timeout control:** Requests wait indefinitely, downstream work continues after callers leave, or nested timeouts exceed the original budget.
- **Emergency switches:** A framework or provider default for emergency switches is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Partial failure**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for partial failure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Fallbacks**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for fallbacks is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Dependency isolation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Lost updates, write skew, or phantoms violate invariants despite all statements being inside a transaction.
- For **Timeout control**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Requests wait indefinitely, downstream work continues after callers leave, or nested timeouts exceed the original budget.
...
- [ ] **Fallbacks:** Locate every implementation path for fallbacks, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Load shedding:** Locate every implementation path for load shedding, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Failure isolation:** Construct concurrent schedules that would violate the invariant and verify the chosen isolation/locking rejects one participant.
- [ ] **Dependency isolation:** Construct concurrent schedules that would violate the invariant and verify the chosen isolation/locking rejects one participant.
- [ ] **Timeout control:** Inject slow dependencies at each hop and verify cancellation reaches database, network calls, streams, and workers.
- [ ] **Emergency switches:** Locate every implementation path for emergency switches, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
**Paper metadata:** canonical subtopics: 12; layer: `cross-cutting`; domain profile: `resilience`; verified through: `2026-08-17`.
