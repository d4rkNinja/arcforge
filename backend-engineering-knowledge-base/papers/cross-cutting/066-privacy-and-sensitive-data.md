...
**Retention**, **Backups containing deleted data**.

### Security, privacy, and abuse

**PII classification**, **Sensitive fields**, **Data minimization**, **Data masking**, **Anonymization**, **Pseudonymization**, **Deletion**, **Export**, **Consent**, **Access controls**, **Logs containing PII**.

### Boundary of the paper

This paper treats **Privacy & Sensitive Data** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Privacy & Sensitive Data path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `PII classification`, `Data masking`, `Retention`. |
...
| Privacy / retention | Classify data produced or touched by Privacy & Sensitive Data; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Privacy & Sensitive Data** before choosing framework or provider mechanisms.
...
- **MUST** — For **PII classification**: Classify fields and flows, collect the minimum, limit purpose and access, use irreversible anonymization only when re-identification risk is acceptably low, and propagate deletion/retention to derived systems and backups.
- **MUST** — For **Data minimization**: Classify fields and flows, collect the minimum, limit purpose and access, use irreversible anonymization only when re-identification risk is acceptably low, and propagate deletion/retention to derived systems and backups.
- **MUST** — For **Pseudonymization**: Classify fields and flows, collect the minimum, limit purpose and access, use irreversible anonymization only when re-identification risk is acceptably low, and propagate deletion/retention to derived systems and backups.
- **MUST** — For **Deletion**: Define the exact semantics of **Deletion** within Privacy & Sensitive Data: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **PII classification:** Build a data-flow inventory and verify each copy's lawful purpose, encryption, access, retention, export, and deletion behavior.
- [ ] **Data minimization:** Build a data-flow inventory and verify each copy's lawful purpose, encryption, access, retention, export, and deletion behavior.
- [ ] **Pseudonymization:** Build a data-flow inventory and verify each copy's lawful purpose, encryption, access, retention, export, and deletion behavior.
...
- [ ] **Access controls:** Locate every implementation path for access controls, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Backups containing deleted data:** Locate every implementation path for backups containing deleted data, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **PII classification:** Sensitive data spreads into logs, caches, search, analytics, and backups where deletion and access controls are weaker.
- **Data minimization:** Sensitive data spreads into logs, caches, search, analytics, and backups where deletion and access controls are weaker.
...
- **Export:** A framework or provider default for export is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Access controls:** A framework or provider default for access controls is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Backups containing deleted data:** A framework or provider default for backups containing deleted data is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Privacy & Sensitive Data** correct, and which component is authoritative for it?
...
- For **PII classification**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Sensitive data spreads into logs, caches, search, analytics, and backups where deletion and access controls are weaker.
- For **Data minimization**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Sensitive data spreads into logs, caches, search, analytics, and backups where deletion and access controls are weaker.
- For **Pseudonymization**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Sensitive data spreads into logs, caches, search, analytics, and backups where deletion and access controls are weaker.
- For **Deletion**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for deletion is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Access controls**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for access controls is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Privacy & Sensitive Data**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Export:** Locate every implementation path for export, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Access controls:** Locate every implementation path for access controls, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Backups containing deleted data:** Locate every implementation path for backups containing deleted data, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [142. AI Memory](../systems/142-ai-memory.md) — layer: `systems`; profile: `ai`.
- [143. RAG Infrastructure](../systems/143-rag-infrastructure.md) — layer: `systems`; profile: `ai`.
