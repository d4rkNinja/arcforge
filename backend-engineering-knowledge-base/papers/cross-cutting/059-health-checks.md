...
**Health Checks** exists to make system behavior, causality, saturation, and security-relevant actions reconstructable without leaking sensitive data. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Liveness**, **Readiness**, **Dependency checks**, **Shallow vs deep health checks**, **Cache health**, **Queue health**, **External dependency health**, **Health-check overload**.

### Boundary of the paper

This paper treats **Health Checks** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Liveness:** Liveness should detect irrecoverable process failure without depending on remote services; readiness should reflect ability to serve the routed workload; startup probes protect slow initialization.
- **SHOULD — Startup probes:** Liveness should detect irrecoverable process failure without depending on remote services; readiness should reflect ability to serve the routed workload; startup probes protect slow initialization.
- **SHOULD — Shallow vs deep health checks:** Liveness should detect irrecoverable process failure without depending on remote services; readiness should reflect ability to serve the routed workload; startup probes protect slow initialization.
- **SHOULD — Database health:** Define the exact semantics of **Database health** within Health Checks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Queue health:** Define the exact semantics of **Queue health** within Health Checks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Health-check overload:** Define the exact semantics of **Health-check overload** within Health Checks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|---|
| Liveness vs readiness | Liveness answers whether restart may help; readiness answers whether traffic is safe. Deep dependency checks in liveness cause restart cascades. | Keep liveness shallow and readiness tied only to required correctness dependencies. |
...
### 7.4. Dependency checks

- **SHOULD — engineering rule:** Define the exact semantics of **Dependency checks** within Health Checks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for dependency checks is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for dependency checks, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Shallow vs deep health checks
...
- **SHOULD — engineering rule:** Define the exact semantics of **Database health** within Health Checks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for database health is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for database health, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Cache health

- **SHOULD — engineering rule:** Define the exact semantics of **Cache health** within Health Checks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for cache health is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for cache health, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Queue health

- **SHOULD — engineering rule:** Define the exact semantics of **Queue health** within Health Checks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for queue health is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for queue health, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. External dependency health

- **SHOULD — engineering rule:** Define the exact semantics of **External dependency health** within Health Checks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for external dependency health is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for external dependency health, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Health-check overload

- **SHOULD — engineering rule:** Define the exact semantics of **Health-check overload** within Health Checks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for health-check overload is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for health-check overload, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Health Checks path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Liveness`, `Startup probes`, `Shallow vs deep health checks`. |
...
| Privacy / retention | Classify data produced or touched by Health Checks; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Health Checks** before choosing framework or provider mechanisms.
...
- **MUST** — For **Liveness**: Liveness should detect irrecoverable process failure without depending on remote services; readiness should reflect ability to serve the routed workload; startup probes protect slow initialization.
- **MUST** — For **Startup probes**: Liveness should detect irrecoverable process failure without depending on remote services; readiness should reflect ability to serve the routed workload; startup probes protect slow initialization.
- **MUST** — For **Shallow vs deep health checks**: Liveness should detect irrecoverable process failure without depending on remote services; readiness should reflect ability to serve the routed workload; startup probes protect slow initialization.
- **MUST** — For **Database health**: Define the exact semantics of **Database health** within Health Checks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **Liveness vs readiness** according to the stated trade-off: Keep liveness shallow and readiness tied only to required correctness dependencies.
...
- [ ] **Liveness:** Fail each dependency separately and verify probe response, routing, restart behavior, and probe cost under fleet scale.
- [ ] **Startup probes:** Fail each dependency separately and verify probe response, routing, restart behavior, and probe cost under fleet scale.
- [ ] **Shallow vs deep health checks:** Fail each dependency separately and verify probe response, routing, restart behavior, and probe cost under fleet scale.
- [ ] **Database health:** Locate every implementation path for database health, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Queue health:** Locate every implementation path for queue health, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Health-check overload:** Locate every implementation path for health-check overload, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Liveness:** A dependency outage triggers restart storms, or a process receives traffic before initialization and migrations are complete.
- **Startup probes:** A dependency outage triggers restart storms, or a process receives traffic before initialization and migrations are complete.
- **Dependency checks:** A framework or provider default for dependency checks is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Shallow vs deep health checks:** A dependency outage triggers restart storms, or a process receives traffic before initialization and migrations are complete.
- **Cache health:** A framework or provider default for cache health is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **External dependency health:** A framework or provider default for external dependency health is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Health-check overload:** A framework or provider default for health-check overload is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Health Checks** correct, and which component is authoritative for it?
...
- For **Liveness**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A dependency outage triggers restart storms, or a process receives traffic before initialization and migrations are complete.
- For **Startup probes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A dependency outage triggers restart storms, or a process receives traffic before initialization and migrations are complete.
- For **Shallow vs deep health checks**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A dependency outage triggers restart storms, or a process receives traffic before initialization and migrations are complete.
- For **Database health**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for database health is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Queue health**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for queue health is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Health Checks**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Liveness:** Fail each dependency separately and verify probe response, routing, restart behavior, and probe cost under fleet scale.
- [ ] **Startup probes:** Fail each dependency separately and verify probe response, routing, restart behavior, and probe cost under fleet scale.
- [ ] **Dependency checks:** Locate every implementation path for dependency checks, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Shallow vs deep health checks:** Fail each dependency separately and verify probe response, routing, restart behavior, and probe cost under fleet scale.
- [ ] **Cache health:** Locate every implementation path for cache health, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **External dependency health:** Locate every implementation path for external dependency health, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Health-check overload:** Locate every implementation path for health-check overload, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
