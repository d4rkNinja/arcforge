...
**Fencing tokens**, **Lock ownership**.

### State and lifecycle

**Expiration**, **Failure recovery**.
...
**Redis locks**, **Coordination services**, **Lease locks**, **Lock renewal**, **Split-brain safety**.

### Boundary of the paper

This paper treats **Distributed Locks** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
|---|---|---|
| Database lock vs coordination-service/Redis lease | Database locks align with data writes; external leases coordinate broader work but create stale-holder risk. | Prefer atomic database predicates when protecting data; use leases with fencing for distributed work. |
...
- **SHOULD — engineering rule:** Define the exact semantics of **Coordination services** within Distributed Locks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for coordination services is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for coordination services, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.8. Split-brain safety

- **SHOULD — engineering rule:** Define the exact semantics of **Split-brain safety** within Distributed Locks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for split-brain safety is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for split-brain safety, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Lock ownership

- **MUST — engineering rule:** Define the exact semantics of **Lock ownership** within Distributed Locks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for lock ownership is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for lock ownership, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Failure recovery

- **MUST — engineering rule:** Define the exact semantics of **Failure recovery** within Distributed Locks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Distributed Locks path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Redis locks`, `Coordination services`, `Lock renewal`. |
...
| Privacy / retention | Classify data produced or touched by Distributed Locks; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Distributed Locks** before choosing framework or provider mechanisms.
...
- **MAY** — Choose **Database lock vs coordination-service/Redis lease** according to the stated trade-off: Prefer atomic database predicates when protecting data; use leases with fencing for distributed work.
...
- **Redis locks:** A framework or provider default for redis locks is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Coordination services:** A framework or provider default for coordination services is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Lease locks:** A paused or partitioned former owner resumes after expiry and performs a stale write despite a new owner holding the lock.
- **Lock renewal:** A framework or provider default for lock renewal is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Fencing tokens:** A paused or partitioned former owner resumes after expiry and performs a stale write despite a new owner holding the lock.
- **Lock ownership:** A framework or provider default for lock ownership is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Distributed Locks** correct, and which component is authoritative for it?
...
- [ ] Map every entry point for **Distributed Locks**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Redis locks:** Locate every implementation path for redis locks, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Coordination services:** Locate every implementation path for coordination services, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Lease locks:** Pause the first holder past expiry, acquire with a second holder, then prove the resource rejects the first holder's later operation.
- [ ] **Lock renewal:** Locate every implementation path for lock renewal, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Fencing tokens:** Pause the first holder past expiry, acquire with a second holder, then prove the resource rejects the first holder's later operation.
- [ ] **Lock ownership:** Locate every implementation path for lock ownership, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Failure recovery:** Locate every implementation path for failure recovery, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
