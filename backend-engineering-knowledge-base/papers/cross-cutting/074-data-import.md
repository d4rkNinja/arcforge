...
**Data Import** exists to restore correct service and data after deletion, corruption, regional failure, or operator error within explicit recovery objectives. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Idempotent import**, **Async import**, **Retry**.

### Operations and observability

**CSV**, **JSON**, **Bulk imports**, **Mapping**, **Preview**, **Partial failures**, **Duplicate handling**, **Progress**, **Rollback**.

### Boundary of the paper

This paper treats **Data Import** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — CSV:** Define the exact semantics of **CSV** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Validation:** Define the exact semantics of **Validation** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Preview:** Define the exact semantics of **Preview** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Idempotent import:** Define the exact semantics of **Idempotent import** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Progress:** Define the exact semantics of **Progress** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Huge files:** Enforce bounded sizes at the earliest layer and again where decompression, decoding, or aggregation can expand data. Stream rather than buffer where feasible.
...
- **SHOULD — engineering rule:** Define the exact semantics of **CSV** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **JSON** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Bulk imports** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for bulk imports is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for bulk imports, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Validation

- **MUST — engineering rule:** Define the exact semantics of **Validation** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Mapping** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for mapping is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for mapping, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Preview

- **SHOULD — engineering rule:** Define the exact semantics of **Preview** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for preview is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for preview, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Partial failures

- **SHOULD — engineering rule:** Define the exact semantics of **Partial failures** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Duplicate handling** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for duplicate handling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for duplicate handling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Idempotent import

- **MUST — engineering rule:** Define the exact semantics of **Idempotent import** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for idempotent import is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for idempotent import, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Async import

- **SHOULD — engineering rule:** Define the exact semantics of **Async import** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for async import is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for async import, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Progress

- **SHOULD — engineering rule:** Define the exact semantics of **Progress** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Data Import path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `CSV`, `Validation`, `Partial failures`. |
...
| Privacy / retention | Classify data produced or touched by Data Import; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Data Import** before choosing framework or provider mechanisms.
...
- **MUST** — For **CSV**: Define the exact semantics of **CSV** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Validation**: Define the exact semantics of **Validation** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Preview**: Define the exact semantics of **Preview** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Idempotent import**: Define the exact semantics of **Idempotent import** within Data Import: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Preview:** Locate every implementation path for preview, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Idempotent import:** Locate every implementation path for idempotent import, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Progress:** Locate every implementation path for progress, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Huge files:** Test chunked transfer, missing length, compressed expansion, multipart overhead, and gateway/application limit mismatches.
...
- **Bulk imports:** A framework or provider default for bulk imports is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Mapping:** A framework or provider default for mapping is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Async import:** A framework or provider default for async import is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Retry:** Permanent failures loop forever, retries synchronize into a storm, or a timed-out successful operation is duplicated.
- **Huge files:** An attacker or legitimate large request exhausts memory, disk, CPU, connection slots, or downstream limits before application validation runs.
...
- What exact invariant or user/system promise makes **Data Import** correct, and which component is authoritative for it?
...
- For **Validation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for validation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Preview**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for preview is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Idempotent import**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for idempotent import is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Progress**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for progress is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Data Import**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **CSV:** Locate every implementation path for csv, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Bulk imports:** Locate every implementation path for bulk imports, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Mapping:** Locate every implementation path for mapping, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Async import:** Locate every implementation path for async import, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Retry:** Inject each error class and verify attempt count, schedule, deadline budget, duplicate behavior, and terminal routing.
- [ ] **Huge files:** Test chunked transfer, missing length, compressed expansion, multipart overhead, and gateway/application limit mismatches.
...
- [075. Data Export](075-data-export.md) — layer: `cross-cutting`; profile: `backup_dr`.
...
**Paper metadata:** canonical subtopics: 14; layer: `cross-cutting`; domain profile: `backup_dr`; verified through: `2026-08-17`.
