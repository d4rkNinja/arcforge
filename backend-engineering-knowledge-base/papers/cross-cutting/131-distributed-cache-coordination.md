---
paper_number: 131
title: "Distributed Cache Coordination"
layer: cross-cutting
...
**Invalidation messages**, **Cache versions**.

### Concurrency and distributed behavior

**Generational keys**, **Distributed refresh**, **Stale-while-revalidate**, **Stampede protection**, **Multi-region cache behavior**.

### Boundary of the paper

This paper treats **Distributed Cache Coordination** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **MUST — Invalidation messages:** Define the exact semantics of **Invalidation messages** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Cache versions:** Define the exact semantics of **Cache versions** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Generational keys:** Define the exact semantics of **Generational keys** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Stale-while-revalidate:** Define the exact semantics of **Stale-while-revalidate** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Stampede protection:** Use request coalescing/singleflight, bounded locking with fencing where needed, jittered expiry, negative caching, admission control, and precomputation for known hot objects.
- **MAY — Multi-region cache behavior:** Define the exact semantics of **Multi-region cache behavior** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.1. Invalidation messages

- **MUST — engineering rule:** Define the exact semantics of **Invalidation messages** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for invalidation messages is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for invalidation messages, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Cache versions

- **SHOULD — engineering rule:** Define the exact semantics of **Cache versions** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for cache versions is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for cache versions, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Generational keys

- **SHOULD — engineering rule:** Define the exact semantics of **Generational keys** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for generational keys is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for generational keys, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Distributed refresh

- **SHOULD — engineering rule:** Define the exact semantics of **Distributed refresh** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for distributed refresh is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for distributed refresh, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Stale-while-revalidate

- **SHOULD — engineering rule:** Define the exact semantics of **Stale-while-revalidate** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for stale-while-revalidate is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for stale-while-revalidate, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.7. Multi-region cache behavior

- **MAY — engineering rule:** Define the exact semantics of **Multi-region cache behavior** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for multi-region cache behavior is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for multi-region cache behavior, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST** — For **Invalidation messages**: Define the exact semantics of **Invalidation messages** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Cache versions**: Define the exact semantics of **Cache versions** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Generational keys**: Define the exact semantics of **Generational keys** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Stale-while-revalidate**: Define the exact semantics of **Stale-while-revalidate** within Distributed Cache Coordination: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Invalidation messages:** Locate every implementation path for invalidation messages, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Cache versions:** Locate every implementation path for cache versions, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Generational keys:** Locate every implementation path for generational keys, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Stale-while-revalidate:** Locate every implementation path for stale-while-revalidate, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Stampede protection:** Load-test synchronized expiry and high-cardinality misses while monitoring source QPS and waiter limits.
- [ ] **Multi-region cache behavior:** Locate every implementation path for multi-region cache behavior, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Invalidation messages:** A framework or provider default for invalidation messages is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Cache versions:** A framework or provider default for cache versions is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Generational keys:** A framework or provider default for generational keys is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Distributed refresh:** A framework or provider default for distributed refresh is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Stale-while-revalidate:** A framework or provider default for stale-while-revalidate is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Stampede protection:** A popular miss fans out to the source and causes cascading failure; malicious distinct misses defeat the cache.
- **Multi-region cache behavior:** A framework or provider default for multi-region cache behavior is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Invalidation messages**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for invalidation messages is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Cache versions**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for cache versions is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Generational keys**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for generational keys is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Stale-while-revalidate**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for stale-while-revalidate is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Stampede protection**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A popular miss fans out to the source and causes cascading failure; malicious distinct misses defeat the cache.
...
- [ ] **Cache versions:** Locate every implementation path for cache versions, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Generational keys:** Locate every implementation path for generational keys, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Distributed refresh:** Locate every implementation path for distributed refresh, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Stale-while-revalidate:** Locate every implementation path for stale-while-revalidate, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Stampede protection:** Load-test synchronized expiry and high-cardinality misses while monitoring source QPS and waiter limits.
- [ ] **Multi-region cache behavior:** Locate every implementation path for multi-region cache behavior, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [038. Rate Limiting](038-rate-limiting.md) — layer: `cross-cutting`; profile: `cache`.
...
- [125. Cleanup Jobs](125-cleanup-jobs.md) — layer: `cross-cutting`; profile: `data_ops`.
...
**Paper metadata:** canonical subtopics: 7; layer: `cross-cutting`; domain profile: `cache`; verified through: `2026-08-17`.
