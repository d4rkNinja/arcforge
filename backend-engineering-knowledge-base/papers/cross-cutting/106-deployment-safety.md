...
**Rolling deployment**, **Blue/green**, **Canary**, **Rollback**, **Health gating**, **Deployment validation**.

### Testing and evolution

**Progressive rollout**, **Feature flags**, **Database compatibility**, **Mixed-version compatibility**.

### Boundary of the paper

This paper treats **Deployment Safety** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Rolling deployment:** Define a compatibility window in which old and new readers/writers coexist. Make changes additive first, preserve unknown fields where required, and test both deployment orders.
- **SHOULD — Canary:** Define the exact semantics of **Canary** within Deployment Safety: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — Feature flags:** Give flags typed ownership, safe defaults, evaluation context, audit, expiry, dependency rules, and deterministic assignment. Separate release flags from durable configuration.
- **SHOULD — Mixed-version compatibility:** Define a compatibility window in which old and new readers/writers coexist. Make changes additive first, preserve unknown fields where required, and test both deployment orders.
- **MUST — Deployment validation:** Define the exact semantics of **Deployment validation** within Deployment Safety: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Blue/green** within Deployment Safety: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for blue/green is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for blue/green, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Canary

- **SHOULD — engineering rule:** Define the exact semantics of **Canary** within Deployment Safety: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for canary is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for canary, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Progressive rollout

- **SHOULD — engineering rule:** Define the exact semantics of **Progressive rollout** within Deployment Safety: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for progressive rollout is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for progressive rollout, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Database compatibility** within Deployment Safety: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.8. Mixed-version compatibility
...
- **SHOULD — engineering rule:** Define the exact semantics of **Health gating** within Deployment Safety: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for health gating is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for health gating, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Deployment validation

- **MUST — engineering rule:** Define the exact semantics of **Deployment validation** within Deployment Safety: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for deployment validation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for deployment validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST** — For **Canary**: Define the exact semantics of **Canary** within Deployment Safety: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Rolling deployment:** Run an N/N+1 matrix for requests, stored data, events, queues, caches, and rollback.
- [ ] **Canary:** Locate every implementation path for canary, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Rollback:** Perform a production-like rollback drill after generating new-version data and partially completed work.
- [ ] **Feature flags:** Test provider outage, missing/invalid values, rollout stability, rollback, authorization of changes, and cleanup.
- [ ] **Mixed-version compatibility:** Run an N/N+1 matrix for requests, stored data, events, queues, caches, and rollback.
- [ ] **Deployment validation:** Locate every implementation path for deployment validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Rolling deployment:** A new writer emits data an old reader cannot parse, or rollback code cannot read records created during the failed release.
- **Canary:** A framework or provider default for canary is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Progressive rollout:** A framework or provider default for progressive rollout is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Rollback:** Code rolls back while schema/data/side effects do not, creating a second outage or corruption.
- **Database compatibility:** A framework or provider default for database compatibility is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Health gating:** A framework or provider default for health gating is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Deployment validation:** A framework or provider default for deployment validation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Rolling deployment**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A new writer emits data an old reader cannot parse, or rollback code cannot read records created during the failed release.
- For **Canary**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for canary is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Feature flags**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A missing flag service changes behavior unsafely, cohorts flap, or stale dead flags permanently fork code paths.
- For **Mixed-version compatibility**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A new writer emits data an old reader cannot parse, or rollback code cannot read records created during the failed release.
...
- [ ] **Rolling deployment:** Run an N/N+1 matrix for requests, stored data, events, queues, caches, and rollback.
- [ ] **Canary:** Locate every implementation path for canary, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Progressive rollout:** Locate every implementation path for progressive rollout, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Rollback:** Perform a production-like rollback drill after generating new-version data and partially completed work.
- [ ] **Database compatibility:** Locate every implementation path for database compatibility, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Health gating:** Locate every implementation path for health gating, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Deployment validation:** Locate every implementation path for deployment validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
