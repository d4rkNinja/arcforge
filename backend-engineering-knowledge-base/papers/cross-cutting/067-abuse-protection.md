...
**Brute force**, **Spam**, **Bot traffic**, **API abuse**, **CAPTCHA**, **Device/IP reputation**, **Abuse detection**, **Abuse logging**.
...
This paper treats **Abuse Protection** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Brute force:** Layer per-account, credential, device, IP/network, tenant, and global controls; normalize identifiers before counting; avoid revealing existence; collect signals with privacy limits and provide recovery from false positives.
- **SHOULD — Enumeration:** Layer per-account, credential, device, IP/network, tenant, and global controls; normalize identifiers before counting; avoid revealing existence; collect signals with privacy limits and provide recovery from false positives.
- **SHOULD — Scraping:** Layer per-account, credential, device, IP/network, tenant, and global controls; normalize identifiers before counting; avoid revealing existence; collect signals with privacy limits and provide recovery from false positives.
- **SHOULD — Resource exhaustion:** Define the exact semantics of **Resource exhaustion** within Abuse Protection: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Device/IP reputation:** Define the exact semantics of **Device/IP reputation** within Abuse Protection: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Abuse logging:** Layer per-account, credential, device, IP/network, tenant, and global controls; normalize identifiers before counting; avoid revealing existence; collect signals with privacy limits and provide recovery from false positives.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Spam** within Abuse Protection: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for spam is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for spam, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.8. Resource exhaustion

- **SHOULD — engineering rule:** Define the exact semantics of **Resource exhaustion** within Abuse Protection: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for resource exhaustion is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for resource exhaustion, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **CAPTCHA** within Abuse Protection: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for captcha is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for captcha, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Device/IP reputation

- **SHOULD — engineering rule:** Define the exact semantics of **Device/IP reputation** within Abuse Protection: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for device/ip reputation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for device/ip reputation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.12. Abuse detection
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Abuse Protection path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Brute force`, `Spam`, `API abuse`. |
...
| Privacy / retention | Classify data produced or touched by Abuse Protection; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Abuse Protection** before choosing framework or provider mechanisms.
...
- **MUST** — For **Brute force**: Layer per-account, credential, device, IP/network, tenant, and global controls; normalize identifiers before counting; avoid revealing existence; collect signals with privacy limits and provide recovery from false positives.
- **MUST** — For **Enumeration**: Layer per-account, credential, device, IP/network, tenant, and global controls; normalize identifiers before counting; avoid revealing existence; collect signals with privacy limits and provide recovery from false positives.
- **MUST** — For **Scraping**: Layer per-account, credential, device, IP/network, tenant, and global controls; normalize identifiers before counting; avoid revealing existence; collect signals with privacy limits and provide recovery from false positives.
- **MUST** — For **Resource exhaustion**: Define the exact semantics of **Resource exhaustion** within Abuse Protection: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Brute force:** Replay distributed attack patterns and compare status, body, timing, notification, and recovery behavior for existing/non-existing targets.
- [ ] **Enumeration:** Replay distributed attack patterns and compare status, body, timing, notification, and recovery behavior for existing/non-existing targets.
- [ ] **Scraping:** Replay distributed attack patterns and compare status, body, timing, notification, and recovery behavior for existing/non-existing targets.
- [ ] **Resource exhaustion:** Locate every implementation path for resource exhaustion, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Device/IP reputation:** Locate every implementation path for device/ip reputation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Abuse logging:** Replay distributed attack patterns and compare status, body, timing, notification, and recovery behavior for existing/non-existing targets.
...
- **Brute force:** Attackers distribute attempts across keys, weaponize lockouts, or infer valid accounts from timing and error differences.
- **Enumeration:** Attackers distribute attempts across keys, weaponize lockouts, or infer valid accounts from timing and error differences.
- **Bot traffic:** Attackers distribute attempts across keys, weaponize lockouts, or infer valid accounts from timing and error differences.
- **API abuse:** Attackers distribute attempts across keys, weaponize lockouts, or infer valid accounts from timing and error differences.
- **Rate limits:** Window-boundary bursts, racey counters, NAT collateral damage, or fail-open behavior allow abuse; fail-closed can create an outage.
- **Device/IP reputation:** A framework or provider default for device/ip reputation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Abuse logging:** Attackers distribute attempts across keys, weaponize lockouts, or infer valid accounts from timing and error differences.
...
- What exact invariant or user/system promise makes **Abuse Protection** correct, and which component is authoritative for it?
...
- For **Brute force**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Attackers distribute attempts across keys, weaponize lockouts, or infer valid accounts from timing and error differences.
- For **Enumeration**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Attackers distribute attempts across keys, weaponize lockouts, or infer valid accounts from timing and error differences.
- For **Scraping**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Attackers distribute attempts across keys, weaponize lockouts, or infer valid accounts from timing and error differences.
- For **Resource exhaustion**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for resource exhaustion is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Device/IP reputation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for device/ip reputation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Abuse Protection**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Brute force:** Replay distributed attack patterns and compare status, body, timing, notification, and recovery behavior for existing/non-existing targets.
- [ ] **Enumeration:** Replay distributed attack patterns and compare status, body, timing, notification, and recovery behavior for existing/non-existing targets.
- [ ] **Bot traffic:** Replay distributed attack patterns and compare status, body, timing, notification, and recovery behavior for existing/non-existing targets.
- [ ] **API abuse:** Replay distributed attack patterns and compare status, body, timing, notification, and recovery behavior for existing/non-existing targets.
...
**Paper metadata:** canonical subtopics: 13; layer: `cross-cutting`; domain profile: `security`; verified through: `2026-08-17`.
