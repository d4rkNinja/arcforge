...
**Concurrency Testing** exists to produce evidence that observable behavior and invariants survive realistic inputs, concurrency, failures, and change. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Deadlocks**, **Idempotency races**.

### Testing and evolution

**Race-condition tests**, **Duplicate request tests**, **Simultaneous updates**.

### Boundary of the paper

This paper treats **Concurrency Testing** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Race-condition tests:** Use barriers/hooks to force the critical interleaving, repeat across real database isolation, and assert the invariant rather than timing or one response.
- **SHOULD — Duplicate request tests:** Define the exact semantics of **Duplicate request tests** within Concurrency Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Simultaneous deletes:** Use barriers/hooks to force the critical interleaving, repeat across real database isolation, and assert the invariant rather than timing or one response.
- **MUST — Lock contention:** Use barriers/hooks to force the critical interleaving, repeat across real database isolation, and assert the invariant rather than timing or one response.
- **MUST — Idempotency races:** Use barriers/hooks to force the critical interleaving, repeat across real database isolation, and assert the invariant rather than timing or one response.
- **MUST — Transaction conflicts:** Define the exact semantics of **Transaction conflicts** within Concurrency Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.2. Duplicate request tests

- **SHOULD — engineering rule:** Define the exact semantics of **Duplicate request tests** within Concurrency Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for duplicate request tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for duplicate request tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.7. Idempotency races

- **MUST — engineering rule:** Use barriers/hooks to force the critical interleaving, repeat across real database isolation, and assert the invariant rather than timing or one response.
- **Production failure mode:** A probabilistic sleep-based test passes while the actual race remains reachable.
- **Existing-codebase evidence:** Instrument the exact read/check/write or lease boundary and run deterministic competing operations.

### 7.8. Transaction conflicts

- **MUST — engineering rule:** Define the exact semantics of **Transaction conflicts** within Concurrency Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for transaction conflicts is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for transaction conflicts, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Concurrency Testing path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Race-condition tests`, `Simultaneous updates`, `Lock contention`. |
...
| Privacy / retention | Classify data produced or touched by Concurrency Testing; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Concurrency Testing** before choosing framework or provider mechanisms.
...
- **MUST** — For **Race-condition tests**: Use barriers/hooks to force the critical interleaving, repeat across real database isolation, and assert the invariant rather than timing or one response.
- **MUST** — For **Duplicate request tests**: Define the exact semantics of **Duplicate request tests** within Concurrency Testing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Simultaneous deletes**: Use barriers/hooks to force the critical interleaving, repeat across real database isolation, and assert the invariant rather than timing or one response.
- **MUST** — For **Lock contention**: Use barriers/hooks to force the critical interleaving, repeat across real database isolation, and assert the invariant rather than timing or one response.
...
- [ ] **Race-condition tests:** Instrument the exact read/check/write or lease boundary and run deterministic competing operations.
- [ ] **Duplicate request tests:** Locate every implementation path for duplicate request tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Simultaneous deletes:** Instrument the exact read/check/write or lease boundary and run deterministic competing operations.
- [ ] **Lock contention:** Instrument the exact read/check/write or lease boundary and run deterministic competing operations.
- [ ] **Idempotency races:** Instrument the exact read/check/write or lease boundary and run deterministic competing operations.
- [ ] **Transaction conflicts:** Locate every implementation path for transaction conflicts, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Race-condition tests:** A probabilistic sleep-based test passes while the actual race remains reachable.
- **Duplicate request tests:** A framework or provider default for duplicate request tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Simultaneous updates:** A probabilistic sleep-based test passes while the actual race remains reachable.
- **Lock contention:** A probabilistic sleep-based test passes while the actual race remains reachable.
- **Deadlocks:** Workers hang, retry only the failed statement, or create a synchronized retry storm.
- **Idempotency races:** A probabilistic sleep-based test passes while the actual race remains reachable.
- **Transaction conflicts:** A framework or provider default for transaction conflicts is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Concurrency Testing** correct, and which component is authoritative for it?
...
- For **Race-condition tests**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A probabilistic sleep-based test passes while the actual race remains reachable.
- For **Duplicate request tests**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for duplicate request tests is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Simultaneous deletes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A probabilistic sleep-based test passes while the actual race remains reachable.
- For **Lock contention**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A probabilistic sleep-based test passes while the actual race remains reachable.
- For **Idempotency races**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A probabilistic sleep-based test passes while the actual race remains reachable.
...
- [ ] Map every entry point for **Concurrency Testing**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Race-condition tests:** Instrument the exact read/check/write or lease boundary and run deterministic competing operations.
- [ ] **Duplicate request tests:** Locate every implementation path for duplicate request tests, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Simultaneous updates:** Instrument the exact read/check/write or lease boundary and run deterministic competing operations.
- [ ] **Lock contention:** Instrument the exact read/check/write or lease boundary and run deterministic competing operations.
...
**Paper metadata:** canonical subtopics: 8; layer: `cross-cutting`; domain profile: `testing`; verified through: `2026-08-17`.
