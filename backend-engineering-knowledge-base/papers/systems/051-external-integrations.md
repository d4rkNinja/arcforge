...
**External Integrations** exists to bound the blast radius and resource cost of partial failure while preserving useful service. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**API clients**, **Provider errors**, **Version changes**.

### Concurrency and distributed behavior

**Timeouts**, **Partial synchronization**.

### Operations and observability

**Provider rate limits**, **Retries**, **Data mapping**, **Reconciliation**, **Provider outages**, **Fallback providers**, **Circuit breakers**.

### Boundary of the paper

This paper treats **External Integrations** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — engineering rule:** Define the exact semantics of **API clients** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for api clients is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for api clients, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Authentication

- **MUST — engineering rule:** Define the exact semantics of **Authentication** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Credentials** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for credentials is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for credentials, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Token refresh

- **SHOULD — engineering rule:** Define the exact semantics of **Token refresh** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for token refresh is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for token refresh, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Provider rate limits
...
- **SHOULD — engineering rule:** Define the exact semantics of **Retries** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Version changes** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for version changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for version changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Data mapping

- **SHOULD — engineering rule:** Define the exact semantics of **Data mapping** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for data mapping is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for data mapping, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Sync state

- **SHOULD — engineering rule:** Define the exact semantics of **Sync state** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for sync state is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for sync state, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.12. Partial synchronization

- **SHOULD — engineering rule:** Define the exact semantics of **Partial synchronization** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for partial synchronization is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for partial synchronization, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. Reconciliation

- **SHOULD — engineering rule:** Define the exact semantics of **Reconciliation** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.14. Provider outages

- **SHOULD — engineering rule:** Classify provider failures, set deadlines and bulkheads, respect rate limits, preserve request identity for safe retry/reconciliation, and define whether fallback changes semantics, data residency, or user consent.
- **Production failure mode:** The system retries unsafe requests, sends duplicates to two providers, or silently degrades to a provider with incompatible behavior.
- **Existing-codebase evidence:** Inject DNS/TLS/timeout/429/5xx/partial-success cases and reconcile provider state after ambiguous outcomes.

### 7.15. Fallback providers

- **MAY — engineering rule:** Classify provider failures, set deadlines and bulkheads, respect rate limits, preserve request identity for safe retry/reconciliation, and define whether fallback changes semantics, data residency, or user consent.
- **Production failure mode:** The system retries unsafe requests, sends duplicates to two providers, or silently degrades to a provider with incompatible behavior.
- **Existing-codebase evidence:** Inject DNS/TLS/timeout/429/5xx/partial-success cases and reconcile provider state after ambiguous outcomes.

### 7.16. Circuit breakers

- **SHOULD — engineering rule:** Define the exact semantics of **Circuit breakers** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for circuit breakers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for circuit breakers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every External Integrations path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `API clients`, `Provider rate limits`, `Version changes`. |
...
| Privacy / retention | Classify data produced or touched by External Integrations; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **External Integrations** before choosing framework or provider mechanisms.
...
- **MUST** — For **API clients**: Define the exact semantics of **API clients** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Token refresh**: Define the exact semantics of **Token refresh** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Retries**: Define the exact semantics of **Retries** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Data mapping**: Define the exact semantics of **Data mapping** within External Integrations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **API clients:** Locate every implementation path for api clients, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Token refresh:** Locate every implementation path for token refresh, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Data mapping:** Locate every implementation path for data mapping, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Circuit breakers:** Locate every implementation path for circuit breakers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **API clients:** A framework or provider default for api clients is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Credentials:** A framework or provider default for credentials is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Timeouts:** Requests wait indefinitely, downstream work continues after callers leave, or nested timeouts exceed the original budget.
- **Version changes:** A framework or provider default for version changes is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Sync state:** A framework or provider default for sync state is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Circuit breakers:** A framework or provider default for circuit breakers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **External Integrations** correct, and which component is authoritative for it?
...
- For **API clients**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for api clients is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Token refresh**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for token refresh is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Data mapping**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for data mapping is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **External Integrations**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **API clients:** Locate every implementation path for api clients, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Credentials:** Locate every implementation path for credentials, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Timeouts:** Inject slow dependencies at each hop and verify cancellation reaches database, network calls, streams, and workers.
- [ ] **Version changes:** Locate every implementation path for version changes, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Sync state:** Locate every implementation path for sync state, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Reconciliation:** Locate every implementation path for reconciliation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Circuit breakers:** Locate every implementation path for circuit breakers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [054. Circuit Breakers](../primitives/054-circuit-breakers.md) — layer: `primitives`; profile: `resilience`.
...
**Paper metadata:** canonical subtopics: 16; layer: `systems`; domain profile: `resilience`; verified through: `2026-08-17`.
