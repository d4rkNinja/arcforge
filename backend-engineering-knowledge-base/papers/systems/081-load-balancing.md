...
**Load Balancing** exists to make process startup, steady-state execution, and termination deterministic across environments and orchestrators. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Round robin**, **Least connections**, **Health-aware routing**, **Layer 4**, **Layer 7**, **Regional routing**, **Failover**, **Connection draining**.

### Boundary of the paper

This paper treats **Load Balancing** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Round robin:** Define the exact semantics of **Round robin** within Load Balancing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Consistent hashing:** Use well-reviewed libraries and current algorithms; define key purpose, scope, version, generation, storage, rotation, revocation, nonce uniqueness, and authenticated context.
- **SHOULD — Sticky sessions:** Choose routing from connection duration, state locality, health, skew, and failure behavior. Ensure draining and retries do not duplicate unsafe work.
- **SHOULD — Layer 4:** Define the exact semantics of **Layer 4** within Load Balancing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Regional routing:** Define the exact semantics of **Regional routing** within Load Balancing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Connection draining:** On termination, mark unready/stop admission, drain connections, stop leasing new jobs, finish or safely abandon in-flight work, flush bounded telemetry, close resources in dependency order, and exit before the platform kill deadline.
...
|---|---|---|
| Stateless balancing vs affinity/consistent hashing | Round robin is simple for homogeneous stateless work; affinity improves cache/state locality but creates skew and failover movement. | Keep services stateless where possible; use affinity only for an explicit locality need. |
...
- **SHOULD — engineering rule:** Define the exact semantics of **Round robin** within Load Balancing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for round robin is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for round robin, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Least connections

- **SHOULD — engineering rule:** Define the exact semantics of **Least connections** within Load Balancing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for least connections is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for least connections, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.4. Health-aware routing

- **SHOULD — engineering rule:** Define the exact semantics of **Health-aware routing** within Load Balancing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for health-aware routing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for health-aware routing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Layer 4** within Load Balancing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for layer 4 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for layer 4, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Layer 7

- **SHOULD — engineering rule:** Define the exact semantics of **Layer 7** within Load Balancing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for layer 7 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for layer 7, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Regional routing

- **SHOULD — engineering rule:** Define the exact semantics of **Regional routing** within Load Balancing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for regional routing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for regional routing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Failover

- **SHOULD — engineering rule:** Define the exact semantics of **Failover** within Load Balancing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Load Balancing path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Round robin`, `Consistent hashing`, `Sticky sessions`. |
...
| Privacy / retention | Classify data produced or touched by Load Balancing; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Load Balancing** before choosing framework or provider mechanisms.
...
- **MUST** — For **Round robin**: Define the exact semantics of **Round robin** within Load Balancing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Consistent hashing**: Use well-reviewed libraries and current algorithms; define key purpose, scope, version, generation, storage, rotation, revocation, nonce uniqueness, and authenticated context.
- **MUST** — For **Sticky sessions**: Choose routing from connection duration, state locality, health, skew, and failure behavior. Ensure draining and retries do not duplicate unsafe work.
- **MUST** — For **Layer 4**: Define the exact semantics of **Layer 4** within Load Balancing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **Stateless balancing vs affinity/consistent hashing** according to the stated trade-off: Keep services stateless where possible; use affinity only for an explicit locality need.
...
- [ ] **Round robin:** Locate every implementation path for round robin, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Consistent hashing:** Review against a cryptographic design document and test key rotation, corruption, wrong context/key, entropy failure, and legacy ciphertext.
- [ ] **Sticky sessions:** Remove/add instances under load, create skewed keys, and verify drain, failover, and session/state behavior.
- [ ] **Layer 4:** Locate every implementation path for layer 4, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Regional routing:** Locate every implementation path for regional routing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Round robin:** A framework or provider default for round robin is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Consistent hashing:** Custom crypto, nonce reuse, weak randomness, key/algorithm confusion, or unauthenticated encryption destroys the intended guarantee.
- **Health-aware routing:** A framework or provider default for health-aware routing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Sticky sessions:** Sticky keys create hot instances, unhealthy connections persist, or retry routes a completed write to another backend.
- **Layer 7:** A framework or provider default for layer 7 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Failover:** A framework or provider default for failover is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Load Balancing** correct, and which component is authoritative for it?
...
- For **Round robin**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for round robin is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Consistent hashing**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Custom crypto, nonce reuse, weak randomness, key/algorithm confusion, or unauthenticated encryption destroys the intended guarantee.
- For **Sticky sessions**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Sticky keys create hot instances, unhealthy connections persist, or retry routes a completed write to another backend.
- For **Layer 4**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for layer 4 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Regional routing**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for regional routing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Load Balancing**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Round robin:** Locate every implementation path for round robin, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Consistent hashing:** Review against a cryptographic design document and test key rotation, corruption, wrong context/key, entropy failure, and legacy ciphertext.
- [ ] **Health-aware routing:** Locate every implementation path for health-aware routing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Sticky sessions:** Remove/add instances under load, create skewed keys, and verify drain, failover, and session/state behavior.
- [ ] **Layer 7:** Locate every implementation path for layer 7, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [097. High Availability](../cross-cutting/097-high-availability.md) — layer: `cross-cutting`; profile: `backup_dr`.
...
**Paper metadata:** canonical subtopics: 10; layer: `systems`; domain profile: `runtime`; verified through: `2026-08-17`.
