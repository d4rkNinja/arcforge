...
**Media Processing** exists to encode domain facts and invariants so valid states are easy to represent and invalid states are difficult to persist. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Image resize**, **Image compression**, **Thumbnail generation**, **Video processing**, **Audio processing**, **Failed processing**, **Reprocessing**, **Original-file preservation**, **Metadata stripping**.
...
This paper treats **Media Processing** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Image resize:** Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **SHOULD — Format conversion:** Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **SHOULD — Video processing:** Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **SHOULD — Processing states:** Define the exact semantics of **Processing states** within Media Processing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Reprocessing:** Define the exact semantics of **Reprocessing** within Media Processing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Metadata stripping:** Classify metadata fields, strip sensitive location/device/author information by default for public derivatives, and retain only explicitly required metadata in controlled storage.
...
### 7.7. Async processing

- **SHOULD — engineering rule:** Define the exact semantics of **Async processing** within Media Processing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Processing states** within Media Processing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for processing states is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for processing states, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Failed processing

- **SHOULD — engineering rule:** Define the exact semantics of **Failed processing** within Media Processing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for failed processing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for failed processing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Reprocessing

- **SHOULD — engineering rule:** Define the exact semantics of **Reprocessing** within Media Processing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for reprocessing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for reprocessing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Original-file preservation
...
### 7.12. Metadata stripping

- **SHOULD — engineering rule:** Classify metadata fields, strip sensitive location/device/author information by default for public derivatives, and retain only explicitly required metadata in controlled storage.
- **Production failure mode:** EXIF or container metadata leaks PII despite the visible content being safe.
- **Existing-codebase evidence:** Upload samples carrying GPS, thumbnails, comments, embedded profiles, and multiple metadata blocks; inspect served derivatives.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Media Processing path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Image resize`, `Thumbnail generation`, `Async processing`. |
...
| Privacy / retention | Classify data produced or touched by Media Processing; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Media Processing** before choosing framework or provider mechanisms.
...
- **MUST** — For **Image resize**: Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **MUST** — For **Format conversion**: Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **MUST** — For **Video processing**: Treat decoders and transcoders as untrusted-code surfaces with strict CPU/memory/time/output bounds; preserve the original according to policy and make derivatives versioned/idempotent.
- **MUST** — For **Processing states**: Define the exact semantics of **Processing states** within Media Processing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Image resize:** Fuzz representative parsers, enforce pixel/duration/frame limits, kill hung workers, and reprocess after pipeline version changes.
- [ ] **Format conversion:** Fuzz representative parsers, enforce pixel/duration/frame limits, kill hung workers, and reprocess after pipeline version changes.
- [ ] **Video processing:** Fuzz representative parsers, enforce pixel/duration/frame limits, kill hung workers, and reprocess after pipeline version changes.
- [ ] **Processing states:** Locate every implementation path for processing states, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Reprocessing:** Locate every implementation path for reprocessing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Metadata stripping:** Upload samples carrying GPS, thumbnails, comments, embedded profiles, and multiple metadata blocks; inspect served derivatives.
...
- **Image resize:** Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
- **Format conversion:** Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
- **Video processing:** Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
...
- **Processing states:** A framework or provider default for processing states is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Reprocessing:** A framework or provider default for reprocessing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Metadata stripping:** EXIF or container metadata leaks PII despite the visible content being safe.
...
- What exact invariant or user/system promise makes **Media Processing** correct, and which component is authoritative for it?
...
- For **Image resize**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
- For **Format conversion**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
- For **Video processing**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Malformed media exploits a parser, decompression expands uncontrollably, or retries create conflicting derivatives.
- For **Processing states**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for processing states is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Reprocessing**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for reprocessing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Media Processing**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [040. File Handling](040-file-handling.md) — layer: `systems`; profile: `data_model`.
...
**Paper metadata:** canonical subtopics: 12; layer: `systems`; domain profile: `data_model`; verified through: `2026-08-17`.
