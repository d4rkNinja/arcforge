...
**Registration**, **Endpoint verification**, **Signing**, **Signature verification**, **Timestamp validation**, **Replay protection**, **Disablement**, **Dead-lettering**.

### Concurrency and distributed behavior

**Event selection**, **Delivery retries**, **Backoff**, **Duplicate delivery**, **Out-of-order delivery**, **Timeout**, **Delivery logs**.
...
This paper treats **Webhooks** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Registration:** Define the exact semantics of **Registration** within Webhooks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Signing:** Verify the signature over the exact raw body and canonical signed components before parsing; use constant-time comparison, bounded timestamp skew, endpoint-scoped secrets, rotation overlap, and event deduplication.
...
- **SHOULD — Duplicate delivery:** Define the exact semantics of **Duplicate delivery** within Webhooks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Delivery logs:** Model each delivery attempt with event ID, subscription version, endpoint, timeout, response class, next attempt, and terminal state. Bound retries and disable persistently failing or unsafe endpoints.
- **SHOULD — Webhook versioning:** Model each delivery attempt with event ID, subscription version, endpoint, timeout, response class, next attempt, and terminal state. Bound retries and disable persistently failing or unsafe endpoints.
...
|---|---|---|
| Provider-specific webhook contract vs normalized internal event | Provider-specific handling preserves detail; normalization reduces downstream coupling but can erase semantics. | Verify and persist the raw envelope, then map to a versioned internal event with provenance. |
...
- **SHOULD — engineering rule:** Define the exact semantics of **Registration** within Webhooks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for registration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for registration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Event selection

- **SHOULD — engineering rule:** Define the exact semantics of **Event selection** within Webhooks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for event selection is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for event selection, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Endpoint verification

- **MUST — engineering rule:** Define the exact semantics of **Endpoint verification** within Webhooks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for endpoint verification is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for endpoint verification, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Signing

- **SHOULD — engineering rule:** Verify the signature over the exact raw body and canonical signed components before parsing; use constant-time comparison, bounded timestamp skew, endpoint-scoped secrets, rotation overlap, and event deduplication.
...
### 7.9. Delivery retries

- **SHOULD — engineering rule:** Define the exact semantics of **Delivery retries** within Webhooks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for delivery retries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for delivery retries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Duplicate delivery** within Webhooks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Webhooks path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Registration`, `Signature verification`, `Delivery retries`. |
...
| Privacy / retention | Classify data produced or touched by Webhooks; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Webhooks** before choosing framework or provider mechanisms.
...
- **MUST** — For **Registration**: Define the exact semantics of **Registration** within Webhooks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Signing**: Verify the signature over the exact raw body and canonical signed components before parsing; use constant-time comparison, bounded timestamp skew, endpoint-scoped secrets, rotation overlap, and event deduplication.
- **MUST** — For **Replay protection**: Verify the signature over the exact raw body and canonical signed components before parsing; use constant-time comparison, bounded timestamp skew, endpoint-scoped secrets, rotation overlap, and event deduplication.
- **MUST** — For **Duplicate delivery**: Define the exact semantics of **Duplicate delivery** within Webhooks: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **Provider-specific webhook contract vs normalized internal event** according to the stated trade-off: Verify and persist the raw envelope, then map to a versioned internal event with provenance.
...
- [ ] **Registration:** Locate every implementation path for registration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Signing:** Test whitespace/encoding changes, duplicate headers, old/future timestamps, both rotation keys, large bodies, and repeated event IDs.
...
- [ ] **Duplicate delivery:** Locate every implementation path for duplicate delivery, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Delivery logs:** Simulate timeout, 2xx-after-timeout, 4xx, 429, 5xx, DNS/TLS failure, endpoint changes, and replay.
- [ ] **Webhook versioning:** Simulate timeout, 2xx-after-timeout, 4xx, 429, 5xx, DNS/TLS failure, endpoint changes, and replay.
...
- **Registration:** A framework or provider default for registration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Signing:** Middleware changes bytes, old signed requests replay, or a rotated secret causes either outage or indefinite acceptance.
- **Timestamp validation:** Middleware changes bytes, old signed requests replay, or a rotated secret causes either outage or indefinite acceptance.
- **Delivery retries:** A framework or provider default for delivery retries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Out-of-order delivery:** Parallel partitions or retries apply an older event after a newer one and regress state.
- **Delivery logs:** Slow consumers exhaust workers, duplicate/out-of-order deliveries corrupt state, or one endpoint causes unbounded retry cost.
- **Webhook versioning:** Slow consumers exhaust workers, duplicate/out-of-order deliveries corrupt state, or one endpoint causes unbounded retry cost.
...
- What exact invariant or user/system promise makes **Webhooks** correct, and which component is authoritative for it?
...
- For **Registration**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for registration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Signing**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Middleware changes bytes, old signed requests replay, or a rotated secret causes either outage or indefinite acceptance.
- For **Replay protection**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Middleware changes bytes, old signed requests replay, or a rotated secret causes either outage or indefinite acceptance.
- For **Duplicate delivery**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for duplicate delivery is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Delivery logs**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Slow consumers exhaust workers, duplicate/out-of-order deliveries corrupt state, or one endpoint causes unbounded retry cost.
...
- [ ] Map every entry point for **Webhooks**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Registration:** Locate every implementation path for registration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Signing:** Test whitespace/encoding changes, duplicate headers, old/future timestamps, both rotation keys, large bodies, and repeated event IDs.
- [ ] **Timestamp validation:** Test whitespace/encoding changes, duplicate headers, old/future timestamps, both rotation keys, large bodies, and repeated event IDs.
- [ ] **Delivery retries:** Locate every implementation path for delivery retries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
**Paper metadata:** canonical subtopics: 17; layer: `systems`; domain profile: `api`; verified through: `2026-08-17`.
