...
**Queues**, **Topics**, **Pub/sub**, **Streams**, **Consumer groups**, **Producers**, **Consumers**, **Message acknowledgment**, **Redelivery**, **Ordering**, **Partitioning**, **Delivery guarantees**, **At-most-once**, **At-least-once**, **Exactly-once claims**, **Dead-letter queues**, **Poison messages**, **Backpressure**, **Consumer lag**.

### Boundary of the paper

This paper treats **Messaging / Queues** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Queues:** Define the exact semantics of **Queues** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Consumer groups:** Define the exact semantics of **Consumer groups** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Message acknowledgment:** Acknowledge only after durable effects commit. Size/extend the lease or visibility timeout for real processing duration and make redelivery safe.
- **SHOULD — Delivery guarantees:** Define the exact semantics of **Delivery guarantees** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Exactly-once claims:** Define the exact semantics of **Exactly-once claims** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Consumer lag:** Define the exact semantics of **Consumer lag** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|---|
| Queue vs pub/sub/stream | Queues distribute work to consumers; pub/sub fans out; streams retain ordered logs for replay at operational cost. | Choose from ownership, replay, ordering, retention, and consumer independence. |
...
- **SHOULD — engineering rule:** Define the exact semantics of **Queues** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for queues is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for queues, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Topics

- **SHOULD — engineering rule:** Define the exact semantics of **Topics** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for topics is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for topics, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Pub/sub

- **SHOULD — engineering rule:** Define the exact semantics of **Pub/sub** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for pub/sub is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for pub/sub, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Streams

- **SHOULD — engineering rule:** Define the exact semantics of **Streams** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for streams is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for streams, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Consumer groups

- **SHOULD — engineering rule:** Define the exact semantics of **Consumer groups** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for consumer groups is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for consumer groups, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Producers

- **SHOULD — engineering rule:** Define the exact semantics of **Producers** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for producers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for producers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Consumers

- **SHOULD — engineering rule:** Define the exact semantics of **Consumers** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.8. Message acknowledgment

- **SHOULD — engineering rule:** Acknowledge only after durable effects commit. Size/extend the lease or visibility timeout for real processing duration and make redelivery safe.
- **Production failure mode:** Worker crashes after ack lose work; crashes before ack duplicate effects; lease expiry creates concurrent processing.
- **Existing-codebase evidence:** Terminate workers before/after commit and pause them past the lease to verify duplicate containment.
...
### 7.12. Delivery guarantees

- **SHOULD — engineering rule:** Define the exact semantics of **Delivery guarantees** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for delivery guarantees is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for delivery guarantees, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. At-most-once

- **SHOULD — engineering rule:** Define the exact semantics of **At-most-once** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for at-most-once is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for at-most-once, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.14. At-least-once

- **SHOULD — engineering rule:** Define the exact semantics of **At-least-once** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for at-least-once is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for at-least-once, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.15. Exactly-once claims

- **SHOULD — engineering rule:** Define the exact semantics of **Exactly-once claims** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for exactly-once claims is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for exactly-once claims, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.16. Dead-letter queues
...
- **SHOULD — engineering rule:** Define the exact semantics of **Backpressure** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for backpressure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for backpressure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.19. Consumer lag

- **SHOULD — engineering rule:** Define the exact semantics of **Consumer lag** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for consumer lag is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for consumer lag, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Messaging / Queues path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Queues`, `Consumer groups`, `Ordering`. |
...
| Privacy / retention | Classify data produced or touched by Messaging / Queues; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Messaging / Queues** before choosing framework or provider mechanisms.
...
- **MUST** — For **Queues**: Define the exact semantics of **Queues** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Consumer groups**: Define the exact semantics of **Consumer groups** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Message acknowledgment**: Acknowledge only after durable effects commit. Size/extend the lease or visibility timeout for real processing duration and make redelivery safe.
- **MUST** — For **Delivery guarantees**: Define the exact semantics of **Delivery guarantees** within Messaging / Queues: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **Queue vs pub/sub/stream** according to the stated trade-off: Choose from ownership, replay, ordering, retention, and consumer independence.
...
- [ ] **Queues:** Locate every implementation path for queues, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Consumer groups:** Locate every implementation path for consumer groups, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Message acknowledgment:** Terminate workers before/after commit and pause them past the lease to verify duplicate containment.
- [ ] **Delivery guarantees:** Locate every implementation path for delivery guarantees, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Exactly-once claims:** Locate every implementation path for exactly-once claims, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Consumer lag:** Locate every implementation path for consumer lag, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- What exact invariant or user/system promise makes **Messaging / Queues** correct, and which component is authoritative for it?
...
- For **Queues**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for queues is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Consumer groups**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for consumer groups is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Message acknowledgment**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Worker crashes after ack lose work; crashes before ack duplicate effects; lease expiry creates concurrent processing.
- For **Delivery guarantees**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for delivery guarantees is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Exactly-once claims**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for exactly-once claims is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Messaging / Queues**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Queues:** Locate every implementation path for queues, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Streams:** Locate every implementation path for streams, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Consumers:** Locate every implementation path for consumers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **At-most-once:** Locate every implementation path for at-most-once, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Dead-letter queues:** Create malformed and deterministically failing messages and exercise inspect, repair, replay, skip, and retention workflows.
- [ ] **Consumer lag:** Locate every implementation path for consumer lag, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
**Paper metadata:** canonical subtopics: 19; layer: `systems`; domain profile: `async`; verified through: `2026-08-17`.
