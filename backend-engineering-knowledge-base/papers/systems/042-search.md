...
**Search** exists to encode domain facts and invariants so valid states are easy to represent and invalid states are difficult to persist. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Tokenization**, **Search permissions**.

### Persistence and integrity

**Exact search**, **Prefix search**, **Full-text search**, **Fuzzy search**, **Typo tolerance**, **Ranking**, **Stemming**, **Stop words**, **Filters**, **Facets**, **Autocomplete**, **Index synchronization**, **Search pagination**.
...
This paper treats **Search** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Exact search:** Define the exact semantics of **Exact search** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Fuzzy search:** Define the exact semantics of **Fuzzy search** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Tokenization:** Define the exact semantics of **Tokenization** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Facets:** Define the exact semantics of **Facets** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Index synchronization:** Design indexes from concrete query predicates and ordering, considering equality/range order, selectivity, write amplification, storage, uniqueness, tenant prefixing, and online build behavior.
- **SHOULD — Search pagination:** Use a total deterministic order with a unique tie-breaker; encode cursor position and query shape opaquely; validate limits and preserve authorization/filter semantics across pages.
...
|---|---|---|
| Synchronous vs asynchronous indexing | Synchronous indexing improves read-after-write but couples availability; asynchronous indexing scales and decouples at eventual consistency cost. | Use durable outbox/CDC and expose freshness semantics when indexing asynchronously. |
...
- **SHOULD — engineering rule:** Define the exact semantics of **Exact search** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for exact search is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for exact search, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Prefix search

- **SHOULD — engineering rule:** Define the exact semantics of **Prefix search** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for prefix search is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for prefix search, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Full-text search

- **SHOULD — engineering rule:** Define the exact semantics of **Full-text search** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for full-text search is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for full-text search, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Fuzzy search

- **SHOULD — engineering rule:** Define the exact semantics of **Fuzzy search** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for fuzzy search is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for fuzzy search, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Typo tolerance

- **SHOULD — engineering rule:** Define the exact semantics of **Typo tolerance** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for typo tolerance is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for typo tolerance, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Ranking

- **SHOULD — engineering rule:** Define the exact semantics of **Ranking** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for ranking is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for ranking, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Tokenization

- **SHOULD — engineering rule:** Define the exact semantics of **Tokenization** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for tokenization is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for tokenization, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Stemming

- **SHOULD — engineering rule:** Define the exact semantics of **Stemming** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for stemming is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for stemming, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Stop words

- **SHOULD — engineering rule:** Define the exact semantics of **Stop words** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for stop words is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for stop words, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Filters

- **SHOULD — engineering rule:** Parse filters into a typed allowlisted AST, validate operators and complexity, bind parameters, combine mandatory tenant/auth predicates, and cap execution cost.
- **Production failure mode:** Injection, authorization bypass, unbounded scans, or query-plan explosion occurs when raw user expressions reach the database.
- **Existing-codebase evidence:** Fuzz operators and nesting, verify mandatory predicates cannot be overridden, and benchmark worst allowed combinations.

### 7.11. Facets

- **SHOULD — engineering rule:** Define the exact semantics of **Facets** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for facets is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for facets, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.12. Autocomplete

- **SHOULD — engineering rule:** Define the exact semantics of **Autocomplete** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for autocomplete is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for autocomplete, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. Search permissions
...
### 7.16. Search consistency

- **SHOULD — engineering rule:** Version index schemas and documents, publish changes through an ordered/idempotent path, retain checkpoints, support shadow rebuild, atomic alias swap, and reconciliation with the source.
- **Production failure mode:** Out-of-order updates resurrect old content, deletes are missed, or a rebuild serves a mixed schema.
- **Existing-codebase evidence:** Replay duplicate/out-of-order events, interrupt full rebuild, compare source/index counts and version watermarks, then test swap and rollback.

### 7.17. Search pagination
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Search path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Exact search`, `Typo tolerance`, `Stop words`. |
...
| Privacy / retention | Classify data produced or touched by Search; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Search** before choosing framework or provider mechanisms.
...
- **MUST** — For **Exact search**: Define the exact semantics of **Exact search** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Fuzzy search**: Define the exact semantics of **Fuzzy search** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Tokenization**: Define the exact semantics of **Tokenization** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Facets**: Define the exact semantics of **Facets** within Search: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **Synchronous vs asynchronous indexing** according to the stated trade-off: Use durable outbox/CDC and expose freshness semantics when indexing asynchronously.
...
- [ ] **Exact search:** Locate every implementation path for exact search, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Fuzzy search:** Locate every implementation path for fuzzy search, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Tokenization:** Locate every implementation path for tokenization, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Facets:** Locate every implementation path for facets, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Index synchronization:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
- [ ] **Search pagination:** Paginate while mutating boundary rows and verify every eligible record appears at most once under the documented consistency model.
...
- **Exact search:** A framework or provider default for exact search is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Fuzzy search:** A framework or provider default for fuzzy search is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Ranking:** A framework or provider default for ranking is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Stop words:** A framework or provider default for stop words is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Autocomplete:** A framework or provider default for autocomplete is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Index synchronization:** An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
- **Search pagination:** Concurrent inserts/updates cause duplicates or omissions, cursors are tampered with, or deep offsets exhaust the database.
...
- What exact invariant or user/system promise makes **Search** correct, and which component is authoritative for it?
...
- For **Exact search**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for exact search is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Fuzzy search**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for fuzzy search is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Tokenization**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for tokenization is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Facets**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for facets is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Index synchronization**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
...
- [ ] Map every entry point for **Search**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Exact search:** Locate every implementation path for exact search, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Fuzzy search:** Locate every implementation path for fuzzy search, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Ranking:** Locate every implementation path for ranking, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Stop words:** Locate every implementation path for stop words, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Autocomplete:** Locate every implementation path for autocomplete, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Index synchronization:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
- [ ] **Search pagination:** Paginate while mutating boundary rows and verify every eligible record appears at most once under the documented consistency model.
...
- [130. Search Index Synchronization](130-search-index-synchronization.md) — layer: `systems`; profile: `migration`.
- [028. Query Design](028-query-design.md) — layer: `systems`; profile: `data_model`.
- [073. Change Data Capture](../primitives/073-change-data-capture.md) — layer: `primitives`; profile: `migration`.
- [143. RAG Infrastructure](143-rag-infrastructure.md) — layer: `systems`; profile: `ai`.
- [037. Caching](037-caching.md) — layer: `systems`; profile: `cache`.
...
- <a id="s089"></a> **[S089] pgvector.** pgvector; 2026; Current. [https://github.com/pgvector/pgvector](https://github.com/pgvector/pgvector) — Tags: vector-search, rag, database.
