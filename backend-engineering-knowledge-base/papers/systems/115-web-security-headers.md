...
**HSTS**, **CSP**, **X-Content-Type-Options**, **Referrer-Policy**, **Frame protections**.

### Security, privacy, and abuse

**Cache-control for sensitive pages**, **Cookie security**.

### Boundary of the paper

This paper treats **Web Security Headers** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — HSTS:** Define the exact semantics of **HSTS** within Web Security Headers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — CSP:** Define the exact semantics of **CSP** within Web Security Headers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Referrer-Policy:** Define the exact semantics of **Referrer-Policy** within Web Security Headers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Permissions-Policy:** Define authorization as `(principal, action, resource, context) -> decision + reason + policy version`. Enforce at the owning boundary and include tenant/resource state, not only route-level roles.
- **SHOULD — Cache-control for sensitive pages:** Classify fields and flows, collect the minimum, limit purpose and access, use irreversible anonymization only when re-identification risk is acceptably low, and propagate deletion/retention to derived systems and backups.
- **MUST — Cookie security:** Set the narrowest domain/path and appropriate Secure, HttpOnly, and SameSite attributes; rotate identifiers after authentication or privilege changes and prevent caching of sensitive responses.
...
- **SHOULD — engineering rule:** Define the exact semantics of **HSTS** within Web Security Headers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for hsts is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for hsts, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. CSP

- **SHOULD — engineering rule:** Define the exact semantics of **CSP** within Web Security Headers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.3. X-Content-Type-Options

- **SHOULD — engineering rule:** Treat declared type, filename extension, and content signature as independent signals; parse with a hardened library and apply an allowlist based on intended processing.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Referrer-Policy** within Web Security Headers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for referrer-policy is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for referrer-policy, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Permissions-Policy
...
### 7.6. Frame protections

- **SHOULD — engineering rule:** Define the exact semantics of **Frame protections** within Web Security Headers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for frame protections is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for frame protections, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Cache-control for sensitive pages
...
- **MUST — engineering rule:** Set the narrowest domain/path and appropriate Secure, HttpOnly, and SameSite attributes; rotate identifiers after authentication or privilege changes and prevent caching of sensitive responses.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Web Security Headers path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `HSTS`, `X-Content-Type-Options`, `Permissions-Policy`. |
...
| Privacy / retention | Classify data produced or touched by Web Security Headers; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Web Security Headers** before choosing framework or provider mechanisms.
...
- **MUST** — For **HSTS**: Define the exact semantics of **HSTS** within Web Security Headers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **CSP**: Define the exact semantics of **CSP** within Web Security Headers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Referrer-Policy**: Define the exact semantics of **Referrer-Policy** within Web Security Headers: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Permissions-Policy**: Define authorization as `(principal, action, resource, context) -> decision + reason + policy version`. Enforce at the owning boundary and include tenant/resource state, not only route-level roles.
...
- [ ] **HSTS:** Locate every implementation path for hsts, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Referrer-Policy:** Locate every implementation path for referrer-policy, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Permissions-Policy:** Trace each read/write from externally supplied identifier to the final query/serializer and verify denial before data exposure or mutation.
- [ ] **Cache-control for sensitive pages:** Build a data-flow inventory and verify each copy's lawful purpose, encryption, access, retention, export, and deletion behavior.
- [ ] **Cookie security:** Inspect real `Set-Cookie` headers through proxies and browsers for every environment and authentication flow.
...
- **HSTS:** A framework or provider default for hsts is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **X-Content-Type-Options:** A polyglot or mislabeled file reaches an unsafe parser or is served with active content semantics.
- **Permissions-Policy:** An authenticated caller accesses another object, hidden field, or state transition because lookup and authorization are separated or policy inputs are incomplete.
- **Frame protections:** A framework or provider default for frame protections is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Cache-control for sensitive pages:** Sensitive data spreads into logs, caches, search, analytics, and backups where deletion and access controls are weaker.
- **Cookie security:** Over-broad cookies leak to sibling hosts, cross-site requests carry ambient authority, or a pre-authentication identifier is fixed and upgraded.
...
- What exact invariant or user/system promise makes **Web Security Headers** correct, and which component is authoritative for it?
...
- For **HSTS**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for hsts is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Referrer-Policy**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for referrer-policy is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Permissions-Policy**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: An authenticated caller accesses another object, hidden field, or state transition because lookup and authorization are separated or policy inputs are incomplete.
- For **Cache-control for sensitive pages**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Sensitive data spreads into logs, caches, search, analytics, and backups where deletion and access controls are weaker.
...
- [ ] Map every entry point for **Web Security Headers**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **HSTS:** Locate every implementation path for hsts, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **CSP:** Locate every implementation path for csp, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **X-Content-Type-Options:** Use malformed, polyglot, spoofed-extension, and parser-differential fixtures through the entire upload/download path.
- [ ] **Permissions-Policy:** Trace each read/write from externally supplied identifier to the final query/serializer and verify denial before data exposure or mutation.
- [ ] **Frame protections:** Locate every implementation path for frame protections, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Cache-control for sensitive pages:** Build a data-flow inventory and verify each copy's lawful purpose, encryption, access, retention, export, and deletion behavior.
- [ ] **Cookie security:** Inspect real `Set-Cookie` headers through proxies and browsers for every environment and authentication flow.
...
**Paper metadata:** canonical subtopics: 8; layer: `systems`; domain profile: `api`; verified through: `2026-08-17`.
