...
# 084. Modular Monolith Architecture
...
**Module boundaries**, **Internal APIs**, **Dependency rules**, **Shared infrastructure**, **Cross-module communication**, **Extraction readiness**.

### Concurrency and distributed behavior

**Domain events**, **Preventing coupling**.

### Boundary of the paper

This paper treats **Modular Monolith Architecture** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Module boundaries:** Define the exact semantics of **Module boundaries** within Modular Monolith Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Dependency rules:** Define the exact semantics of **Dependency rules** within Modular Monolith Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Data ownership:** Assign one owner for invariants and writes, expose a narrow versioned interface, enforce dependency direction mechanically, and communicate cross-boundary facts through explicit calls/events.
- **SHOULD — Cross-module communication:** Define the exact semantics of **Cross-module communication** within Modular Monolith Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Domain events:** Define the exact semantics of **Domain events** within Modular Monolith Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Extraction readiness:** Liveness should detect irrecoverable process failure without depending on remote services; readiness should reflect ability to serve the routed workload; startup probes protect slow initialization.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Module boundaries** within Modular Monolith Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Internal APIs** within Modular Monolith Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for internal apis is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for internal apis, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Dependency rules

- **SHOULD — engineering rule:** Define the exact semantics of **Dependency rules** within Modular Monolith Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for dependency rules is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for dependency rules, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Shared infrastructure** within Modular Monolith Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for shared infrastructure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for shared infrastructure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Cross-module communication

- **SHOULD — engineering rule:** Define the exact semantics of **Cross-module communication** within Modular Monolith Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for cross-module communication is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for cross-module communication, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Domain events

- **SHOULD — engineering rule:** Define the exact semantics of **Domain events** within Modular Monolith Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
### 7.8. Preventing coupling

- **SHOULD — engineering rule:** Define the exact semantics of **Preventing coupling** within Modular Monolith Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for preventing coupling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for preventing coupling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Extraction readiness

- **SHOULD — engineering rule:** Liveness should detect irrecoverable process failure without depending on remote services; readiness should reflect ability to serve the routed workload; startup probes protect slow initialization.
- **Production failure mode:** A dependency outage triggers restart storms, or a process receives traffic before initialization and migrations are complete.
- **Existing-codebase evidence:** Fail each dependency separately and verify probe response, routing, restart behavior, and probe cost under fleet scale.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Modular Monolith Architecture path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Module boundaries`, `Dependency rules`, `Shared infrastructure`. |
...
| Privacy / retention | Classify data produced or touched by Modular Monolith Architecture; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Modular Monolith Architecture** before choosing framework or provider mechanisms.
...
- **MUST** — For **Module boundaries**: Define the exact semantics of **Module boundaries** within Modular Monolith Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Dependency rules**: Define the exact semantics of **Dependency rules** within Modular Monolith Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Data ownership**: Assign one owner for invariants and writes, expose a narrow versioned interface, enforce dependency direction mechanically, and communicate cross-boundary facts through explicit calls/events.
- **MUST** — For **Cross-module communication**: Define the exact semantics of **Cross-module communication** within Modular Monolith Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Module boundaries:** Locate every implementation path for module boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Dependency rules:** Locate every implementation path for dependency rules, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Data ownership:** Build an import/dependency graph, map table writes, and flag cross-boundary direct access.
- [ ] **Cross-module communication:** Locate every implementation path for cross-module communication, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Extraction readiness:** Fail each dependency separately and verify probe response, routing, restart behavior, and probe cost under fleet scale.
...
- **Module boundaries:** A framework or provider default for module boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Internal APIs:** A framework or provider default for internal apis is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Data ownership:** Modules share tables/models/utilities until changes require lockstep deployment and no team can reason locally.
- **Shared infrastructure:** A framework or provider default for shared infrastructure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Cross-module communication:** A framework or provider default for cross-module communication is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Preventing coupling:** A framework or provider default for preventing coupling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Extraction readiness:** A dependency outage triggers restart storms, or a process receives traffic before initialization and migrations are complete.
...
- What exact invariant or user/system promise makes **Modular Monolith Architecture** correct, and which component is authoritative for it?
...
- For **Module boundaries**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for module boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Dependency rules**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for dependency rules is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Data ownership**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Modules share tables/models/utilities until changes require lockstep deployment and no team can reason locally.
- For **Cross-module communication**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for cross-module communication is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Modular Monolith Architecture**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Module boundaries:** Locate every implementation path for module boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Internal APIs:** Locate every implementation path for internal apis, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Data ownership:** Build an import/dependency graph, map table writes, and flag cross-boundary direct access.
- [ ] **Shared infrastructure:** Locate every implementation path for shared infrastructure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Cross-module communication:** Locate every implementation path for cross-module communication, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Preventing coupling:** Locate every implementation path for preventing coupling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Extraction readiness:** Fail each dependency separately and verify probe response, routing, restart behavior, and probe cost under fleet scale.
...
**Paper metadata:** canonical subtopics: 9; layer: `systems`; domain profile: `architecture`; verified through: `2026-08-17`.
