...
**Indexing** exists to encode domain facts and invariants so valid states are easy to represent and invalid states are difficult to persist. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Single-column indexes**, **Composite indexes**, **Unique indexes**, **Partial indexes**, **Sparse indexes**, **Covering indexes**, **Full-text indexes**, **Geo indexes**, **Vector indexes**, **Index order**, **Selectivity**, **Write overhead**, **Index bloat**, **Unused indexes**.
...
This paper treats **Indexing** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Single-column indexes:** Design indexes from concrete query predicates and ordering, considering equality/range order, selectivity, write amplification, storage, uniqueness, tenant prefixing, and online build behavior.
- **SHOULD — Partial indexes:** Design indexes from concrete query predicates and ordering, considering equality/range order, selectivity, write amplification, storage, uniqueness, tenant prefixing, and online build behavior.
- **SHOULD — Full-text indexes:** Design indexes from concrete query predicates and ordering, considering equality/range order, selectivity, write amplification, storage, uniqueness, tenant prefixing, and online build behavior.
- **SHOULD — Vector indexes:** Design indexes from concrete query predicates and ordering, considering equality/range order, selectivity, write amplification, storage, uniqueness, tenant prefixing, and online build behavior.
- **SHOULD — Write overhead:** Define the exact semantics of **Write overhead** within Indexing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Index migrations:** Design indexes from concrete query predicates and ordering, considering equality/range order, selectivity, write amplification, storage, uniqueness, tenant prefixing, and online build behavior.
...
- **MUST — engineering rule:** Design indexes from concrete query predicates and ordering, considering equality/range order, selectivity, write amplification, storage, uniqueness, tenant prefixing, and online build behavior.
- **Production failure mode:** An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
- **Existing-codebase evidence:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
...
### 7.6. Covering indexes

- **SHOULD — engineering rule:** Design indexes from concrete query predicates and ordering, considering equality/range order, selectivity, write amplification, storage, uniqueness, tenant prefixing, and online build behavior.
- **Production failure mode:** An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
- **Existing-codebase evidence:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.

### 7.7. Full-text indexes
...
- **SHOULD — engineering rule:** Define the exact semantics of **Write overhead** within Indexing: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for write overhead is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for write overhead, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Indexing path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Single-column indexes`, `Sparse indexes`, `Geo indexes`. |
...
| Privacy / retention | Classify data produced or touched by Indexing; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Indexing** before choosing framework or provider mechanisms.
...
- **MUST** — For **Single-column indexes**: Design indexes from concrete query predicates and ordering, considering equality/range order, selectivity, write amplification, storage, uniqueness, tenant prefixing, and online build behavior.
- **MUST** — For **Partial indexes**: Design indexes from concrete query predicates and ordering, considering equality/range order, selectivity, write amplification, storage, uniqueness, tenant prefixing, and online build behavior.
- **MUST** — For **Full-text indexes**: Design indexes from concrete query predicates and ordering, considering equality/range order, selectivity, write amplification, storage, uniqueness, tenant prefixing, and online build behavior.
- **MUST** — For **Vector indexes**: Design indexes from concrete query predicates and ordering, considering equality/range order, selectivity, write amplification, storage, uniqueness, tenant prefixing, and online build behavior.
...
- [ ] **Single-column indexes:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
- [ ] **Partial indexes:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
- [ ] **Full-text indexes:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
- [ ] **Vector indexes:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
- [ ] **Write overhead:** Locate every implementation path for write overhead, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Index migrations:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
...
- **Single-column indexes:** An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
- **Unique indexes:** An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
- **Covering indexes:** An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
- **Geo indexes:** An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
- **Index order:** An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
- **Index bloat:** An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
- **Index migrations:** An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
...
- What exact invariant or user/system promise makes **Indexing** correct, and which component is authoritative for it?
...
- For **Single-column indexes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
- For **Partial indexes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
- For **Full-text indexes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
- For **Vector indexes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: An index exists but cannot support the actual predicate/order, low-selectivity indexes waste writes, or an online build saturates I/O and replication.
- For **Write overhead**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for write overhead is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Indexing**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Single-column indexes:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
- [ ] **Unique indexes:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
- [ ] **Covering indexes:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
- [ ] **Geo indexes:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
- [ ] **Index order:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
- [ ] **Index bloat:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
- [ ] **Index migrations:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
...
**Paper metadata:** canonical subtopics: 15; layer: `systems`; domain profile: `data_model`; verified through: `2026-08-17`.
