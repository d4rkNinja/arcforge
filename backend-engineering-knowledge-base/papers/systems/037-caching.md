...
**Local cache**, **Distributed cache**, **Cache-aside**, **Read-through**, **Write-through**, **Write-behind**, **TTL**, **Eviction**, **Cache keys**, **Namespacing**, **Cache warming**, **Negative caching**, **Cache stampede**, **Cache penetration**, **Hot keys**, **Stale reads**, **Cache consistency**, **Cache failures**.

### Boundary of the paper

This paper treats **Caching** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Local cache:** Define the exact semantics of **Local cache** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Write-through:** Define the exact semantics of **Write-through** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Cache keys:** Construct keys from explicit canonical dimensions including environment, tenant, resource, representation/version, authorization context, locale, and schema version where relevant.
- **MUST — Cache invalidation:** Define the exact semantics of **Cache invalidation** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Cache penetration:** Use request coalescing/singleflight, bounded locking with fencing where needed, jittered expiry, negative caching, admission control, and precomputation for known hot objects.
- **SHOULD — Cache failures:** Define the exact semantics of **Cache failures** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|---|
| Cache-aside vs write-through/write-behind | Cache-aside is simple but has stale windows; write-through coordinates updates but couples availability; write-behind risks data loss and reordering. | Keep authority explicit and choose freshness/failure behavior deliberately. |
...
- **SHOULD — engineering rule:** Define the exact semantics of **Local cache** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for local cache is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for local cache, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Distributed cache

- **SHOULD — engineering rule:** Define the exact semantics of **Distributed cache** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for distributed cache is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for distributed cache, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Cache-aside

- **SHOULD — engineering rule:** Define the exact semantics of **Cache-aside** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for cache-aside is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for cache-aside, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Read-through

- **SHOULD — engineering rule:** Define the exact semantics of **Read-through** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for read-through is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for read-through, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Write-through

- **SHOULD — engineering rule:** Define the exact semantics of **Write-through** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for write-through is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for write-through, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Write-behind

- **MAY — engineering rule:** Define the exact semantics of **Write-behind** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for write-behind is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for write-behind, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Eviction** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for eviction is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for eviction, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Cache keys

- **SHOULD — engineering rule:** Construct keys from explicit canonical dimensions including environment, tenant, resource, representation/version, authorization context, locale, and schema version where relevant.
- **Production failure mode:** Two contexts collide, sensitive data crosses tenants/users, or a rollout reads an incompatible serialized value.
- **Existing-codebase evidence:** Enumerate every input that changes the result and prove it either appears in the key or is immutable.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Serialization** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for serialization is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for serialization, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.12. Cache invalidation

- **MUST — engineering rule:** Define the exact semantics of **Cache invalidation** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for cache invalidation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for cache invalidation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. Cache warming

- **SHOULD — engineering rule:** Define the exact semantics of **Cache warming** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for cache warming is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for cache warming, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.14. Negative caching

- **SHOULD — engineering rule:** Define the exact semantics of **Negative caching** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for negative caching is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for negative caching, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Stale reads** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for stale reads is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for stale reads, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.19. Cache consistency

- **SHOULD — engineering rule:** Define the exact semantics of **Cache consistency** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for cache consistency is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for cache consistency, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.20. Cache failures

- **SHOULD — engineering rule:** Define the exact semantics of **Cache failures** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for cache failures is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for cache failures, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Caching path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Local cache`, `Write-behind`, `Serialization`. |
...
| Privacy / retention | Classify data produced or touched by Caching; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Caching** before choosing framework or provider mechanisms.
...
- **MUST** — For **Local cache**: Define the exact semantics of **Local cache** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Write-through**: Define the exact semantics of **Write-through** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Cache keys**: Construct keys from explicit canonical dimensions including environment, tenant, resource, representation/version, authorization context, locale, and schema version where relevant.
- **MUST** — For **Cache invalidation**: Define the exact semantics of **Cache invalidation** within Caching: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **Cache-aside vs write-through/write-behind** according to the stated trade-off: Keep authority explicit and choose freshness/failure behavior deliberately.
...
- [ ] **Local cache:** Locate every implementation path for local cache, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Write-through:** Locate every implementation path for write-through, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Cache keys:** Enumerate every input that changes the result and prove it either appears in the key or is immutable.
- [ ] **Cache invalidation:** Locate every implementation path for cache invalidation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Cache penetration:** Load-test synchronized expiry and high-cardinality misses while monitoring source QPS and waiter limits.
- [ ] **Cache failures:** Locate every implementation path for cache failures, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Local cache:** A framework or provider default for local cache is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Read-through:** A framework or provider default for read-through is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **TTL:** Data remains valid too long, expires simultaneously causing a stampede, or code assumes expired records are immediately deleted.
- **Serialization:** A framework or provider default for serialization is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Negative caching:** A framework or provider default for negative caching is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Hot keys:** A popular miss fans out to the source and causes cascading failure; malicious distinct misses defeat the cache.
- **Cache failures:** A framework or provider default for cache failures is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Caching** correct, and which component is authoritative for it?
...
- For **Local cache**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for local cache is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Write-through**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for write-through is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Cache keys**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Two contexts collide, sensitive data crosses tenants/users, or a rollout reads an incompatible serialized value.
- For **Cache invalidation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for cache invalidation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Cache penetration**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A popular miss fans out to the source and causes cascading failure; malicious distinct misses defeat the cache.
...
- [ ] Map every entry point for **Caching**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Local cache:** Locate every implementation path for local cache, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Read-through:** Locate every implementation path for read-through, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **TTL:** Test exact boundary times with controlled clocks, delayed cleanup, clock skew, and mass expiry.
- [ ] **Serialization:** Locate every implementation path for serialization, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Negative caching:** Locate every implementation path for negative caching, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Hot keys:** Load-test synchronized expiry and high-cardinality misses while monitoring source QPS and waiter limits.
- [ ] **Cache failures:** Locate every implementation path for cache failures, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [042. Search](042-search.md) — layer: `systems`; profile: `data_model`.
...
**Paper metadata:** canonical subtopics: 20; layer: `systems`; domain profile: `cache`; verified through: `2026-08-17`.
