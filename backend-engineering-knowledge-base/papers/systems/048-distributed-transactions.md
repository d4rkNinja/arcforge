...
**Local transactions**, **Transaction ownership**.

### Concurrency and distributed behavior

**Two-phase commit**, **Sagas**, **Choreography**, **Orchestration**, **Compensating actions**, **Partial failures**, **Retry safety**, **Compensation failure**, **Consistency boundaries**.

### Boundary of the paper

This paper treats **Distributed Transactions** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
|---|---|---|
| 2PC vs saga | 2PC can preserve atomicity among compatible participants but reduces availability and increases coordination; sagas accept intermediate states and require compensation. | Prefer local transactions and redesign boundaries; use either only when its failure model is understood. |
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Distributed Transactions path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Local transactions`, `Sagas`, `Compensating actions`. |
...
| Privacy / retention | Classify data produced or touched by Distributed Transactions; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Distributed Transactions** before choosing framework or provider mechanisms.
...
- **MUST** — For **Local transactions**: Define the exact semantics of **Local transactions** within Distributed Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Sagas**: Define the exact semantics of **Sagas** within Distributed Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Orchestration**: Define the exact semantics of **Orchestration** within Distributed Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Partial failures**: Define the exact semantics of **Partial failures** within Distributed Transactions: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **2PC vs saga** according to the stated trade-off: Prefer local transactions and redesign boundaries; use either only when its failure model is understood.
...
- [ ] **Local transactions:** Locate every implementation path for local transactions, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Sagas:** Locate every implementation path for sagas, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Orchestration:** Locate every implementation path for orchestration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Compensation failure:** Locate every implementation path for compensation failure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Transaction ownership:** Locate every implementation path for transaction ownership, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Local transactions:** A framework or provider default for local transactions is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Sagas:** A framework or provider default for sagas is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Choreography:** A framework or provider default for choreography is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Compensating actions:** A framework or provider default for compensating actions is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Retry safety:** Permanent failures loop forever, retries synchronize into a storm, or a timed-out successful operation is duplicated.
- **Compensation failure:** A framework or provider default for compensation failure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Transaction ownership:** A framework or provider default for transaction ownership is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Distributed Transactions** correct, and which component is authoritative for it?
...
- For **Local transactions**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for local transactions is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Sagas**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for sagas is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Orchestration**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for orchestration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Partial failures**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for partial failures is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Compensation failure**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for compensation failure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Distributed Transactions**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Local transactions:** Locate every implementation path for local transactions, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Sagas:** Locate every implementation path for sagas, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Choreography:** Locate every implementation path for choreography, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Compensating actions:** Locate every implementation path for compensating actions, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
**Paper metadata:** canonical subtopics: 11; layer: `systems`; domain profile: `transactions`; verified through: `2026-08-17`.
