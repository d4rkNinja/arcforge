---
paper_number: 87
title: "Code-Level Architecture"
layer: systems
domain_profile: architecture
...
**Controllers**, **Handlers**, **Services**, **Use cases**, **Domain models**, **Repositories**, **DTOs**, **Mappers**, **Validators**, **Middleware**, **Adapters**, **Ports**, **Factories**, **Composition roots**, **Error boundaries**.
...
This paper treats **Code-Level Architecture** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Controllers:** Define the exact semantics of **Controllers** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Use cases:** Define the exact semantics of **Use cases** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — DTOs:** Define the exact semantics of **DTOs** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Adapters:** Define the exact semantics of **Adapters** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Composition roots:** Assemble concrete dependencies in a small composition root; inject behavior-bearing interfaces, lifecycle, clocks, and external clients without turning every data structure into an interface.
- **SHOULD — Error boundaries:** Define the exact semantics of **Error boundaries** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Controllers** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for controllers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for controllers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Handlers

- **SHOULD — engineering rule:** Define the exact semantics of **Handlers** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for handlers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for handlers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Services

- **SHOULD — engineering rule:** Define the exact semantics of **Services** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for services is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for services, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Use cases

- **SHOULD — engineering rule:** Define the exact semantics of **Use cases** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for use cases is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for use cases, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Domain models

- **SHOULD — engineering rule:** Define the exact semantics of **Domain models** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for domain models is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for domain models, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Repositories

- **SHOULD — engineering rule:** Define the exact semantics of **Repositories** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for repositories is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for repositories, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. DTOs

- **SHOULD — engineering rule:** Define the exact semantics of **DTOs** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for dtos is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for dtos, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Mappers

- **SHOULD — engineering rule:** Define the exact semantics of **Mappers** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for mappers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for mappers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Validators

- **SHOULD — engineering rule:** Define the exact semantics of **Validators** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for validators is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for validators, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Middleware

- **SHOULD — engineering rule:** Define the exact semantics of **Middleware** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for middleware is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for middleware, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Adapters

- **SHOULD — engineering rule:** Define the exact semantics of **Adapters** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for adapters is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for adapters, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.12. Ports

- **SHOULD — engineering rule:** Define the exact semantics of **Ports** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for ports is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for ports, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. Factories

- **SHOULD — engineering rule:** Define the exact semantics of **Factories** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for factories is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for factories, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.14. Composition roots
...
### 7.16. Transaction boundaries

- **MUST — engineering rule:** Define the exact semantics of **Transaction boundaries** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for transaction boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for transaction boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.17. Error boundaries

- **SHOULD — engineering rule:** Define the exact semantics of **Error boundaries** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for error boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for error boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Code-Level Architecture path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Controllers`, `Domain models`, `Validators`. |
...
| Privacy / retention | Classify data produced or touched by Code-Level Architecture; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Code-Level Architecture** before choosing framework or provider mechanisms.
...
- **MUST** — For **Controllers**: Define the exact semantics of **Controllers** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Use cases**: Define the exact semantics of **Use cases** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **DTOs**: Define the exact semantics of **DTOs** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Adapters**: Define the exact semantics of **Adapters** within Code-Level Architecture: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Controllers:** Locate every implementation path for controllers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Use cases:** Locate every implementation path for use cases, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **DTOs:** Locate every implementation path for dtos, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Adapters:** Locate every implementation path for adapters, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Composition roots:** Trace construction and shutdown ownership for each resource and ensure tests can replace true boundaries without bypassing behavior.
- [ ] **Error boundaries:** Locate every implementation path for error boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- Generic repository hiding query semantics.
- Shared DTO becoming de facto cross-service database schema.
- Distributed monolith with lockstep deployments.
- Business transaction spread across controller hooks.
- **Controllers:** A framework or provider default for controllers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Use cases:** A framework or provider default for use cases is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Repositories:** A framework or provider default for repositories is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Validators:** A framework or provider default for validators is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Ports:** A framework or provider default for ports is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Composition roots:** Hidden globals and service locators make ordering, testing, and resource ownership unknowable; over-abstraction hides capabilities.
- **Error boundaries:** A framework or provider default for error boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.

## 18. AI coding-agent failure modes

An AI agent is especially likely to:

- Creating interfaces for every class without a variability boundary.
- Splitting services around CRUD entities instead of invariants.
- Sharing database tables across claimed owners.
...
- What exact invariant or user/system promise makes **Code-Level Architecture** correct, and which component is authoritative for it?
...
- For **Controllers**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for controllers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Use cases**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for use cases is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **DTOs**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for dtos is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Adapters**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for adapters is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Composition roots**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Hidden globals and service locators make ordering, testing, and resource ownership unknowable; over-abstraction hides capabilities.
...
- [ ] Map every entry point for **Code-Level Architecture**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Controllers:** Locate every implementation path for controllers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Use cases:** Locate every implementation path for use cases, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Repositories:** Locate every implementation path for repositories, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Validators:** Locate every implementation path for validators, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Ports:** Locate every implementation path for ports, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Composition roots:** Trace construction and shutdown ownership for each resource and ensure tests can replace true boundaries without bypassing behavior.
- [ ] **Error boundaries:** Locate every implementation path for error boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [086. Dependency Boundaries](086-dependency-boundaries.md) — layer: `systems`; profile: `architecture`.
- [088. Abstraction Design](088-abstraction-design.md) — layer: `systems`; profile: `architecture`.
- [084. Modular Monolith Architecture](084-modular-monolith-architecture.md) — layer: `systems`; profile: `architecture`.
- [089. Dependency Management](../cross-cutting/089-dependency-management.md) — layer: `cross-cutting`; profile: `architecture`.
...
- [107. CI/CD](../cross-cutting/107-ci-cd.md) — layer: `cross-cutting`; profile: `runtime`.
...
**Paper metadata:** canonical subtopics: 17; layer: `systems`; domain profile: `architecture`; verified through: `2026-08-17`.
