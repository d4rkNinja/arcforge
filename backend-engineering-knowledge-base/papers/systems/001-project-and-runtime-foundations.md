---
paper_number: 1
title: "Project & Runtime Foundations"
layer: systems
...
**Graceful startup**, **Graceful shutdown**, **Resource cleanup**, **Worker lifecycle**, **Request lifecycle**.

### Contracts and validation

**Configuration loading**, **Runtime validation**.

### Security, privacy, and abuse

**Dependency injection**, **Secrets loading**.

### Operations and observability

**Project structure**, **Module/package boundaries**, **Application bootstrap**, **Environment variables**, **Process signals**, **Dependency initialization order**, **Build metadata**, **Environment separation**, **Development/staging/production behavior**.
...
This paper treats **Project & Runtime Foundations** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Project structure:** Define the exact semantics of **Project structure** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Configuration loading:** Define the exact semantics of **Configuration loading** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — Runtime validation:** Use a standards-aware parser and validate only constraints the application truly requires. Separate syntactic validity from ownership, reachability, authorization, and business eligibility.
- **SHOULD — Dependency initialization order:** Define the exact semantics of **Dependency initialization order** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Request lifecycle:** Define the exact semantics of **Request lifecycle** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Development/staging/production behavior:** Define the exact semantics of **Development/staging/production behavior** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Project structure** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for project structure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for project structure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Module/package boundaries

- **SHOULD — engineering rule:** Define the exact semantics of **Module/package boundaries** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for module/package boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for module/package boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Application bootstrap

- **SHOULD — engineering rule:** Define the exact semantics of **Application bootstrap** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for application bootstrap is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for application bootstrap, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Configuration loading** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for configuration loading is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for configuration loading, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Environment variables

- **SHOULD — engineering rule:** Define the exact semantics of **Environment variables** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for environment variables is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for environment variables, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
### 7.12. Dependency initialization order

- **SHOULD — engineering rule:** Define the exact semantics of **Dependency initialization order** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for dependency initialization order is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for dependency initialization order, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. Resource cleanup

- **MUST — engineering rule:** Define the exact semantics of **Resource cleanup** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for resource cleanup is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for resource cleanup, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.14. Worker lifecycle

- **SHOULD — engineering rule:** Define the exact semantics of **Worker lifecycle** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for worker lifecycle is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for worker lifecycle, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.15. Request lifecycle

- **SHOULD — engineering rule:** Define the exact semantics of **Request lifecycle** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for request lifecycle is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for request lifecycle, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.16. Application versioning

- **SHOULD — engineering rule:** Define the exact semantics of **Application versioning** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for application versioning is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for application versioning, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.17. Build metadata

- **SHOULD — engineering rule:** Define the exact semantics of **Build metadata** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for build metadata is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for build metadata, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.18. Environment separation

- **SHOULD — engineering rule:** Define the exact semantics of **Environment separation** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for environment separation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for environment separation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.19. Development/staging/production behavior

- **SHOULD — engineering rule:** Define the exact semantics of **Development/staging/production behavior** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for development/staging/production behavior is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for development/staging/production behavior, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST** — For **Project structure**: Define the exact semantics of **Project structure** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Configuration loading**: Define the exact semantics of **Configuration loading** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MUST** — For **Dependency initialization order**: Define the exact semantics of **Dependency initialization order** within Project & Runtime Foundations: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Project structure:** Locate every implementation path for project structure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Configuration loading:** Locate every implementation path for configuration loading, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Runtime validation:** Maintain positive/negative conformance fixtures and test normalization, international forms, boundary lengths, and parser differentials.
- [ ] **Dependency initialization order:** Locate every implementation path for dependency initialization order, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Request lifecycle:** Locate every implementation path for request lifecycle, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Development/staging/production behavior:** Locate every implementation path for development/staging/production behavior, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Project structure:** A framework or provider default for project structure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Dependency injection:** Hidden globals and service locators make ordering, testing, and resource ownership unknowable; over-abstraction hides capabilities.
- **Secrets loading:** A leaked environment dump or long-lived credential grants broad access, or rotation breaks live processes because only one version is accepted.
- **Graceful shutdown:** A probabilistic sleep-based test passes while the actual race remains reachable.
- **Resource cleanup:** A framework or provider default for resource cleanup is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Application versioning:** A framework or provider default for application versioning is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Development/staging/production behavior:** A framework or provider default for development/staging/production behavior is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Project structure**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for project structure is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Configuration loading**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for configuration loading is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Runtime validation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Overly strict regexes reject valid values while permissive parsing accepts ambiguous or dangerous forms; validation is mistaken for verification.
- For **Dependency initialization order**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for dependency initialization order is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Request lifecycle**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for request lifecycle is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] **Project structure:** Locate every implementation path for project structure, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Dependency injection:** Trace construction and shutdown ownership for each resource and ensure tests can replace true boundaries without bypassing behavior.
- [ ] **Secrets loading:** Scan artifacts/logs, rotate under load, revoke the old value, and verify every consumer refreshes without restart assumptions.
- [ ] **Graceful shutdown:** Instrument the exact read/check/write or lease boundary and run deterministic competing operations.
...
- [ ] **Application versioning:** Locate every implementation path for application versioning, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Development/staging/production behavior:** Locate every implementation path for development/staging/production behavior, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [059. Health Checks](../cross-cutting/059-health-checks.md) — layer: `cross-cutting`; profile: `observability`.
- [011. Request Lifecycle](../primitives/011-request-lifecycle.md) — layer: `primitives`; profile: `api`.
- [109. Resource Management](../cross-cutting/109-resource-management.md) — layer: `cross-cutting`; profile: `performance`.
...
**Paper metadata:** canonical subtopics: 19; layer: `systems`; domain profile: `runtime`; verified through: `2026-08-17`.
