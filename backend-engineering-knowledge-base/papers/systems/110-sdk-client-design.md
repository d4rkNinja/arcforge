...
**Authentication**, **User-agent metadata**.

### Contracts and validation

**Configuration**, **Base URL**, **Retries**, **Pagination**, **Errors**, **Streaming**, **Telemetry**.
...
This paper treats **SDK / Client Design** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **MUST — Authentication:** Define the exact semantics of **Authentication** within SDK / Client Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Base URL:** Define the exact semantics of **Base URL** within SDK / Client Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — Backward compatibility:** Define a compatibility window in which old and new readers/writers coexist. Make changes additive first, preserve unknown fields where required, and test both deployment orders.
- **SHOULD — User-agent metadata:** Define the exact semantics of **User-agent metadata** within SDK / Client Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MUST — engineering rule:** Define the exact semantics of **Authentication** within SDK / Client Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Configuration** within SDK / Client Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for configuration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for configuration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Base URL

- **SHOULD — engineering rule:** Define the exact semantics of **Base URL** within SDK / Client Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for base url is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for base url, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Retries** within SDK / Client Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Errors** within SDK / Client Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Versioning** within SDK / Client Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Streaming** within SDK / Client Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Telemetry** within SDK / Client Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for telemetry is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for telemetry, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. User-agent metadata

- **SHOULD — engineering rule:** Define the exact semantics of **User-agent metadata** within SDK / Client Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for user-agent metadata is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for user-agent metadata, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **MUST** — For **Authentication**: Define the exact semantics of **Authentication** within SDK / Client Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Base URL**: Define the exact semantics of **Base URL** within SDK / Client Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Base URL:** Locate every implementation path for base url, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Backward compatibility:** Run an N/N+1 matrix for requests, stored data, events, queues, caches, and rollback.
- [ ] **User-agent metadata:** Locate every implementation path for user-agent metadata, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Base URL:** A framework or provider default for base url is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Errors:** A framework or provider default for errors is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Backward compatibility:** A new writer emits data an old reader cannot parse, or rollback code cannot read records created during the failed release.
- **User-agent metadata:** A framework or provider default for user-agent metadata is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Base URL**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for base url is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Idempotency**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Two concurrent requests both execute, or the same key is reused with different parameters and returns the wrong result.
- For **Backward compatibility**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A new writer emits data an old reader cannot parse, or rollback code cannot read records created during the failed release.
...
- [ ] **Errors:** Locate every implementation path for errors, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [111. CLI Backend Interaction](111-cli-backend-interaction.md) — layer: `systems`; profile: `client`.
...
**Paper metadata:** canonical subtopics: 13; layer: `systems`; domain profile: `client`; verified through: `2026-08-17`.
