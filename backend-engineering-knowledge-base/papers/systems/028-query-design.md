...
**Query Design** exists to encode domain facts and invariants so valid states are easy to represent and invalid states are difficult to persist. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Query plans**, **N+1 queries**, **Joins**, **Aggregations**, **Batch reads**, **Projection**, **Over-fetching**, **Under-fetching**, **Query limits**, **Query timeouts**, **Slow-query detection**, **Explain plans**, **Query caching**.
...
This paper treats **Query Design** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Query plans:** Treat plans as data-dependent and version-dependent. Observe rows estimated versus actual, chosen indexes, memory/spill, lock time, and replica behavior under representative parameters.
- **SHOULD — Aggregations:** Define the exact semantics of **Aggregations** within Query Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Projection:** Define the exact semantics of **Projection** within Query Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Query limits:** Define the exact semantics of **Query limits** within Query Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Slow-query detection:** Treat plans as data-dependent and version-dependent. Observe rows estimated versus actual, chosen indexes, memory/spill, lock time, and replica behavior under representative parameters.
- **SHOULD — Read replicas:** Define which reads may be stale and how read-your-writes is achieved after mutation. Route authorization, uniqueness, and state-transition checks to an appropriately consistent source.
...
- **SHOULD — engineering rule:** Treat plans as data-dependent and version-dependent. Observe rows estimated versus actual, chosen indexes, memory/spill, lock time, and replica behavior under representative parameters.
- **Production failure mode:** A query is fast in tests but degenerates after data skew, statistics drift, parameter sensitivity, or schema change.
- **Existing-codebase evidence:** Store representative explain/analyze evidence and alert on latency/rows/scans rather than pinning plans blindly.

### 7.2. N+1 queries

- **SHOULD — engineering rule:** Make query count and cardinality visible at the use-case boundary; batch or join according to consistency and memory needs rather than hiding repeated loads behind repositories.
- **Production failure mode:** Latency and database load grow linearly with result count and only fail at production cardinality.
- **Existing-codebase evidence:** Assert bounded query counts for representative pages and inspect traces for repeated identical query shapes.

### 7.3. Joins

- **SHOULD — engineering rule:** Define the exact semantics of **Joins** within Query Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for joins is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for joins, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Aggregations

- **SHOULD — engineering rule:** Define the exact semantics of **Aggregations** within Query Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for aggregations is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for aggregations, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Batch reads

- **SHOULD — engineering rule:** Define the exact semantics of **Batch reads** within Query Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for batch reads is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for batch reads, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Projection

- **SHOULD — engineering rule:** Define the exact semantics of **Projection** within Query Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for projection is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for projection, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Over-fetching

- **SHOULD — engineering rule:** Define the exact semantics of **Over-fetching** within Query Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for over-fetching is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for over-fetching, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Under-fetching

- **SHOULD — engineering rule:** Define the exact semantics of **Under-fetching** within Query Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for under-fetching is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for under-fetching, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Query limits

- **SHOULD — engineering rule:** Define the exact semantics of **Query limits** within Query Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for query limits is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for query limits, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Query caching** within Query Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for query caching is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for query caching, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.14. Read replicas

- **SHOULD — engineering rule:** Define which reads may be stale and how read-your-writes is achieved after mutation. Route authorization, uniqueness, and state-transition checks to an appropriately consistent source.
- **Production failure mode:** A just-written record appears missing, a revoked permission remains active, or a stale check authorizes an invalid transition.
- **Existing-codebase evidence:** Inject lag and failover; test post-write reads, monotonic pagination, and fallback to primary.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Query Design path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Query plans`, `Aggregations`, `Over-fetching`. |
...
| Privacy / retention | Classify data produced or touched by Query Design; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Query Design** before choosing framework or provider mechanisms.
...
- **MUST** — For **Query plans**: Treat plans as data-dependent and version-dependent. Observe rows estimated versus actual, chosen indexes, memory/spill, lock time, and replica behavior under representative parameters.
- **MUST** — For **Aggregations**: Define the exact semantics of **Aggregations** within Query Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Projection**: Define the exact semantics of **Projection** within Query Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Query limits**: Define the exact semantics of **Query limits** within Query Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Query plans:** Store representative explain/analyze evidence and alert on latency/rows/scans rather than pinning plans blindly.
- [ ] **Aggregations:** Locate every implementation path for aggregations, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Projection:** Locate every implementation path for projection, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Query limits:** Locate every implementation path for query limits, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Slow-query detection:** Store representative explain/analyze evidence and alert on latency/rows/scans rather than pinning plans blindly.
- [ ] **Read replicas:** Inject lag and failover; test post-write reads, monotonic pagination, and fallback to primary.
...
- **Query plans:** A query is fast in tests but degenerates after data skew, statistics drift, parameter sensitivity, or schema change.
- **Joins:** A framework or provider default for joins is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Batch reads:** A framework or provider default for batch reads is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Over-fetching:** A framework or provider default for over-fetching is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Query timeouts:** Requests wait indefinitely, downstream work continues after callers leave, or nested timeouts exceed the original budget.
- **Explain plans:** A query is fast in tests but degenerates after data skew, statistics drift, parameter sensitivity, or schema change.
- **Read replicas:** A just-written record appears missing, a revoked permission remains active, or a stale check authorizes an invalid transition.
...
- What exact invariant or user/system promise makes **Query Design** correct, and which component is authoritative for it?
...
- For **Query plans**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A query is fast in tests but degenerates after data skew, statistics drift, parameter sensitivity, or schema change.
- For **Aggregations**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for aggregations is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Projection**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for projection is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Query limits**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for query limits is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Slow-query detection**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A query is fast in tests but degenerates after data skew, statistics drift, parameter sensitivity, or schema change.
...
- [ ] Map every entry point for **Query Design**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Query plans:** Store representative explain/analyze evidence and alert on latency/rows/scans rather than pinning plans blindly.
- [ ] **Joins:** Locate every implementation path for joins, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Batch reads:** Locate every implementation path for batch reads, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Over-fetching:** Locate every implementation path for over-fetching, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Query timeouts:** Inject slow dependencies at each hop and verify cancellation reaches database, network calls, streams, and workers.
- [ ] **Explain plans:** Store representative explain/analyze evidence and alert on latency/rows/scans rather than pinning plans blindly.
- [ ] **Read replicas:** Inject lag and failover; test post-write reads, monotonic pagination, and fallback to primary.
...
- [021. Database Modeling](021-database-modeling.md) — layer: `systems`; profile: `data_model`.
- [042. Search](042-search.md) — layer: `systems`; profile: `data_model`.
- [095. Performance Engineering](../cross-cutting/095-performance-engineering.md) — layer: `cross-cutting`; profile: `performance`.
...
**Paper metadata:** canonical subtopics: 14; layer: `systems`; domain profile: `data_model`; verified through: `2026-08-17`.
