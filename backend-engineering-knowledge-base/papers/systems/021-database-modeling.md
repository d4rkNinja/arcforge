...
**Database Modeling** exists to encode domain facts and invariants so valid states are easy to represent and invalid states are difficult to persist. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**Tables**, **Collections**, **Documents**, **Relationships**, **Embedded data**, **References**, **One-to-one**, **One-to-many**, **Many-to-many**, **Optional relationships**, **Polymorphic relationships**, **Hierarchies**, **Metadata**, **Temporal data**, **Versioned records**, **Denormalization**.

### Boundary of the paper

This paper treats **Database Modeling** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Tables:** Define the exact semantics of **Tables** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Relationships:** Define the exact semantics of **Relationships** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — One-to-one:** Define the exact semantics of **One-to-one** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Polymorphic relationships:** Define the exact semantics of **Polymorphic relationships** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Temporal data:** Define the exact semantics of **Temporal data** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Denormalization:** Name the canonical source, update path, acceptable staleness, version, rebuild algorithm, and reconciliation signal for every duplicated or derived value.
...
|---|---|---|
| Embed vs reference | Embedding improves atomic locality and reads; references reduce duplication and document growth but add joins/fetches and lifecycle coupling. | Embed data with shared lifecycle and bounded size; reference independently owned or unbounded entities. |
...
- **SHOULD — engineering rule:** Define the exact semantics of **Tables** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for tables is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for tables, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Collections

- **SHOULD — engineering rule:** Define the exact semantics of **Collections** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for collections is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for collections, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Documents

- **SHOULD — engineering rule:** Define the exact semantics of **Documents** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for documents is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for documents, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Relationships

- **SHOULD — engineering rule:** Define the exact semantics of **Relationships** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for relationships is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for relationships, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Embedded data

- **SHOULD — engineering rule:** Define the exact semantics of **Embedded data** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for embedded data is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for embedded data, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. References

- **SHOULD — engineering rule:** Define the exact semantics of **References** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for references is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for references, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. One-to-one

- **SHOULD — engineering rule:** Define the exact semantics of **One-to-one** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for one-to-one is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for one-to-one, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. One-to-many

- **SHOULD — engineering rule:** Define the exact semantics of **One-to-many** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for one-to-many is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for one-to-many, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Many-to-many

- **SHOULD — engineering rule:** Define the exact semantics of **Many-to-many** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for many-to-many is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for many-to-many, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Optional relationships

- **SHOULD — engineering rule:** Define the exact semantics of **Optional relationships** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for optional relationships is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for optional relationships, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Polymorphic relationships

- **SHOULD — engineering rule:** Define the exact semantics of **Polymorphic relationships** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for polymorphic relationships is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for polymorphic relationships, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.12. Hierarchies

- **SHOULD — engineering rule:** Define the exact semantics of **Hierarchies** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for hierarchies is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for hierarchies, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. Metadata

- **SHOULD — engineering rule:** Define the exact semantics of **Metadata** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for metadata is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for metadata, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.14. Temporal data

- **SHOULD — engineering rule:** Define the exact semantics of **Temporal data** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for temporal data is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for temporal data, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.15. Versioned records

- **SHOULD — engineering rule:** Define the exact semantics of **Versioned records** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for versioned records is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for versioned records, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Database Modeling path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Tables`, `Embedded data`, `Many-to-many`. |
...
| Privacy / retention | Classify data produced or touched by Database Modeling; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Database Modeling** before choosing framework or provider mechanisms.
...
- **MUST** — For **Tables**: Define the exact semantics of **Tables** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Relationships**: Define the exact semantics of **Relationships** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **One-to-one**: Define the exact semantics of **One-to-one** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Polymorphic relationships**: Define the exact semantics of **Polymorphic relationships** within Database Modeling: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **Embed vs reference** according to the stated trade-off: Embed data with shared lifecycle and bounded size; reference independently owned or unbounded entities.
...
- [ ] **Tables:** Locate every implementation path for tables, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Relationships:** Locate every implementation path for relationships, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **One-to-one:** Locate every implementation path for one-to-one, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Polymorphic relationships:** Locate every implementation path for polymorphic relationships, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Temporal data:** Locate every implementation path for temporal data, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Denormalization:** Introduce missed/out-of-order updates and verify periodic comparison and deterministic rebuild.
...
- **Tables:** A framework or provider default for tables is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Relationships:** A framework or provider default for relationships is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **References:** A framework or provider default for references is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Many-to-many:** A framework or provider default for many-to-many is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Hierarchies:** A framework or provider default for hierarchies is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Temporal data:** A framework or provider default for temporal data is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Denormalization:** Partial writes or event loss cause durable divergence that no component owns.
...
- What exact invariant or user/system promise makes **Database Modeling** correct, and which component is authoritative for it?
...
- For **Tables**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for tables is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Relationships**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for relationships is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **One-to-one**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for one-to-one is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Polymorphic relationships**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for polymorphic relationships is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Temporal data**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for temporal data is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Database Modeling**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Tables:** Locate every implementation path for tables, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Relationships:** Locate every implementation path for relationships, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **References:** Locate every implementation path for references, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Many-to-many:** Locate every implementation path for many-to-many, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Hierarchies:** Locate every implementation path for hierarchies, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Temporal data:** Locate every implementation path for temporal data, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Denormalization:** Introduce missed/out-of-order updates and verify periodic comparison and deterministic rebuild.
...
- [020. Money / Numeric Precision](../primitives/020-money-numeric-precision.md) — layer: `primitives`; profile: `data_model`.
- [022. Database Constraints](../primitives/022-database-constraints.md) — layer: `primitives`; profile: `data_model`.
- [122. Data Provenance](../primitives/122-data-provenance.md) — layer: `primitives`; profile: `data_model`.
...
- [027. Indexing](027-indexing.md) — layer: `systems`; profile: `data_model`.
- [028. Query Design](028-query-design.md) — layer: `systems`; profile: `data_model`.
...
**Paper metadata:** canonical subtopics: 17; layer: `systems`; domain profile: `data_model`; verified through: `2026-08-17`.
