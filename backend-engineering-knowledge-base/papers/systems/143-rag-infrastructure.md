...
**Vector storage**, **Document deletion**.

### Security, privacy, and abuse

**Ingestion**, **Parsing**, **Chunking**, **Embeddings**, **Metadata**, **Access control**, **Retrieval**, **Reranking**, **Hybrid search**, **Citation mapping**.
...
This paper treats **RAG Infrastructure** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Ingestion:** Define the exact semantics of **Ingestion** within RAG Infrastructure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Embeddings:** Version parser, chunker, embedding model, metadata, ACLs, and source revision; retain byte/page offsets for citations and evaluate retrieval/reranking separately from generation.
- **SHOULD — Metadata:** Define the exact semantics of **Metadata** within RAG Infrastructure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Reranking:** Version parser, chunker, embedding model, metadata, ACLs, and source revision; retain byte/page offsets for citations and evaluate retrieval/reranking separately from generation.
- **SHOULD — Citation mapping:** Version parser, chunker, embedding model, metadata, ACLs, and source revision; retain byte/page offsets for citations and evaluate retrieval/reranking separately from generation.
- **SHOULD — Version synchronization:** Define the exact semantics of **Version synchronization** within RAG Infrastructure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Ingestion** within RAG Infrastructure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for ingestion is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for ingestion, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Parsing

- **SHOULD — engineering rule:** Define the exact semantics of **Parsing** within RAG Infrastructure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for parsing is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for parsing, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Vector storage** within RAG Infrastructure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for vector storage is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for vector storage, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Metadata

- **SHOULD — engineering rule:** Define the exact semantics of **Metadata** within RAG Infrastructure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Access control** within RAG Infrastructure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for access control is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for access control, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Retrieval

- **SHOULD — engineering rule:** Define the exact semantics of **Retrieval** within RAG Infrastructure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for retrieval is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for retrieval, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Hybrid search** within RAG Infrastructure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for hybrid search is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for hybrid search, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Citation mapping
...
### 7.13. Document deletion

- **MUST — engineering rule:** Define the exact semantics of **Document deletion** within RAG Infrastructure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for document deletion is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for document deletion, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.14. Version synchronization

- **SHOULD — engineering rule:** Define the exact semantics of **Version synchronization** within RAG Infrastructure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for version synchronization is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for version synchronization, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every RAG Infrastructure path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Ingestion`, `Embeddings`, `Access control`. |
...
| Privacy / retention | Classify data produced or touched by RAG Infrastructure; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **RAG Infrastructure** before choosing framework or provider mechanisms.
...
- **MUST** — For **Ingestion**: Define the exact semantics of **Ingestion** within RAG Infrastructure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Embeddings**: Version parser, chunker, embedding model, metadata, ACLs, and source revision; retain byte/page offsets for citations and evaluate retrieval/reranking separately from generation.
- **MUST** — For **Metadata**: Define the exact semantics of **Metadata** within RAG Infrastructure: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Reranking**: Version parser, chunker, embedding model, metadata, ACLs, and source revision; retain byte/page offsets for citations and evaluate retrieval/reranking separately from generation.
...
- [ ] **Ingestion:** Locate every implementation path for ingestion, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Embeddings:** Use a labeled retrieval corpus, deletion/ACL tests, reindex shadow comparison, and citation offset verification.
- [ ] **Metadata:** Locate every implementation path for metadata, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Reranking:** Use a labeled retrieval corpus, deletion/ACL tests, reindex shadow comparison, and citation offset verification.
- [ ] **Citation mapping:** Use a labeled retrieval corpus, deletion/ACL tests, reindex shadow comparison, and citation offset verification.
- [ ] **Version synchronization:** Locate every implementation path for version synchronization, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Ingestion:** A framework or provider default for ingestion is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Chunking:** Restart repeats or skips rows, live writes are overwritten by stale data, or the job saturates production.
- **Vector storage:** A framework or provider default for vector storage is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Access control:** A framework or provider default for access control is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Hybrid search:** A framework or provider default for hybrid search is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Reindexing:** Reindex runs without a golden retrieval-quality gate before cutover, the old index is deleted before rollback is possible, or dual storage costs are unbounded because no retirement trigger exists.
- **Version synchronization:** A framework or provider default for version synchronization is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **RAG Infrastructure** correct, and which component is authoritative for it?
...
- For **Ingestion**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for ingestion is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Embeddings**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Index entries cannot be traced to current source text, ACLs drift, or a model change makes old/new vectors incomparable.
- For **Metadata**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for metadata is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Reranking**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Index entries cannot be traced to current source text, ACLs drift, or a model change makes old/new vectors incomparable.
- For **Citation mapping**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Index entries cannot be traced to current source text, ACLs drift, or a model change makes old/new vectors incomparable.
...
- [ ] Map every entry point for **RAG Infrastructure**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Ingestion:** Locate every implementation path for ingestion, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Chunking:** Kill workers at every boundary, mutate rows during backfill, resume from checkpoints, and compare source/target counts and checksums.
- [ ] **Vector storage:** Locate every implementation path for vector storage, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Access control:** Locate every implementation path for access control, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Hybrid search:** Locate every implementation path for hybrid search, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Reindexing:** Capture explain plans and latency for representative cardinalities; rehearse create/drop/rebuild on production-like data.
- [ ] **Version synchronization:** Locate every implementation path for version synchronization, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
