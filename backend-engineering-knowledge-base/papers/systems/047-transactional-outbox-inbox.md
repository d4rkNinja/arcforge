...
# 047. Transactional Outbox / Inbox
...
**Delivery state**, **Failure recovery**, **Cleanup**.

### Persistence and integrity

**Outbox table**, **Transaction atomicity**.

### Concurrency and distributed behavior

**Outbox publishing**, **Polling**, **CDC-based publishing**, **Duplicate publication**, **Inbox deduplication**, **Ordering**.

### Boundary of the paper

This paper treats **Transactional Outbox / Inbox** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Outbox table:** Write domain state and outbox record in one local transaction; publish asynchronously with stable event IDs and consumer inbox deduplication. Track watermark, retries, ordering, and cleanup.
...
|---|---|---|
| Polling outbox vs CDC | Polling is simple and portable but adds scans/latency; CDC is efficient and ordered with log/connector operational complexity. | Select by volume and database platform; retain dedupe and monitoring either way. |
...
- **SHOULD — engineering rule:** Define the exact semantics of **Polling** within Transactional Outbox / Inbox: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for polling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for polling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Duplicate publication** within Transactional Outbox / Inbox: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for duplicate publication is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for duplicate publication, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Delivery state** within Transactional Outbox / Inbox: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for delivery state is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for delivery state, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Failure recovery

- **MUST — engineering rule:** Define the exact semantics of **Failure recovery** within Transactional Outbox / Inbox: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MUST — engineering rule:** Define the exact semantics of **Transaction atomicity** within Transactional Outbox / Inbox: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for transaction atomicity is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for transaction atomicity, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Transactional Outbox / Inbox path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Outbox table`, `Polling`, `Inbox deduplication`. |
...
| Privacy / retention | Classify data produced or touched by Transactional Outbox / Inbox; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Transactional Outbox / Inbox** before choosing framework or provider mechanisms.
...
- **MUST** — For **Outbox table**: Write domain state and outbox record in one local transaction; publish asynchronously with stable event IDs and consumer inbox deduplication. Track watermark, retries, ordering, and cleanup.
- **MUST** — For **Polling**: Define the exact semantics of **Polling** within Transactional Outbox / Inbox: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Duplicate publication**: Define the exact semantics of **Duplicate publication** within Transactional Outbox / Inbox: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Delivery state**: Define the exact semantics of **Delivery state** within Transactional Outbox / Inbox: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **Polling outbox vs CDC** according to the stated trade-off: Select by volume and database platform; retain dedupe and monitoring either way.
...
- **Outbox table:** State commits without an event, event publishes before rollback, or duplicate publication repeats effects.
- **Polling:** A framework or provider default for polling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Transactional Outbox / Inbox** correct, and which component is authoritative for it?
...
- For **Outbox table**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: State commits without an event, event publishes before rollback, or duplicate publication repeats effects.
...
- [ ] Map every entry point for **Transactional Outbox / Inbox**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
