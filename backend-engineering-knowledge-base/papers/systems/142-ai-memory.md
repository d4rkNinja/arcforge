...
**User memory**, **Session memory**, **Permission-aware memory**.
...
**Short-term memory**, **Long-term memory**, **Retrieval**, **Memory ranking**, **Provenance**, **Conflict resolution**, **Deletion**, **Privacy**.

### Boundary of the paper

This paper treats **AI Memory** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Short-term memory:** Measure under representative load before changing code; correlate profiles with request classes, queueing, GC pauses, I/O, and tail latency. Preserve correctness and observability while optimizing.
- **SHOULD — User memory:** Measure under representative load before changing code; correlate profiles with request classes, queueing, GC pauses, I/O, and tail latency. Preserve correctness and observability while optimizing.
- **SHOULD — Retrieval:** Store memory with subject/tenant scope, provenance, confidence, timestamps, permissions, retention, and deletion linkage; rank and resolve conflicts without converting inference into fact.
- **SHOULD — Provenance:** Store memory with subject/tenant scope, provenance, confidence, timestamps, permissions, retention, and deletion linkage; rank and resolve conflicts without converting inference into fact.
- **MUST — Permission-aware memory:** Define authorization as `(principal, action, resource, context) -> decision + reason + policy version`. Enforce at the owning boundary and include tenant/resource state, not only route-level roles.
- **MUST — Privacy:** Define the exact semantics of **Privacy** within AI Memory: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|---|
| Ephemeral vs durable AI memory | Short-term context limits retention and privacy risk; durable memory improves continuity but creates provenance, permission, staleness, and deletion obligations. | Persist only justified, permission-scoped facts with expiry and provenance. |
...
- **SHOULD — engineering rule:** Store memory with subject/tenant scope, provenance, confidence, timestamps, permissions, retention, and deletion linkage; rank and resolve conflicts without converting inference into fact.
- **Production failure mode:** A stale or cross-user memory influences behavior, deleted data remains retrievable, or an untrusted statement becomes a durable profile.
- **Existing-codebase evidence:** Test conflicting memories, permission changes, deletion, expiry, source retraction, and cross-tenant retrieval.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Conflict resolution** within AI Memory: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MUST — engineering rule:** Define the exact semantics of **Deletion** within AI Memory: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MUST — engineering rule:** Define the exact semantics of **Privacy** within AI Memory: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every AI Memory path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Short-term memory`, `Session memory`, `Expiration`. |
...
| Privacy / retention | Classify data produced or touched by AI Memory; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **AI Memory** before choosing framework or provider mechanisms.
...
- **MUST** — For **Short-term memory**: Measure under representative load before changing code; correlate profiles with request classes, queueing, GC pauses, I/O, and tail latency. Preserve correctness and observability while optimizing.
- **MUST** — For **User memory**: Measure under representative load before changing code; correlate profiles with request classes, queueing, GC pauses, I/O, and tail latency. Preserve correctness and observability while optimizing.
- **MUST** — For **Retrieval**: Store memory with subject/tenant scope, provenance, confidence, timestamps, permissions, retention, and deletion linkage; rank and resolve conflicts without converting inference into fact.
- **MUST** — For **Provenance**: Store memory with subject/tenant scope, provenance, confidence, timestamps, permissions, retention, and deletion linkage; rank and resolve conflicts without converting inference into fact.
...
- **MAY** — Choose **Ephemeral vs durable AI memory** according to the stated trade-off: Persist only justified, permission-scoped facts with expiry and provenance.
...
- [ ] **Short-term memory:** Capture before/after profiles and end-to-end metrics with equivalent workloads and statistical confidence.
- [ ] **User memory:** Capture before/after profiles and end-to-end metrics with equivalent workloads and statistical confidence.
- [ ] **Retrieval:** Test conflicting memories, permission changes, deletion, expiry, source retraction, and cross-tenant retrieval.
- [ ] **Provenance:** Test conflicting memories, permission changes, deletion, expiry, source retraction, and cross-tenant retrieval.
- [ ] **Permission-aware memory:** Trace each read/write from externally supplied identifier to the final query/serializer and verify denial before data exposure or mutation.
- [ ] **Privacy:** Locate every implementation path for privacy, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- What exact invariant or user/system promise makes **AI Memory** correct, and which component is authoritative for it?
...
- For **Short-term memory**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A micro-optimization shifts pressure to memory, database, or network and worsens production tails.
- For **User memory**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A micro-optimization shifts pressure to memory, database, or network and worsens production tails.
- For **Retrieval**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A stale or cross-user memory influences behavior, deleted data remains retrievable, or an untrusted statement becomes a durable profile.
- For **Provenance**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A stale or cross-user memory influences behavior, deleted data remains retrievable, or an untrusted statement becomes a durable profile.
- For **Permission-aware memory**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: An authenticated caller accesses another object, hidden field, or state transition because lookup and authorization are separated or policy inputs are incomplete.
...
- [ ] Map every entry point for **AI Memory**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
**Paper metadata:** canonical subtopics: 12; layer: `systems`; domain profile: `ai`; verified through: `2026-08-17`.
