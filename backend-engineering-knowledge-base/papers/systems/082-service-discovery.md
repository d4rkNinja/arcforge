...
**Service Discovery** exists to make process startup, steady-state execution, and termination deterministic across environments and orchestrators. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**DNS discovery**, **Registry discovery**, **Static discovery**, **Dynamic discovery**, **Health checks**, **Service registration**, **Deregistration**, **Stale endpoints**.

### Boundary of the paper

This paper treats **Service Discovery** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — DNS discovery:** Respect TTL and resolver caching behavior, handle multiple addresses and endpoint churn, drain removed instances, and avoid assuming a DNS lookup is a permanent binding.
- **SHOULD — Registry discovery:** Define the exact semantics of **Registry discovery** within Service Discovery: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Dynamic discovery:** Define the exact semantics of **Dynamic discovery** within Service Discovery: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Health checks:** Liveness should detect irrecoverable process failure without depending on remote services; readiness should reflect ability to serve the routed workload; startup probes protect slow initialization.
- **SHOULD — Deregistration:** Define the exact semantics of **Deregistration** within Service Discovery: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Stale endpoints:** Respect TTL and resolver caching behavior, handle multiple addresses and endpoint churn, drain removed instances, and avoid assuming a DNS lookup is a permanent binding.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Registry discovery** within Service Discovery: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for registry discovery is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for registry discovery, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Static discovery

- **SHOULD — engineering rule:** Define the exact semantics of **Static discovery** within Service Discovery: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for static discovery is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for static discovery, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Dynamic discovery

- **SHOULD — engineering rule:** Define the exact semantics of **Dynamic discovery** within Service Discovery: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for dynamic discovery is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for dynamic discovery, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Service registration** within Service Discovery: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for service registration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for service registration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Deregistration

- **SHOULD — engineering rule:** Define the exact semantics of **Deregistration** within Service Discovery: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for deregistration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for deregistration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Service Discovery path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `DNS discovery`, `Static discovery`, `Health checks`. |
...
| Privacy / retention | Classify data produced or touched by Service Discovery; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Service Discovery** before choosing framework or provider mechanisms.
...
- **MUST** — For **DNS discovery**: Respect TTL and resolver caching behavior, handle multiple addresses and endpoint churn, drain removed instances, and avoid assuming a DNS lookup is a permanent binding.
- **MUST** — For **Registry discovery**: Define the exact semantics of **Registry discovery** within Service Discovery: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Dynamic discovery**: Define the exact semantics of **Dynamic discovery** within Service Discovery: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Health checks**: Liveness should detect irrecoverable process failure without depending on remote services; readiness should reflect ability to serve the routed workload; startup probes protect slow initialization.
...
- [ ] **DNS discovery:** Rotate endpoints under long-lived connections and test TTL, negative caching, partial DNS failure, and drain.
- [ ] **Registry discovery:** Locate every implementation path for registry discovery, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Dynamic discovery:** Locate every implementation path for dynamic discovery, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Deregistration:** Locate every implementation path for deregistration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Stale endpoints:** Rotate endpoints under long-lived connections and test TTL, negative caching, partial DNS failure, and drain.
...
- **DNS discovery:** Clients pin dead addresses, failover waits for hidden caches, or stale endpoints receive writes after removal.
- **Registry discovery:** A framework or provider default for registry discovery is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Static discovery:** A framework or provider default for static discovery is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Service registration:** A framework or provider default for service registration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Deregistration:** A framework or provider default for deregistration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Stale endpoints:** Clients pin dead addresses, failover waits for hidden caches, or stale endpoints receive writes after removal.
...
- What exact invariant or user/system promise makes **Service Discovery** correct, and which component is authoritative for it?
...
- For **DNS discovery**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Clients pin dead addresses, failover waits for hidden caches, or stale endpoints receive writes after removal.
- For **Registry discovery**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for registry discovery is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Dynamic discovery**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for dynamic discovery is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Health checks**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A dependency outage triggers restart storms, or a process receives traffic before initialization and migrations are complete.
- For **Deregistration**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for deregistration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Service Discovery**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **DNS discovery:** Rotate endpoints under long-lived connections and test TTL, negative caching, partial DNS failure, and drain.
- [ ] **Registry discovery:** Locate every implementation path for registry discovery, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Static discovery:** Locate every implementation path for static discovery, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Health checks:** Fail each dependency separately and verify probe response, routing, restart behavior, and probe cost under fleet scale.
- [ ] **Service registration:** Locate every implementation path for service registration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Deregistration:** Locate every implementation path for deregistration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Stale endpoints:** Rotate endpoints under long-lived connections and test TTL, negative caching, partial DNS failure, and drain.
...
**Paper metadata:** canonical subtopics: 8; layer: `systems`; domain profile: `runtime`; verified through: `2026-08-17`.
