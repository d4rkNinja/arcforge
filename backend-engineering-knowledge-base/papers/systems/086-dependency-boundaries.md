...
**Layer boundaries**, **Module boundaries**, **Domain boundaries**, **Circular dependencies**, **Dependency inversion**, **Public interfaces**, **Internal interfaces**, **Shared libraries**, **Shared models**, **Dependency leakage**.

### Boundary of the paper

This paper treats **Dependency Boundaries** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — Layer boundaries:** Define the exact semantics of **Layer boundaries** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Domain boundaries:** Define the exact semantics of **Domain boundaries** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Dependency inversion:** Pin reproducibly, review transitive code and licenses, generate SBOM/provenance, monitor advisories, test upgrades, and maintain a replacement/removal plan for critical dependencies.
- **SHOULD — Public interfaces:** Define the exact semantics of **Public interfaces** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Shared libraries:** Define the exact semantics of **Shared libraries** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Dependency leakage:** Define the exact semantics of **Dependency leakage** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Layer boundaries** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for layer boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for layer boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Module boundaries

- **SHOULD — engineering rule:** Define the exact semantics of **Module boundaries** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for module boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for module boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Domain boundaries

- **SHOULD — engineering rule:** Define the exact semantics of **Domain boundaries** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for domain boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for domain boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Circular dependencies

- **SHOULD — engineering rule:** Assign one owner for invariants and writes, expose a narrow versioned interface, enforce dependency direction mechanically, and communicate cross-boundary facts through explicit calls/events.
...
- **SHOULD — engineering rule:** Pin reproducibly, review transitive code and licenses, generate SBOM/provenance, monitor advisories, test upgrades, and maintain a replacement/removal plan for critical dependencies.
- **Production failure mode:** Unreviewed transitive changes or compromised packages enter builds; emergency upgrades break behavior with no compatibility evidence.
- **Existing-codebase evidence:** Rebuild from a clean environment, verify lock/SBOM/signatures, scan vulnerabilities/licenses, and rehearse a critical dependency upgrade.

### 7.6. Public interfaces

- **SHOULD — engineering rule:** Define the exact semantics of **Public interfaces** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for public interfaces is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for public interfaces, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Internal interfaces

- **SHOULD — engineering rule:** Define the exact semantics of **Internal interfaces** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for internal interfaces is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for internal interfaces, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Shared libraries

- **SHOULD — engineering rule:** Define the exact semantics of **Shared libraries** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for shared libraries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for shared libraries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Shared models

- **SHOULD — engineering rule:** Define the exact semantics of **Shared models** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for shared models is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for shared models, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Dependency leakage

- **SHOULD — engineering rule:** Define the exact semantics of **Dependency leakage** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for dependency leakage is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for dependency leakage, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Dependency Boundaries path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Layer boundaries`, `Domain boundaries`, `Dependency inversion`. |
...
| Privacy / retention | Classify data produced or touched by Dependency Boundaries; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Dependency Boundaries** before choosing framework or provider mechanisms.
...
- **MUST** — For **Layer boundaries**: Define the exact semantics of **Layer boundaries** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Domain boundaries**: Define the exact semantics of **Domain boundaries** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Dependency inversion**: Pin reproducibly, review transitive code and licenses, generate SBOM/provenance, monitor advisories, test upgrades, and maintain a replacement/removal plan for critical dependencies.
- **MUST** — For **Public interfaces**: Define the exact semantics of **Public interfaces** within Dependency Boundaries: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Layer boundaries:** Locate every implementation path for layer boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Domain boundaries:** Locate every implementation path for domain boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Dependency inversion:** Rebuild from a clean environment, verify lock/SBOM/signatures, scan vulnerabilities/licenses, and rehearse a critical dependency upgrade.
- [ ] **Public interfaces:** Locate every implementation path for public interfaces, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Shared libraries:** Locate every implementation path for shared libraries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Dependency leakage:** Locate every implementation path for dependency leakage, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Layer boundaries:** A framework or provider default for layer boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Domain boundaries:** A framework or provider default for domain boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Circular dependencies:** Modules share tables/models/utilities until changes require lockstep deployment and no team can reason locally.
- **Dependency inversion:** Unreviewed transitive changes or compromised packages enter builds; emergency upgrades break behavior with no compatibility evidence.
- **Internal interfaces:** A framework or provider default for internal interfaces is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Shared models:** A framework or provider default for shared models is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Dependency leakage:** A framework or provider default for dependency leakage is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Dependency Boundaries** correct, and which component is authoritative for it?
...
- For **Layer boundaries**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for layer boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Domain boundaries**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for domain boundaries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Dependency inversion**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Unreviewed transitive changes or compromised packages enter builds; emergency upgrades break behavior with no compatibility evidence.
- For **Public interfaces**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for public interfaces is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Shared libraries**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for shared libraries is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Dependency Boundaries**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Layer boundaries:** Locate every implementation path for layer boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Domain boundaries:** Locate every implementation path for domain boundaries, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Circular dependencies:** Build an import/dependency graph, map table writes, and flag cross-boundary direct access.
- [ ] **Dependency inversion:** Rebuild from a clean environment, verify lock/SBOM/signatures, scan vulnerabilities/licenses, and rehearse a critical dependency upgrade.
- [ ] **Internal interfaces:** Locate every implementation path for internal interfaces, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Shared models:** Locate every implementation path for shared models, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Dependency leakage:** Locate every implementation path for dependency leakage, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [087. Code-Level Architecture](087-code-level-architecture.md) — layer: `systems`; profile: `architecture`.
...
- [136. Legacy-System Integration](136-legacy-system-integration.md) — layer: `systems`; profile: `migration`.
...
**Paper metadata:** canonical subtopics: 10; layer: `systems`; domain profile: `architecture`; verified through: `2026-08-17`.
