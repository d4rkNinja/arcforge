---
paper_number: 80
title: "Networking Basics for Backend"
layer: systems
...
# 080. Networking Basics for Backend
...
**Networking Basics for Backend** exists to make process startup, steady-state execution, and termination deterministic across environments and orchestrators. A basic implementation usually handles the visible happy path; a production implementation must also preserve identity and ownership, valid state transitions, race-safe invariants, failure recovery, compatibility, and bounded operations.
...
**DNS**, **TCP**, **UDP**, **HTTP/1.1**, **HTTP/2**, **HTTP/3**, **QUIC**, **Proxies**, **Reverse proxies**, **NAT**, **Keep-alive**, **Connection reuse**.

### Boundary of the paper

This paper treats **Networking Basics for Backend** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — DNS:** Respect TTL and resolver caching behavior, handle multiple addresses and endpoint churn, drain removed instances, and avoid assuming a DNS lookup is a permanent binding.
- **SHOULD — HTTP/1.1:** Define the exact semantics of **HTTP/1.1** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — QUIC:** Define the exact semantics of **QUIC** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Proxies:** Define the exact semantics of **Proxies** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Load balancers:** Choose routing from connection duration, state locality, health, skew, and failure behavior. Ensure draining and retries do not duplicate unsafe work.
- **SHOULD — Network partitions:** Choose a key from access patterns, cardinality, locality, tenant size, growth, and rebalancing needs; design routing metadata and cross-shard operations before scale forces migration.
...
- **SHOULD — engineering rule:** Define the exact semantics of **TCP** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for tcp is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for tcp, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. UDP

- **SHOULD — engineering rule:** Define the exact semantics of **UDP** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for udp is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for udp, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. HTTP/1.1

- **SHOULD — engineering rule:** Define the exact semantics of **HTTP/1.1** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for http/1.1 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for http/1.1, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. HTTP/2

- **SHOULD — engineering rule:** Define the exact semantics of **HTTP/2** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for http/2 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for http/2, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. HTTP/3

- **SHOULD — engineering rule:** Define the exact semantics of **HTTP/3** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for http/3 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for http/3, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. QUIC

- **SHOULD — engineering rule:** Define the exact semantics of **QUIC** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for quic is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for quic, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. TLS

- **SHOULD — engineering rule:** Define the exact semantics of **TLS** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Proxies** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for proxies is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for proxies, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Reverse proxies

- **SHOULD — engineering rule:** Define the exact semantics of **Reverse proxies** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for reverse proxies is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for reverse proxies, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. NAT

- **SHOULD — engineering rule:** Define the exact semantics of **NAT** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for nat is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for nat, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Keep-alive** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for keep-alive is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for keep-alive, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.14. Connection reuse

- **SHOULD — engineering rule:** Define the exact semantics of **Connection reuse** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for connection reuse is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for connection reuse, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.15. Network partitions
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Networking Basics for Backend path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `DNS`, `HTTP/2`, `TLS`. |
...
| Privacy / retention | Classify data produced or touched by Networking Basics for Backend; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Networking Basics for Backend** before choosing framework or provider mechanisms.
...
- **MUST** — For **DNS**: Respect TTL and resolver caching behavior, handle multiple addresses and endpoint churn, drain removed instances, and avoid assuming a DNS lookup is a permanent binding.
- **MUST** — For **HTTP/1.1**: Define the exact semantics of **HTTP/1.1** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **QUIC**: Define the exact semantics of **QUIC** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Proxies**: Define the exact semantics of **Proxies** within Networking Basics for Backend: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **DNS:** Rotate endpoints under long-lived connections and test TTL, negative caching, partial DNS failure, and drain.
- [ ] **HTTP/1.1:** Locate every implementation path for http/1.1, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **QUIC:** Locate every implementation path for quic, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Proxies:** Locate every implementation path for proxies, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Load balancers:** Remove/add instances under load, create skewed keys, and verify drain, failover, and session/state behavior.
- [ ] **Network partitions:** Replay skewed production-like distributions and rehearse split/move/failover with concurrent reads and writes.
...
- **DNS:** Clients pin dead addresses, failover waits for hidden caches, or stale endpoints receive writes after removal.
- **UDP:** A framework or provider default for udp is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **HTTP/3:** A framework or provider default for http/3 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Reverse proxies:** A framework or provider default for reverse proxies is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Keep-alive:** A framework or provider default for keep-alive is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Network partitions:** Monotonic or celebrity keys overload one shard, cross-shard queries become the norm, or resharding blocks writes.
...
- What exact invariant or user/system promise makes **Networking Basics for Backend** correct, and which component is authoritative for it?
...
- For **DNS**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Clients pin dead addresses, failover waits for hidden caches, or stale endpoints receive writes after removal.
- For **HTTP/1.1**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for http/1.1 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **QUIC**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for quic is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Proxies**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for proxies is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Load balancers**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Sticky keys create hot instances, unhealthy connections persist, or retry routes a completed write to another backend.
...
- [ ] Map every entry point for **Networking Basics for Backend**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **DNS:** Rotate endpoints under long-lived connections and test TTL, negative caching, partial DNS failure, and drain.
- [ ] **UDP:** Locate every implementation path for udp, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **HTTP/3:** Locate every implementation path for http/3, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **TLS:** Locate every implementation path for tls, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Reverse proxies:** Locate every implementation path for reverse proxies, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Keep-alive:** Locate every implementation path for keep-alive, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Network partitions:** Replay skewed production-like distributions and rehearse split/move/failover with concurrent reads and writes.
...
- [083. Service-to-Service Communication](083-service-to-service-communication.md) — layer: `systems`; profile: `runtime`.
- [098. Distributed Systems Fundamentals](../cross-cutting/098-distributed-systems-fundamentals.md) — layer: `cross-cutting`; profile: `transactions`.
- [065. TLS / PKI](../cross-cutting/065-tls-pki.md) — layer: `cross-cutting`; profile: `security`.
...
**Paper metadata:** canonical subtopics: 15; layer: `systems`; domain profile: `runtime`; verified through: `2026-08-17`.
