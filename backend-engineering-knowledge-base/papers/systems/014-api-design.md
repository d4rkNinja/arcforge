...
**REST**, **RPC**, **gRPC**, **GraphQL**, **Async APIs**, **Command design**, **Endpoint naming**, **Request contracts**, **Response contracts**, **HTTP methods**, **Status codes**, **Partial updates**, **Bulk APIs**, **Batch APIs**, **Sparse fields**, **API consistency**.
...
**Resource design**, **Nested resources**, **Resource expansion**.

### Boundary of the paper

This paper treats **API Design** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — REST:** Define the exact semantics of **REST** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Async APIs:** Define the exact semantics of **Async APIs** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Request contracts:** Define the exact semantics of **Request contracts** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Status codes:** Preserve protocol semantics because clients, caches, proxies, and retry logic depend on them. Document safe/idempotent behavior, cacheability, and machine-readable error contracts.
- **SHOULD — Nested resources:** Define the exact semantics of **Nested resources** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — API documentation:** Define the exact semantics of **API documentation** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **REST** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **RPC** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for rpc is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for rpc, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. gRPC

- **SHOULD — engineering rule:** Define the exact semantics of **gRPC** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **GraphQL** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for graphql is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for graphql, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Async APIs

- **SHOULD — engineering rule:** Define the exact semantics of **Async APIs** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for async apis is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for async apis, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Resource design

- **SHOULD — engineering rule:** Define the exact semantics of **Resource design** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for resource design is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for resource design, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Command design

- **SHOULD — engineering rule:** Define the exact semantics of **Command design** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for command design is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for command design, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Endpoint naming

- **SHOULD — engineering rule:** Define the exact semantics of **Endpoint naming** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for endpoint naming is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for endpoint naming, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Request contracts

- **SHOULD — engineering rule:** Define the exact semantics of **Request contracts** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for request contracts is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for request contracts, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.10. Response contracts

- **SHOULD — engineering rule:** Define the exact semantics of **Response contracts** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for response contracts is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for response contracts, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. HTTP methods

- **SHOULD — engineering rule:** Preserve protocol semantics because clients, caches, proxies, and retry logic depend on them. Document safe/idempotent behavior, cacheability, and machine-readable error contracts.
- **Production failure mode:** A misleading method or status causes unsafe retries, stale caches, broken clients, or invisible failures.
- **Existing-codebase evidence:** Contract-test actual wire responses through the production proxy/gateway, including errors, redirects, conditional requests, and empty bodies.
...
### 7.13. Partial updates

- **SHOULD — engineering rule:** Define the exact semantics of **Partial updates** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for partial updates is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for partial updates, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.14. Bulk APIs

- **SHOULD — engineering rule:** Define the exact semantics of **Bulk APIs** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for bulk apis is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for bulk apis, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.15. Batch APIs

- **SHOULD — engineering rule:** Define the exact semantics of **Batch APIs** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for batch apis is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for batch apis, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.16. Nested resources

- **SHOULD — engineering rule:** Define the exact semantics of **Nested resources** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for nested resources is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for nested resources, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.17. Sparse fields

- **SHOULD — engineering rule:** Define the exact semantics of **Sparse fields** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for sparse fields is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for sparse fields, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.18. Resource expansion

- **SHOULD — engineering rule:** Define the exact semantics of **Resource expansion** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for resource expansion is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for resource expansion, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.19. API consistency

- **SHOULD — engineering rule:** Define the exact semantics of **API consistency** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for api consistency is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for api consistency, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.20. API documentation

- **SHOULD — engineering rule:** Define the exact semantics of **API documentation** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for api documentation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for api documentation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every API Design path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `REST`, `Resource design`, `HTTP methods`. |
...
| Privacy / retention | Classify data produced or touched by API Design; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **API Design** before choosing framework or provider mechanisms.
...
- **MUST** — For **REST**: Define the exact semantics of **REST** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Async APIs**: Define the exact semantics of **Async APIs** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Request contracts**: Define the exact semantics of **Request contracts** within API Design: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Status codes**: Preserve protocol semantics because clients, caches, proxies, and retry logic depend on them. Document safe/idempotent behavior, cacheability, and machine-readable error contracts.
...
- [ ] **Async APIs:** Locate every implementation path for async apis, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Request contracts:** Locate every implementation path for request contracts, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Status codes:** Contract-test actual wire responses through the production proxy/gateway, including errors, redirects, conditional requests, and empty bodies.
- [ ] **Nested resources:** Locate every implementation path for nested resources, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **API documentation:** Locate every implementation path for api documentation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **GraphQL:** A framework or provider default for graphql is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Command design:** A framework or provider default for command design is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **HTTP methods:** A misleading method or status causes unsafe retries, stale caches, broken clients, or invisible failures.
- **Bulk APIs:** A framework or provider default for bulk apis is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Sparse fields:** A framework or provider default for sparse fields is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **API documentation:** A framework or provider default for api documentation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **API Design** correct, and which component is authoritative for it?
...
- For **Async APIs**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for async apis is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Request contracts**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for request contracts is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Status codes**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A misleading method or status causes unsafe retries, stale caches, broken clients, or invisible failures.
- For **Nested resources**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for nested resources is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **API Design**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **REST:** Locate every implementation path for rest, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **GraphQL:** Locate every implementation path for graphql, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Command design:** Locate every implementation path for command design, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **HTTP methods:** Contract-test actual wire responses through the production proxy/gateway, including errors, redirects, conditional requests, and empty bodies.
- [ ] **Bulk APIs:** Locate every implementation path for bulk apis, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Sparse fields:** Locate every implementation path for sparse fields, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **API documentation:** Locate every implementation path for api documentation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [013. Error Architecture](../primitives/013-error-architecture.md) — layer: `primitives`; profile: `api`.
- [012. Input Validation](../primitives/012-input-validation.md) — layer: `primitives`; profile: `api`.
- [017. Filtering / Sorting / Query APIs](../primitives/017-filtering-sorting-query-apis.md) — layer: `primitives`; profile: `api`.
- [011. Request Lifecycle](../primitives/011-request-lifecycle.md) — layer: `primitives`; profile: `api`.
- [016. Pagination](../primitives/016-pagination.md) — layer: `primitives`; profile: `api`.
- [015. API Versioning & Compatibility](015-api-versioning-and-compatibility.md) — layer: `systems`; profile: `api`.
- [116. Data Serialization](../primitives/116-data-serialization.md) — layer: `primitives`; profile: `api`.
- [070. API / Event Schema Evolution](../primitives/070-api-event-schema-evolution.md) — layer: `primitives`; profile: `migration`.
- [018. Identifiers](../primitives/018-identifiers.md) — layer: `primitives`; profile: `data_model`.
- [110. SDK / Client Design](110-sdk-client-design.md) — layer: `systems`; profile: `client`.
- [049. Webhooks](049-webhooks.md) — layer: `systems`; profile: `api`.
...
**Paper metadata:** canonical subtopics: 20; layer: `systems`; domain profile: `api`; verified through: `2026-08-17`.
