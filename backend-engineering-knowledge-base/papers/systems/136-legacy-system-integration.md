...
**Anti-corruption layers**, **Adapters**, **Translation layers**, **Legacy identifiers**, **Partial migration**, **Strangler pattern**, **Compatibility**.

### Boundary of the paper

This paper treats **Legacy-System Integration** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Anti-corruption layers** within Legacy-System Integration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for anti-corruption layers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for anti-corruption layers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. Adapters

- **SHOULD — engineering rule:** Define the exact semantics of **Adapters** within Legacy-System Integration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Translation layers** within Legacy-System Integration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for translation layers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for translation layers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Data synchronization

- **SHOULD — engineering rule:** Define the exact semantics of **Data synchronization** within Legacy-System Integration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for data synchronization is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for data synchronization, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Legacy identifiers

- **SHOULD — engineering rule:** Define the exact semantics of **Legacy identifiers** within Legacy-System Integration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for legacy identifiers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for legacy identifiers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.6. Partial migration

- **SHOULD — engineering rule:** Define the exact semantics of **Partial migration** within Legacy-System Integration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for partial migration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for partial migration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.7. Strangler pattern

- **SHOULD — engineering rule:** Define the exact semantics of **Strangler pattern** within Legacy-System Integration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for strangler pattern is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for strangler pattern, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.8. Compatibility

- **SHOULD — engineering rule:** Define the exact semantics of **Compatibility** within Legacy-System Integration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Legacy-System Integration path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `Anti-corruption layers`, `Translation layers`, `Legacy identifiers`. |
...
| Privacy / retention | Classify data produced or touched by Legacy-System Integration; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Legacy-System Integration** before choosing framework or provider mechanisms.
...
- **MUST** — For **Anti-corruption layers**: Define the exact semantics of **Anti-corruption layers** within Legacy-System Integration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Adapters**: Define the exact semantics of **Adapters** within Legacy-System Integration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Data synchronization**: Define the exact semantics of **Data synchronization** within Legacy-System Integration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Legacy identifiers**: Define the exact semantics of **Legacy identifiers** within Legacy-System Integration: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- [ ] **Anti-corruption layers:** Locate every implementation path for anti-corruption layers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [ ] **Data synchronization:** Locate every implementation path for data synchronization, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Legacy identifiers:** Locate every implementation path for legacy identifiers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Strangler pattern:** Locate every implementation path for strangler pattern, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **Anti-corruption layers:** A framework or provider default for anti-corruption layers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Adapters:** A framework or provider default for adapters is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Translation layers:** A framework or provider default for translation layers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Legacy identifiers:** A framework or provider default for legacy identifiers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Partial migration:** A framework or provider default for partial migration is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Strangler pattern:** A framework or provider default for strangler pattern is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Legacy-System Integration** correct, and which component is authoritative for it?
...
- For **Anti-corruption layers**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for anti-corruption layers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- For **Data synchronization**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for data synchronization is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Legacy identifiers**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for legacy identifiers is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Strangler pattern**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for strangler pattern is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Legacy-System Integration**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Anti-corruption layers:** Locate every implementation path for anti-corruption layers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Adapters:** Locate every implementation path for adapters, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Translation layers:** Locate every implementation path for translation layers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Legacy identifiers:** Locate every implementation path for legacy identifiers, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Partial migration:** Locate every implementation path for partial migration, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Strangler pattern:** Locate every implementation path for strangler pattern, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Compatibility:** Locate every implementation path for compatibility, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
**Paper metadata:** canonical subtopics: 8; layer: `systems`; domain profile: `migration`; verified through: `2026-08-17`.
