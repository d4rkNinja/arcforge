...
# 005. OAuth / Social Authentication
...
**OAuth 2.0**, **OpenID Connect**, **Authorization code flow**, **PKCE**, **Provider token validation**, **Apple Private Relay**, **Apple first-authorization behavior**, **Provider account linking**, **Provider account unlinking**, **Existing-password-account linking**, **Duplicate provider identities**, **Provider token refresh**, **Provider outages**.

### State and lifecycle

**State validation**, **Provider revocation**.

### Contracts and validation

**Nonce validation**, **Redirect URI validation**, **Missing email**, **Unverified email**, **Changed email**, **Hidden email**.
...
- **MUST — OAuth 2.0:** Define the exact semantics of **OAuth 2.0** within OAuth / Social Authentication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST — State validation:** Generate high-entropy one-time values, bind them to the initiating browser/session and redirect intent, enforce short expiry, and consume atomically.
- **SHOULD — Provider subject identifiers:** Persist the issuer/provider namespace together with the provider's stable subject. Never infer identity from display name or mutable profile attributes.
- **SHOULD — Apple Private Relay:** Persist the first authorization payload and provider subject immediately; some Apple profile fields are returned only once and relay addresses are aliases, not durable real-email evidence.
- **SHOULD — Existing-password-account linking:** Use a modern password hashing function with per-record salt and centrally versioned work factors; compare in constant-time libraries, throttle attempts, and support transparent rehash after successful authentication.
- **SHOULD — Provider outages:** Classify provider failures, set deadlines and bulkheads, respect rate limits, preserve request identity for safe retry/reconciliation, and define whether fallback changes semantics, data residency, or user consent.
...
|---|---|---|
| Automatic account linking vs explicit linking | Automatic linking reduces friction but turns provider/email assurance mistakes into takeover; explicit linking adds a reauthentication ceremony. | Require authenticated proof of both accounts unless a provider-specific, verified, policy-approved rule is demonstrably safe. |
...
- **MUST — engineering rule:** Define the exact semantics of **OAuth 2.0** within OAuth / Social Authentication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for oauth 2.0 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for oauth 2.0, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. OpenID Connect

- **SHOULD — engineering rule:** Define the exact semantics of **OpenID Connect** within OAuth / Social Authentication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for openid connect is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for openid connect, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Authorization code flow

- **MUST — engineering rule:** Define the exact semantics of **Authorization code flow** within OAuth / Social Authentication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for authorization code flow is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for authorization code flow, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. PKCE

- **SHOULD — engineering rule:** Use PKCE with a fresh high-entropy verifier and `S256`; bind the verifier to the authorization transaction and never expose it in logs or URLs.
- **Production failure mode:** A stolen authorization code can be redeemed by another client when the verifier is absent, weak, reused, or mismatched.
- **Existing-codebase evidence:** Test verifier reuse, wrong verifier, plain method rejection where policy requires S256, and callback replay.

### 7.5. State validation

- **MUST — engineering rule:** Generate high-entropy one-time values, bind them to the initiating browser/session and redirect intent, enforce short expiry, and consume atomically.
- **Production failure mode:** Replay, login CSRF, authorization response injection, or mix-up attacks succeed when values are predictable, reusable, or not bound to the correct transaction.
- **Existing-codebase evidence:** Test missing, altered, expired, cross-session, and concurrently reused values; verify atomic consume semantics.

### 7.6. Nonce validation

- **MUST — engineering rule:** Generate high-entropy one-time values, bind them to the initiating browser/session and redirect intent, enforce short expiry, and consume atomically.
- **Production failure mode:** Replay, login CSRF, authorization response injection, or mix-up attacks succeed when values are predictable, reusable, or not bound to the correct transaction.
- **Existing-codebase evidence:** Test missing, altered, expired, cross-session, and concurrently reused values; verify atomic consume semantics.

### 7.7. Redirect URI validation

- **MUST — engineering rule:** Use exact pre-registered redirect URI matching except where a governing specification explicitly permits a narrow exception. Reject user-controlled forwarding after callback.
- **Production failure mode:** Prefix, wildcard, or open-redirect matching leaks codes or tokens to an attacker-controlled origin.
- **Existing-codebase evidence:** Enumerate every registered URI and add negative tests for scheme, host, port, path, encoding, case, fragment, and open-redirect variations.

### 7.8. Provider token validation

- **MUST — engineering rule:** Define the exact semantics of **Provider token validation** within OAuth / Social Authentication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for provider token validation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for provider token validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.9. Provider subject identifiers
...
- **SHOULD — engineering rule:** Make email optional in the federated identity contract. Store provider verification metadata separately and require an application-controlled verification step before using email for recovery or linking.
- **Production failure mode:** Signup fails for legitimate users, or an unverified/recycled address is trusted to link an attacker to an existing account.
- **Existing-codebase evidence:** Exercise provider fixtures with no email, private relay email, changed email, and `email_verified=false`.

### 7.11. Unverified email
...
### 7.14. Apple Private Relay

- **SHOULD — engineering rule:** Persist the first authorization payload and provider subject immediately; some Apple profile fields are returned only once and relay addresses are aliases, not durable real-email evidence.
- **Production failure mode:** A retry after a partial failure loses name/email data, or later logic assumes the relay address equals the user's permanent address.
- **Existing-codebase evidence:** Review callback persistence ordering and test a first authorization followed by subsequent authorizations that omit profile fields.

### 7.15. Apple first-authorization behavior

- **MUST — engineering rule:** Persist the first authorization payload and provider subject immediately; some Apple profile fields are returned only once and relay addresses are aliases, not durable real-email evidence.
- **Production failure mode:** A retry after a partial failure loses name/email data, or later logic assumes the relay address equals the user's permanent address.
- **Existing-codebase evidence:** Review callback persistence ordering and test a first authorization followed by subsequent authorizations that omit profile fields.

### 7.16. Provider account linking

- **SHOULD — engineering rule:** Define the exact semantics of **Provider account linking** within OAuth / Social Authentication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for provider account linking is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for provider account linking, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.17. Provider account unlinking

- **SHOULD — engineering rule:** Define the exact semantics of **Provider account unlinking** within OAuth / Social Authentication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for provider account unlinking is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for provider account unlinking, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.18. Existing-password-account linking
...
### 7.19. Duplicate provider identities

- **SHOULD — engineering rule:** Define the exact semantics of **Duplicate provider identities** within OAuth / Social Authentication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for duplicate provider identities is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for duplicate provider identities, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.20. Provider token refresh

- **SHOULD — engineering rule:** Define the exact semantics of **Provider token refresh** within OAuth / Social Authentication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for provider token refresh is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for provider token refresh, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.21. Provider revocation

- **MUST — engineering rule:** Define the exact semantics of **Provider revocation** within OAuth / Social Authentication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for provider revocation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for provider revocation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.22. Provider outages

- **SHOULD — engineering rule:** Classify provider failures, set deadlines and bulkheads, respect rate limits, preserve request identity for safe retry/reconciliation, and define whether fallback changes semantics, data residency, or user consent.
- **Production failure mode:** The system retries unsafe requests, sends duplicates to two providers, or silently degrades to a provider with incompatible behavior.
- **Existing-codebase evidence:** Inject DNS/TLS/timeout/429/5xx/partial-success cases and reconcile provider state after ambiguous outcomes.
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `OAuth 2.0`, `Nonce validation`, `Unverified email`. |
...
| Privacy / retention | Classify data produced or touched by OAuth / Social Authentication; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **OAuth / Social Authentication** before choosing framework or provider mechanisms.
...
- **MUST** — For **OAuth 2.0**: Define the exact semantics of **OAuth 2.0** within OAuth / Social Authentication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **State validation**: Generate high-entropy one-time values, bind them to the initiating browser/session and redirect intent, enforce short expiry, and consume atomically.
- **MUST** — For **Provider subject identifiers**: Persist the issuer/provider namespace together with the provider's stable subject. Never infer identity from display name or mutable profile attributes.
- **MUST** — For **Apple Private Relay**: Persist the first authorization payload and provider subject immediately; some Apple profile fields are returned only once and relay addresses are aliases, not durable real-email evidence.
...
- **MAY** — Choose **Automatic account linking vs explicit linking** according to the stated trade-off: Require authenticated proof of both accounts unless a provider-specific, verified, policy-approved rule is demonstrably safe.
...
- [ ] **OAuth 2.0:** Locate every implementation path for oauth 2.0, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **State validation:** Test missing, altered, expired, cross-session, and concurrently reused values; verify atomic consume semantics.
- [ ] **Provider subject identifiers:** Verify uniqueness on `(issuer, subject)` and test account lookup after provider email/profile changes.
- [ ] **Apple Private Relay:** Review callback persistence ordering and test a first authorization followed by subsequent authorizations that omit profile fields.
- [ ] **Existing-password-account linking:** Inspect stored format and parameters, benchmark work factors, test legacy-hash upgrade, enumeration resistance, and reset-token invalidation.
- [ ] **Provider outages:** Inject DNS/TLS/timeout/429/5xx/partial-success cases and reconcile provider state after ambiguous outcomes.
...
- **OAuth 2.0:** A framework or provider default for oauth 2.0 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **State validation:** Replay, login CSRF, authorization response injection, or mix-up attacks succeed when values are predictable, reusable, or not bound to the correct transaction.
- **Provider token validation:** A framework or provider default for provider token validation is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Unverified email:** Signup fails for legitimate users, or an unverified/recycled address is trusted to link an attacker to an existing account.
- **Apple first-authorization behavior:** A retry after a partial failure loses name/email data, or later logic assumes the relay address equals the user's permanent address.
- **Duplicate provider identities:** A framework or provider default for duplicate provider identities is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Provider outages:** The system retries unsafe requests, sends duplicates to two providers, or silently degrades to a provider with incompatible behavior.
...
- What exact invariant or user/system promise makes **OAuth / Social Authentication** correct, and which component is authoritative for it?
...
- For **OAuth 2.0**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for oauth 2.0 is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **State validation**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Replay, login CSRF, authorization response injection, or mix-up attacks succeed when values are predictable, reusable, or not bound to the correct transaction.
- For **Provider subject identifiers**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Two providers can issue the same subject string, while one provider can change email without changing subject; either mistake creates collision or duplicate accounts.
- For **Apple Private Relay**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A retry after a partial failure loses name/email data, or later logic assumes the relay address equals the user's permanent address.
- For **Existing-password-account linking**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Fast hashes, global salts, insecure reset flows, or unbounded attempts turn a database leak or credential stuffing campaign into account takeover.
...
- [ ] Map every entry point for **OAuth / Social Authentication**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **OAuth 2.0:** Locate every implementation path for oauth 2.0, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **State validation:** Test missing, altered, expired, cross-session, and concurrently reused values; verify atomic consume semantics.
- [ ] **Provider token validation:** Locate every implementation path for provider token validation, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Unverified email:** Exercise provider fixtures with no email, private relay email, changed email, and `email_verified=false`.
- [ ] **Apple first-authorization behavior:** Review callback persistence ordering and test a first authorization followed by subsequent authorizations that omit profile fields.
- [ ] **Duplicate provider identities:** Locate every implementation path for duplicate provider identities, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Provider outages:** Inject DNS/TLS/timeout/429/5xx/partial-success cases and reconcile provider state after ambiguous outcomes.
...
**Paper metadata:** canonical subtopics: 22; layer: `systems`; domain profile: `authentication`; verified through: `2026-08-17`.
