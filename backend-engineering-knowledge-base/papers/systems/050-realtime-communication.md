...
**WebSockets**, **SSE**, **Long polling**, **Connections**, **Reconnection**, **Heartbeats**, **Presence**, **Channels**, **Rooms**, **Broadcast**, **Offline clients**, **Horizontal scaling**, **Connection limits**.

### Concurrency and distributed behavior

**Message ordering**, **Message loss**.

### Boundary of the paper

This paper treats **Realtime Communication** as a production subsystem or reusable reasoning primitive. It covers semantics, ownership, persistence, APIs, security, concurrency, distributed behavior, operations, evolution, and verification. Framework syntax and large implementation listings are intentionally excluded.
...
- **SHOULD — WebSockets:** Define the exact semantics of **WebSockets** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Connections:** Define the exact semantics of **Connections** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Connection expiry:** Authenticate the connection and periodically revalidate expiry/authorization; use heartbeats to detect half-open connections, exponential reconnect with jitter, resumable cursors where supported, and bounded offline buffers.
- **SHOULD — Channels:** Define the exact semantics of **Channels** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Message loss:** Define the exact semantics of **Message loss** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **SHOULD — Connection limits:** Define the exact semantics of **Connection limits** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
|---|---|---|
| WebSocket vs SSE | WebSockets are bidirectional; SSE is simpler server-to-client over HTTP but text/event-stream specific. | Choose from directionality, intermediaries, reconnect semantics, and scale. |
...
- **SHOULD — engineering rule:** Define the exact semantics of **WebSockets** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for websockets is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for websockets, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.2. SSE

- **SHOULD — engineering rule:** Define the exact semantics of **SSE** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for sse is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for sse, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.3. Long polling

- **SHOULD — engineering rule:** Define the exact semantics of **Long polling** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for long polling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for long polling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.4. Connections

- **SHOULD — engineering rule:** Define the exact semantics of **Connections** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for connections is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for connections, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.5. Reconnection

- **SHOULD — engineering rule:** Authenticate the connection and periodically revalidate expiry/authorization; use heartbeats to detect half-open connections, exponential reconnect with jitter, resumable cursors where supported, and bounded offline buffers.
- **Production failure mode:** Revoked users retain live access, reconnect storms overload the service, or messages disappear/duplicate around disconnect.
- **Existing-codebase evidence:** Expire credentials mid-connection, drop one network direction, restart nodes, and reconnect thousands of clients simultaneously.

### 7.6. Authentication

- **MUST — engineering rule:** Define the exact semantics of **Authentication** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MUST — engineering rule:** Define the exact semantics of **Authorization** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **SHOULD — engineering rule:** Define the exact semantics of **Presence** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for presence is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for presence, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.11. Channels

- **SHOULD — engineering rule:** Define the exact semantics of **Channels** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for channels is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for channels, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.12. Rooms

- **SHOULD — engineering rule:** Define the exact semantics of **Rooms** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for rooms is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for rooms, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.13. Broadcast

- **SHOULD — engineering rule:** Define the exact semantics of **Broadcast** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for broadcast is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for broadcast, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.14. Message ordering
...
- **SHOULD — engineering rule:** Define the exact semantics of **Message loss** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for message loss is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for message loss, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.16. Offline clients

- **SHOULD — engineering rule:** Authenticate the connection and periodically revalidate expiry/authorization; use heartbeats to detect half-open connections, exponential reconnect with jitter, resumable cursors where supported, and bounded offline buffers.
- **Production failure mode:** Revoked users retain live access, reconnect storms overload the service, or messages disappear/duplicate around disconnect.
- **Existing-codebase evidence:** Expire credentials mid-connection, drop one network direction, restart nodes, and reconnect thousands of clients simultaneously.

### 7.17. Horizontal scaling

- **SHOULD — engineering rule:** Define the exact semantics of **Horizontal scaling** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for horizontal scaling is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for horizontal scaling, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.

### 7.18. Connection limits

- **SHOULD — engineering rule:** Define the exact semantics of **Connection limits** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **Production failure mode:** A framework or provider default for connection limits is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Existing-codebase evidence:** Locate every implementation path for connection limits, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
|---|---|
| Authentication / actor | Identify the human, service, device, worker, or anonymous actor for every Realtime Communication path; do not inherit ambient identity across asynchronous or administrative boundaries. |
...
| Validation / canonicalization | Define syntax, semantic invariants, unknown-field behavior, size/complexity limits, normalization, and error mapping for `WebSockets`, `Reconnection`, `Heartbeats`. |
...
| Privacy / retention | Classify data produced or touched by Realtime Communication; minimize collection, define access and export, propagate deletion, and address logs, derived stores, backups, and legal hold. |
...
- **MUST** — Define the authoritative owner, invariants, lifecycle, and trust boundary for **Realtime Communication** before choosing framework or provider mechanisms.
...
- **MUST** — For **WebSockets**: Define the exact semantics of **WebSockets** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Connections**: Define the exact semantics of **Connections** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
- **MUST** — For **Connection expiry**: Authenticate the connection and periodically revalidate expiry/authorization; use heartbeats to detect half-open connections, exponential reconnect with jitter, resumable cursors where supported, and bounded offline buffers.
- **MUST** — For **Channels**: Define the exact semantics of **Channels** within Realtime Communication: owner, inputs, outputs, invariants, lifecycle, failure classification, and compatibility contract. Make the rule enforceable at the narrowest authoritative boundary.
...
- **MAY** — Choose **WebSocket vs SSE** according to the stated trade-off: Choose from directionality, intermediaries, reconnect semantics, and scale.
...
- [ ] **WebSockets:** Locate every implementation path for websockets, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Connections:** Locate every implementation path for connections, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Connection expiry:** Expire credentials mid-connection, drop one network direction, restart nodes, and reconnect thousands of clients simultaneously.
- [ ] **Channels:** Locate every implementation path for channels, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Message loss:** Locate every implementation path for message loss, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Connection limits:** Locate every implementation path for connection limits, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- **WebSockets:** A framework or provider default for websockets is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Connections:** A framework or provider default for connections is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- **Heartbeats:** Revoked users retain live access, reconnect storms overload the service, or messages disappear/duplicate around disconnect.
- **Rooms:** A framework or provider default for rooms is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Message loss:** A framework or provider default for message loss is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- **Connection limits:** A framework or provider default for connection limits is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- What exact invariant or user/system promise makes **Realtime Communication** correct, and which component is authoritative for it?
...
- For **WebSockets**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for websockets is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Connections**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for connections is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Connection expiry**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: Revoked users retain live access, reconnect storms overload the service, or messages disappear/duplicate around disconnect.
- For **Channels**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for channels is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
- For **Message loss**, what authoritative boundary enforces the rule, and how will the team prove the failure described here cannot occur: A framework or provider default for message loss is accepted without proving it matches the domain, causing ambiguous state, race-sensitive behavior, or an operational gap.
...
- [ ] Map every entry point for **Realtime Communication**: public/internal APIs, middleware, jobs, event handlers, migrations, admin tools, CLIs, scripts, and tests.
...
- [ ] **Heartbeats:** Expire credentials mid-connection, drop one network direction, restart nodes, and reconnect thousands of clients simultaneously.
- [ ] **Rooms:** Locate every implementation path for rooms, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Message loss:** Locate every implementation path for message loss, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
- [ ] **Connection limits:** Locate every implementation path for connection limits, compare behavior across APIs, jobs, migrations, and admin tooling, and add evidence for normal, invalid, duplicate, concurrent, timed-out, and recovery cases.
...
- [014. API Design](014-api-design.md) — layer: `systems`; profile: `api`.
...
**Paper metadata:** canonical subtopics: 18; layer: `systems`; domain profile: `api`; verified through: `2026-08-17`.
