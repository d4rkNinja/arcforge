...
- Canonical implementation corpus, packaging pipeline, and corpus validation: `backend-engineering-knowledge-base/`
- Repository documentation: `README.md`, `docs/`, `CONTRIBUTING.md`, and `SECURITY.md`
- Behavioral pressure cases: `evals/`

## Implementation paper pipeline

- The canonical corpus is `backend-engineering-knowledge-base/papers/`; it is validated by `tools/validate_corpus.py` and never edited in place casually.
- Skill copies under `skills/*/references/papers/` are generated by `tools/package_papers.py`, which strips corpus bookkeeping, removes template boilerplate duplicated across sections (keeping each distinct statement once), collapses fully-templated subtopics into a "Default obligations" list, front-loads the pre-implementation questions and existing-codebase checks, renumbers sections, and rewrites cross-paper links. To change a paper, edit the canonical corpus, re-run the packager, and update checksums and manifest word counts through the corpus validation flow.
- Do not hand-edit generated paper copies under `skills/`; edit the corpus and re-package.
