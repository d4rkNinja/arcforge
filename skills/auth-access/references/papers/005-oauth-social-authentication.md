...
### 8.6. Nonce validation

- **MUST — engineering rule:** Generate high-entropy one-time values, bind them to the initiating browser/session and redirect intent, enforce short expiry, and consume atomically.
- **Production failure mode:** Replay, login CSRF, authorization response injection, or mix-up attacks succeed when values are predictable, reusable, or not bound to the correct transaction.
- **Existing-codebase evidence:** Test missing, altered, expired, cross-session, and concurrently reused values; verify atomic consume semantics.

### 8.7. Redirect URI validation
...
### 8.9. Provider subject identifiers
...
### 8.15. Apple first-authorization behavior
...
### 8.18. Existing-password-account linking
