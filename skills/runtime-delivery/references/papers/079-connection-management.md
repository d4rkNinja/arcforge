...
### 8.2. HTTP connection pools

- **SHOULD — engineering rule:** Size pools from downstream capacity and instance count, bound acquisition time, instrument wait/usage, validate connections, and always release through structured cleanup/cancellation.
- **Production failure mode:** Scale-out opens more connections than the database can serve, or leaked/slow operations make every request wait indefinitely.
- **Existing-codebase evidence:** Load-test at maximum replicas, inject slow queries/network resets, and assert bounded wait plus zero leaked checkouts.

### 8.5. Maximum connections
...
### 8.8. Connection leaks

- **SHOULD — engineering rule:** Size pools from downstream capacity and instance count, bound acquisition time, instrument wait/usage, validate connections, and always release through structured cleanup/cancellation.
- **Production failure mode:** Scale-out opens more connections than the database can serve, or leaked/slow operations make every request wait indefinitely.
- **Existing-codebase evidence:** Load-test at maximum replicas, inject slow queries/network resets, and assert bounded wait plus zero leaked checkouts.

### 8.9. Connection timeout
...
- [083. Service-to-Service Communication](083-service-to-service-communication.md)
